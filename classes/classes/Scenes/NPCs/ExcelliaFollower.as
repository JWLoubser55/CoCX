...
public function humanCow(strHuman:String, strCow:String):String {
    if (flags[kFLAGS.EXCELLIA_LA_BOVA] == 2) //cow face
        return strCow;
    if (flags[kFLAGS.EXCELLIA_RECRUITED] > 2) //follower
        return flags[kFLAGS.EXCELLIA_LA_BOVA] == 0 && flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1 ? strHuman : strCow; //any bova stuff = not pure human
    else return flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 3 ? strHuman : strCow; //sucmilk - slower TF... but probova=1 is still okay
}

public function girlRacial():String {
    if (flags[kFLAGS.EXCELLIA_LA_BOVA] == 2) //cow face
        return "cow-morph";
    if (flags[kFLAGS.EXCELLIA_RECRUITED] > 2) //follower
        return flags[kFLAGS.EXCELLIA_LA_BOVA] == 0 && flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1 ? "girl" : "cowgirl"; //any bova stuff = not pure human
    else return flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 3 ? "demon-morph" : "cowgirl"; //sucmilk - slower TF... but probova=1 is still okay
}

public function fixed():Boolean {
    return flags[kFLAGS.EXCELLIA_RECRUITED] > 32;
} 
...
public function ExcelliaCampMainMenuFixHer2():void {
	if (fixed()) {
		outputText("When you approach, the sizable [exc race] turns to you with a warm smile on her face.\n\n");
	...
	if (fixed()) addButton(14, "Back", camp.campLoversMenu);
...
public function ExcelliaCampFixHerAppearance():void {
	...
	outputText("When Excellia catches you staring, "+ (flags[kFLAGS.EXCELLIA_HUMMUS_OR_SUCCUBUS_MILK] == 1 ? "she giggles then blows a kiss at you" : "she giggles a bit as her tail swishes excitedly back and forth") +".\n\n");
	menu();
...
public function ExcelliaCampFixHerHerself():void {
	...
	if (fixed()) {
		...
		outputText("You can't help but smile at hearing the [exc race]'s kind words. She pulls you into a near crushing hug to which you gladly return.\n\n");
	}
	else {
...
public function ExcelliaMilkEffects():void {
	pc.HPChange((((player.level * 20) + 25) * 2 * (1 + player.newGamePlusMod())), true, false);
...
public function ExcelliaCampFixHerGiveItemsDeBimbo():void {
	...
	if (flags[kFLAGS.EXCELLIA_RECRUITED] > 23) {
		...
		outputText("She lets out a faint [exc moo] as the potion kicks in. When she finally calms down, she looks up at you. You can see her eyes brightened and become clearer. She looks around, fully taking in her surroundings then looks back up at you.\n\n");
		outputText("\"<i>A-Ah… [name]... [name]... I can… finally think straight for once.</i>\"\n\n");
		outputText("She smiles at you with tears streaming down her face. Before you can say anything the [exc race] pulls you into a tight hug, crying into your shoulder.\n\n");
		...
		outputText("You tell the ex-cow queen to close her eyes and open her mouth. She smiles excitedly then does what she's told. You bring out the concentrated brew then put it up to her lips. She quickly slurps down the brew in mere seconds. Shortly after, her eyes shoot open in shock then she clutches her head. She lets out a pained [exc moo] as the potion kicks in. ");
...
public function ExcelliaCampFixHerSexFuckHer():void {
	...
	if (fixed()) outputText("\"<i>A-Ah please [name]... I want you…</i>\"\n\n");
    ...
    function maleF():void {
		...
		outputText("Excellia wraps her legs around your waist to pull you closer and force you deeper inside her. You grip her hips tighter as you begin fucking her in earnest. She [exc moo]s out louder feeling your [cock] rubbing against her sensitive walls. The way her huge tits jiggle with each thrust is too much to ignore. You reach up and grab her pillowy breasts. Your fingers sink into the supple flesh as you knead and squeeze her milk filled jugs. Creamy milk leaks from her large teats, dripping down the curve of her perky breasts. You lean down and lap at the sweet cream letting your tongue swipe over her nipples.\n\n");
		outputText("The dual sensation of drinking her milk and pounding away at her pussy "+(player.cockTotal() > 1 ? "and ass ":"")+"becomes too much for the [exc race]. She pulls you up into a tongue filled kiss [exc moo]ing into your mouth as she cums. Her pussy "+(player.cockTotal() > 1 ? "and ass ":"")+"clamps down on your dick"+(player.cockTotal() > 1 ? "s":"")+" like a vice as a gush of femcum gushes out around your [cock] drenching your crotch. You can’t help but moan out when your orgasm tears through you. ");
		outputText("Your dick"+(player.cockTotal() > 1 ? "s":"")+" twitches and erupts inside the still orgasming [exc race]. Your hot seed fills her utterly as some dribbles back out and drips down into the growing puddle beneath her. Excellia doesn’t let you go until you empty the last of your spunk inside her.\n\n");
		outputText("Spent, you pull out of the panting [exc race], shivering as you leave her silky confines. She [exc moo]s lowly when a stream of your cum drools from her well used snatch"+(player.cockTotal() > 1 ? " and ass":"")+". She looks up at you with a tired smile.\n\n");
    ...
    function femF():void {
        outputText("You smirk as you slip a hand down to her soaked pussy. She lets out a tiny gasp when your fingers roam over her soft vulva. You rub your thumb over her big pleasure button. You can feel it grow and harden from its hood to a full 1.5 inches. You pinch it gently between your fingers, making the [exc race] [exc moo] lowly. You smirk slipping one of your digits inside with ease. She shudders, feeling your finger rubbing against her sensitive walls. ");
		outputText("You decide to slip another finger inside her, exploring her innermost depths. She lets out a moan when you brush over her sweet spot. You continue rubbing over it, watching the [exc race] squirm and moan as your skillful digits take advantage of her most sensitive area.\n\n");
    ...
    function sharedEnd():void {
        if (fixed()) outputText("\"<i>Oh [name]... That was just amazing… Oooo, I’m still tingling!</i>\"\n\n");
		else outputText("\"<i>A-Ah thanking you my " + player.mf("Lord", "Lady") + "... You truly are amazing...</i>\"\n\n");
        outputText("You can’t help but feel the same way. You lay next to the [exc race] nuzzling into her arms for a rest. After a quick nap, you help her get cleaned up and say your goodbyes to her as you prepare to head back out.\n\n");
        if (flags[kFLAGS.EXCELLIA_RECRUITED] < 32) flags[kFLAGS.EXCELLIA_RECRUITED]++;
        doNext(camp.returnToCampUseOneHour);
    }
}
...
public function ExcelliaCampFixHerSexOralFun():void {
	...
	if (fixed()) outputText("\"<i>Ah… That tongue of yours is devious. I hope you use it on me again VERY soon [name].</i>\"\n\n");
...
public function ExcelliaCampFixHerSexSuckleFuck():void {
	...
	outputText("Unconsciously, you wrap your lips around one of her milk dripping teats causing her to let out a small [exc moo]. You gently suckle and lap at her teat until a steady stream of her sweet milk pours into your mouth. Her sweet cream invigorates and ebbs away the aches and pains from your adventures.\n\n");
	outputText("Though you would be content to just sit there and gulp down all the milk she can provide, the constant aching in your loins is becoming too much to ignore. You lift her up in your lap, not once releasing her teat from your mouth, and press the head of your dick against her soaked entrance. The [exc race] gasps then lets out a drawn out [exc moo] when the head of your dick slips past her folds and sinks into her a few inches. You gently guide her down until her hips finally meets yours. You shiver feeling her warm heavenly walls wrap around you so tightly as if they didn't want to let you go.\n\n");
	outputText("Empowered by her restorative milk, you waste no time pumping into her depths. Excellia pants and moans louder as you fuck and greedily suckle from her. The stream flowing into your mouth gradually increases in intensity as it nearly turns into a river. You do your best to keep up with the deluge, struggling to gulp down the cream, just as another takes its place. Her other free breast bounces with each hard thrust dribbling its milk on you and the [exc race]. Upset to see such amazing milk go to waste, you push her huge milky pillows together and stuff both teats into your mouth. At that moment your [cock] brushes against the [exc race]'s sensitive walls causing her to [exc moo] louder then grab the back of your head, mashing your face into her breasts. ");
	outputText("You struggle to breathe as a flood of her milk pours into your mouth. You quickly gulp down as much of it as you can just to avoid suffocating or drowning. She eagerly bounces in your lap riding your dick hard until she's finally pushed over the edge. She [exc moo]s loudly when her orgasm hits, the combination of your nursing and pounding at her soaking pussy becoming too much. Working on very little air, you quickly pump you hips as you fuck her harder and faster to reach your peak as well. Just as darkness is about to set in, your orgasm tears through you, surging up your dick and blasting the still orgasming [exc race]'s depths with your hot seed.\n\n");
	outputText("Just as your consciousness is about to slip, your head is abruptly pulled back, letting the [exc race]'s leaking teats pop from your mouth and gulping in some much needed air. You cough and sputter, trying to catch your breath. You can feel a hand stroking the back of your head as you try to regain your bearings.\n\n");
	if (fixed()) outputText("\"<i>I'm so sorry [name]! It just felt so good and I just couldn't control myself!</i>\"\n\n");
	else outputText("\"<i>Oh my apologies my " + player.mf("Lord", "Lady") + "! I might've gotten a bit carried away...</i>\"\n\n");
	outputText("You cough a bit telling the [exc race] that it's okay. You might've gotten a bit carried away yourself to be honest. You untangle yourself from her taking note of your bulging stomach. It sloshes and groans from the amount of milk you drank. At least you won't be hungry for a while and you feel more refreshed than ever. You kiss Excellia thanking her for such a wonderful meal. She smiles, glad you aren't upset over the near boob suffocation. She helps you get cleaned up as you get ready to head back out.\n\n");
	if (flags[kFLAGS.EXCELLIA_RECRUITED] < 32) flags[kFLAGS.EXCELLIA_RECRUITED]++;
	if (flags[kFLAGS.EXCELLIA_RECRUITED] >= 33) excelliaPreg();
...
public function ExcelliaCampFixHerSleepToggle():void {
	...
	if(flags[kFLAGS.SLEEP_WITH] != "Excellia") {
		...
		outputText("You tell the [exc race] girl that you would prefer to sleep alone at night. Excellia huffs then pouts.\n\n");
...
public function ExcelliaCampMakeSlaveGiveItemsProBova():void {
	...
	else outputText("You force Excellia to take the tainted la bova, making her gulp it down. She [exc moo]s and shudders when it has an immediate effect on her. She groans when her legs reshape into bovine hooves covered in smooth red fur. Her nipples grow out and multiply, forming into four long teats.\n\n");
	flags[kFLAGS.EXCELLIA_LA_BOVA]++;
	doNext(camp.returnToCampUseOneHour);
}
...
public function ExcelliaCampMakeSlaveSexFuckHer():void {
    ...
    function maleF():void {
        ...
        if (player.cockTotal() > 1) outputText("You grab her hips then pull back until only the heads of your dicks are inside her.");
        else outputText("You grab her hips then pull back until only the head of your dick is inside her.");
		outputText(" She’s about to complain when you slam back inside her eliciting a loud moan from her. You begin a fast pace, thrusting into her silky depths. You can’t help but groan feeling her inner walls hug and massage you in ways you didn’t think possible. The tightness in your lower belly slowly builds as you’re pushed closer to the edge but you do your best to force it back down to prolong this wonderful feeling.\n\n");
		...
		outputText("The dual sensation of drinking her milk and pounding away at her pussy "+(player.cockTotal() > 1 ? "and ass become":"becomes")+" too much for the [exc slut]. She pulls you up into a tongue filled kiss [exc moo]ing into your mouth as she cums. Her pussy "+(player.cockTotal() > 1 ? "and ass clamp":" clamps")+" down on your dick"+(player.cockTotal() > 1 ? "s":"")+" like a vice as a gush of femcum gushes out around your [cock] drenching your crotch. Your can’t help but moan out when your orgasm tears through you. ");
		outputText("Your dick"+(player.cockTotal() > 1 ? "s twitch and erupt":"twitches and erupts")+" inside the still orgasming ex-cow queen. Your hot seed fills her utterly as some dribbles back out and drips down into the growing puddle beneath her. Excellia doesn’t let you go until you empty the last of you spunk inside her.\n\n");
		outputText("Spent, you pull out of the panting [exc race] jerking off your dick"+(player.cockTotal() > 1 ? "s":"")+" and dribbling what little cum you have let onto her chest and stomach. She [exc moo]s lowly when a stream of your cum drools from her well used snatch"+(player.cockTotal() > 1 ? " and ass":"")+". She looks up at you with a tired smile.\n\n");
		...
		outputText("You smirk patting your loyal slave on the head telling her she performed adequately. You lay next to the [exc race] as she nuzzles against you for a rest. After a quick nap, you get cleaned up and leave the still sleeping dairy [exc slut] as you prepare to head back out.\n\n");
		excelliaPreg();
        player.sexReward("vaginalFluids","Dick");
	    doNext(camp.returnToCampUseOneHour);
    }
    function femF():void {
		outputText("You grin as you slip a hand down to her soaked pussy. She lets out a tiny gasp when your fingers roam over her soft vulva. You rub your thumb over her big pleasure button. You can feel it grow and harden from its hood to a full 1.5 inches. You pinch it between your fingers making the [exc slut] [exc moo] lowly. You smirk slipping one of your digits inside with ease. She shudders, feeling your finger rubbing against her sensitive walls. ");
...
public function ExcelliaCampMakeSlaveSexMilkyMassage():void {
	...
	outputText("The dairy [exc slut] giggles then climbs on top of you to straddle your waist, making a point to grind her wet pussy against your "+(player.hasCock() ? "[cock]":"")+(player.gender == 3 ? " and ":"")+(player.hasVagina() ? "moist snatch":"")+". She reaches up to grope and knead her own chest. She rubs over her milk leaking nipples, the sweet cream dribbling between her fingers down onto your stomach. Soon the cream leaking from her teats turn into a steady stream as it pours down onto you and coats your [skin]. You shudder, feeling a strange warmth slowly spreading through your body.\n\n");
	outputText("The dairy [exc slut] smiles then leans over you, rubbing the milk into your [skin]. The warmth deepens as she massages you, her touches brushing over your most sensitive areas. You moan when she continues grinding against your "+(player.hasCock() ? "hardened cock":"")+(player.gender == 3 ? " and ":"")+(player.hasVagina() ? "pussy":"")+". Her hands roam over your body as more of her milk pours over you. It feels surprisingly good, your muscles relax and loosen from the milky massage. Excellia moans with you as she continues grinding against you. Her hips grinds against yours more feverishly, her hands desperately rubbing and grabbing at your [skin].\n\n");
	outputText("The combination of the mounting pressure and warmth spreading through your body becomes too much. You moan out loud as an orgasm rips through you. "+(player.hasCock() ? "You cum, painting your stomach and chest with ropes hot seed. ":"")+(player.hasVagina() ? "Your pussy clenches tightly, milking at a phantom cock. ":"")+"The dairy cow [exc moo]s as she virtually drenches you in femcum and her sweet milk already coating your body.\n\n");
	outputText("You shudder as the pleasure slowly passes. Your body feels more relaxed than ever before. The [exc slut]slut [exc moo]s quietly on top of you, leaning in and giving you a deep kiss. "+(player.hasCock() ? "Your [cock] surges back to hardness as her aphrodisiac laced lips begin to have an effect on you. ":"")+"She climbs off of you, allowing you to get up. You stretch, all the aches and knots in your body are completely gone.\n\n");
...
private function ExcelliaPoopsBabies2():void {
	...
	clearOutput();
	player.createStatusEffect(StatusEffects.ExcelliaChildbirth, rand(2), 0, 0, 0);
	outputText("You can hear loud [exc moo]ing coming from nearby. You follow the source of the noise until it leads you to Excellia sitting near your "+(flags[kFLAGS.CAMP_CABIN_FURNITURE_BED] > 0 ? "cabin":"tent")+". The [exc race] sits back with her legs splayed wide and a hand on her rounded belly, panting heavily. Her breasts practically leak like a running faucet and her vagina is dilated wide as a gush of fluids rushes out pulling underneath her.\n\n");
	outputText("\"<i>M-My " + player.mf("Lord", "Lady") + " Ah yes the baby is-ah-coming!</i>\"\n\n");
	outputText("You smirk, coming to the [exc race]'s side, encouraging her to expel your soon-to-born child. She groans in pain when a strong contraction rocks through her. You can see the faint outline of your child slowly worming their way down her birth canal. Excellia [exc moo]s as she pushes to help the child along.\n\n");
	outputText("A delicious thought comes to mind as the [exc race] strains to push the child out that you just can't pass up. You slip between her legs then lean down to give an experimental lick to her spasming pussy lips. The fluid has a bit of a tangy yet sweet taste, not at all unpleasant. She lets out a low moan that slowly turns into a [exc moo]. Seemingly having an effect on her, you push her legs further apart then lap at her sensitive pussy and suck on her hardened clitty. She nearly squeals as pain slowly mixes with pleasure. Your child seems to move faster down her birth canal. Soon enough you can see the head peeking between her lips. She strains as the shoulders stretches her wider but with one final push the child slips out of her into your waiting arms with a gush of fluids.\n\n");
