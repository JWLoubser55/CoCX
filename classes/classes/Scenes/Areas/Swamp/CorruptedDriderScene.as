...
//*Victory Rape: PC Penis goes in Vagina (done)
		private function winDriderPCDickInSpiderCunt():void
		{
			...
			if (monster.HP < 1) outputText("giving up little, lewd moans");
			else outputText("breathing out lurid moans");
			outputText(" of pleasure and her ");
			if (monster.nipplesPierced > 0) outputText("chain-pierced-");
			outputText("nipples and clit look as hard as her natural body-armor.  You boldly grope one of her ");
			...
			outputText("Meanwhile, the drider's swollen clit prods at your loins with each penetrating thrust, leaving lines of sticky spider-juice all over your crotch.  ");
			if (player.cockTotal() > 1) {
				outputText("Your extra penis");
				if (player.cockTotal() > 2) outputText("es fan out to the side, forgotten while you give yourself over to the silken caresses of her glorious vagina.");
			...
			outputText("She lets your head hang limply, and all you can do is whine, and try to look up at her with the most pathetic expression in your eyes.  You're beaten - she's so much smarter than you and she's got you by your dick.  Why didn't you think?  ");

			if (player.cor > 80 && player.lib > 50) outputText("Gods it was a bad idea not to bind her lips... ");
			else outputText("Gods this was a bad idea... ");
			...
			outputText("The spider-woman shudders from head to all eight of her 'toes', her six eyelids closing while her mouth gasps out, \"<i>Cum for me.</i>\"\n\n");

			outputText("The first spurt of pent-up seed splatters the spider's womb before you even realize she gave you permission, painting the squirming walls white.  You growl and cum, releasing spurt after spurt of spunk while her pussy seems to drink it all down");
			if (player.cumQ() > 1000) outputText(", even though you've packed it full once already");
			outputText(".  Her legs skitter and tremble, her pussy convulses, and your dick gets the squeezing, sucking orgasm of a lifetime.  It feels like you're pumping out two or three orgasms worth of spooge, each dick-stretching explosion of cream dilating your slit almost painfully wide.");
			if (player.cumQ() > 1000) {
				outputText("  The stuff dribbles out around your tool along with a mix of girl-lube, too much even for her prodigious pussy to contain.");
				if (player.cumQ() > 2000) outputText("  The flow thickens into a river, and soon the puddle of cum looks more like a pond, turning the dirt into a soup of muddy cum.");
			}
			outputText("\n\n");
...
//Part 2 of the above
		private function winDriderPCDickInSpiderCunt2():void
		{
			...
			outputText("You collapse into the dirt, not even caring that it stinks of dried cum and girl-jizz.  You've completely emptied out every speck of sperm from your [balls], and you even kept going for a while after that.  It wasn't until " + sMultiCockDesc() + " went soft at last that you found enough peace to lie down.  ");
			if (player.cor > 80 && player.lib > 50) outputText("The longer you sit there, the more your strength returns to you, and the more the submissiveness placed into you by her venom recedes. Still, you've tired yourself out too much to leave at the moment.");
			else outputText("The longer you sit there, the more you remember of yourself, and the more you forget the animal, the rut-beast that the spider-woman placed inside you.");
			outputText("  You shiver and wrap your arms around her, snuggling against her while a primal, buried part of your brain insists that she's YOURS.  You fall asleep like that, resting dreamlessly even through your mate's waking and departure.");
			hideUpDown();
			monster.lust = 98;
			monster.HP = 2;
...
//*Victory Rape: Ride Spider-Cawk in Vagoo (done)
		private function winVSDriderTakeItsCockInCunt():void
		{
			...
			outputText(" inside of you.");
			player.cuntChange(monster.cockArea(0), true, true, false);
			outputText("  You lean down, straddling her sides and reaching over to plant a kiss on one of the spider-woman's ");
...
//*Victory Rape: Ride Ovi Vaginal (done)
		private function victoryVSDriderRideOviVaginal():void
		{
			...
			if (monster.HP < 1) {
				...
				if (player.hasCock()) outputText(" and " + sMultiCockDesc() + " twitch");
			...
			if (player.hasCock()) {
				outputText("  Your own [cocks] retaliate");
				if (player.cockTotal() == 1) outputText("s");
				outputText(" automatically, dumping ");
				if (player.cumQ() < 500) outputText("scores of");
				else outputText("unending");
				outputText(" jizz-ropes on your groin and the base of the drider's slimy stalk.");
			}
			...
			outputText("You watch her for a time, until her orgasms finally die down.  By that point, you've recovered enough to get dressed.  You walk away a little bow-legged, with a new, heavy weight in your belly, just waiting to be born.");
			//GET KNOCKED UP
			//Pregnancy Goes Here
			player.knockUp(PregnancyStore.PREGNANCY_DRIDER_EGGS, PregnancyStore.INCUBATION_DRIDER, 151);
...
//*Victory Rape: Ride Ovi Anal sex-insenitive (done)
		private function victoryVSDriderRideOviAnal():void
		{
			...
			outputText("The spider's look of ");
			if (monster.HP < 1) {
				outputText("hate fades slightly when she sees you eyeing her ovipositor, and with a sigh of annoyance, allows the segmented organ to slowly slide out from her body.  It grows to just over two feet in length, swaying sinuously in the air, though you can tell it isn't completely engorged yet.  Still, your " + assholeDescript());
				if (player.hasCock()) outputText(" and " + sMultiCockDesc() + " twitch");
				else outputText(" twitches");
				outputText(" at the thought of trying to pack all of that meaty, alien pole inside you.\n\n");
			}
			...
			outputText("  Each segment somehow feels larger than the one preceding it, though it seems impossible given how widely stretched you feel by the time the fourth is inside you.");
			if (player.hasCock()) {
				outputText("  The warm organ brushes past your prostate, the slippery caress making " + sMultiCockDesc() + " dribble a ");
				if (player.cumQ() < 500) outputText("drop");
				else outputText("stream");
				outputText(" of pre-cum down its length.");
			}
			if (player.hasVagina()) {
				outputText("  Your " + vaginaDescript(0) + " quivers");
			...
			else outputText("\"<i>Just because I have an exoskeleton doesn't mean I can't feel you dripping on me.  I thought I was the corrupt bitch here, but you... you take the cake.</i>\"");
			outputText("\n\n");

			outputText("With a final hard shove, the last few segments pop inside you");
			if (player.hasCock()) outputText(", nearly crushing your prostate from the tight squeeze");
			outputText(".  ");
			...
			if (player.hasCock()) outputText("while your [cocks] dribbles white, wasted goo onto the black and " + monster.hairColor + " striped carapace below");
			else if (player.hasVagina()) outputText("while your " + vaginaDescript(0) + " dribbles fem-slime onto the black and " + monster.hairColor + " striped carapace below");
			else outputText("while your fingers tweak your " + nippleDescript(0) + "s and your starved sphincter sucks at its slimy, squirming stuffing");
			...
			outputText("The large, round egg pushes its way up your butt, squeezes past your ring");
			if (player.hasCock()) outputText(" and your prostate");
			outputText(", and travels deep inside you, bulging your belly the slightest bit once it's deposited.  With a groan of deep satisfaction, you start humping the fluid-slicked chitin beneath you, barely registering the long, seemingly unending series of eggs that infiltrate your ass");
			if (player.hasCock()) outputText(" and milk your prostate as they push past");
			outputText(".\n\n");

			outputText("The stream of slime only spreads as the stimulation of the sliding spheres slewing into your sphincter sets off");
			if (player.gender > 0) {
				outputText(", simultaneously, ");
				if (player.hasCock()) {
					outputText("a storm of semen-squirting spurts from your shaft");
					if (player.cockTotal() > 1) outputText("s");
				}
				else if (player.hasVagina()) outputText("a siege of spray-scattering spasms from your snatch");
				outputText(" and");
			}
...
//(please view the spinneret as a spider-horsecock because for all intents and purposes that is really what it is) - shambadibs (1.5-inch width max because i'm racist against buur-bous american penis) (also probably a corruption minimum this is a pretty dick move) (I GET IT)
		private function victoryVSDriderStickDickInSpinneret():void
		{
			...
			outputText("Annoyed by her rebelliousness, you reach back, ");
			if (monster.nipplesPierced > 0) outputText("grasp the intersection of her nipple-chains, ");
			else outputText("grasp her nipples, ");
			...
			if (player.biggestTitSize() >= 3) outputText("  The semi-sticky stuff soaks into the cleavage of your [allbreasts], the feeling surprisingly enjoyable.  You even knead the web in a bit, biting your bottom lip from the sensation.");
			...
			if (player.cumQ() < 50) outputText("  She easily keeps in rhythm with your spurts.");
			else if (player.cumQ() < 350) outputText("  She sputters a bit from the amount of semen, but she catches up well enough.");
			else if (player.cumQ() < 1000) outputText("  She's hard-pressed to swallow enough of your seed before another waves washes in.");
			else outputText("  Her throat bulges from the sheer amount of cum you deposit into her mouth, and she nearly chokes as she attempts to swallow all of it.");
			if (player.cockTotal() > 1) {
				outputText("  Your other ");
				if (player.cockTotal() > 2) outputText("cocks give ");
				else outputText("cock gives ");
				outputText("you a picture-in-picture view of what's happening inside, launching identical strokes all over her face.  She's in for a stinging surprise if she opens her eyes quickly when she wakes.");
			}
			outputText("\n\n");
...
//*Loss Ride Ovi Anal unsexed - gats (done)
		private function loseVSDriderGetAnalledOvi():void
		{
			...
			else outputText("chest, trickling between your " + nippleDescript(0) + "s");
...
//*Loss Vaginal Impregnation and forced BJ - Symphonie (done)
		private function loseVSDriderAndVaginallyImpreggleOvi():void
		{
			...
			outputText(".  It seems that, regardless of their slenderness, she can still put an impressive amount of weight behind her legs.  They ");
			if (!player.isNaga()) outputText("pull your own [feet] apart");
			else outputText("scrape inquisitively at your scales");
			outputText(", her body climbing atop you.  Before you can realize her plans her spinneret is at work, ");
			if (!player.isNaga()) {
				outputText("coating your ");
				if (player.isTaur()) outputText("rear ");
				outputText(player.legs() + " and pinning you in that prone, separated position");
			}
			else outputText("spraying webbing across your tail and weighing it down to pin it against the earth");
			outputText(".  She turns with the eerie speed of the arachnids of Mareth, quickly performing the same acts to your ");
			if (!player.isTaur()) outputText("wrists");
			else outputText("forelegs");
			outputText(" and leaving you pinned and spread-eagled on the ground.  \"<i>That should stem that.</i>\"\n\n");

			outputText("Turning, she makes as though to bite you, before pausing.  A thought runs through her head, and if the wicked curve of her dark lips are any indication, she likes it.  She gathers webbing on her fingers and bends down, inspecting your features carefully.  Placing said fingers on your chin she forces it open, quickly and expertly weaving sticky strands to connect your jaw and your chest.  The strands are just short enough to keep your chin held down, while strong enough that you can't seem to pull at them.  ");
			if (!player.isTaur()) outputText("Her fingers then move to the sides of your head, lacing webbing through your hair and sticking it firmly to the ground. More vulnerable than ever, you can't lift your head without ripping your hair from your scalp in chunks.");
			else {
				outputText("Her fingers then reach behind your head, pulling your upper body off of the damp dirt and stretching sticky strands between your skull and your immobile forelegs.  ");
				if (player.biggestTitSize() > 4) outputText("With pain and then a pop, your [allbreasts] squash out to the sides as she presses your torso into your knees.  ");
				outputText("Reaching back for more webbing, she ties your arms behind your back in a bundle of white thread.  Now pretzeled into an uncomfortable curled shape, you can't pull your torso away from your forelegs without wrenching your head off of your neck.  You feel completely helpless.");
			}
			outputText("  \"<i>Now, isn't that just precious,</i>\" the drider chuckles again.  She leans in low, her fingers trickling over your thighs like the steps of... well, of small spiders.  You gurgle in response, mouth held open uncomfortably.  She roams slowly up your body.");
			if (player.hasCock() && monster.nipplesPierced > 0) outputText("  Chains brush over " + sMultiCockDesc() + " as her chest passes your crotch.");
			outputText("  Her fangs trail teasingly along your belly, her fingers crawling up your sides.");
			if (player.biggestTitSize() > 3) outputText("  Venom dribbles down the cleavage of your " + biggestBreastSizeDescript() + " as she approaches your vulnerable face.  Her own quartet of matronly tits comes to rest on them as her torso appears over their horizon.");
			outputText("\n\n");

			outputText("Drawing ever closer, at last her eyes blink slowly in front of yours.  Her lips seem to be pouting, then smirking, then - oh.  She's swirling something around in her mouth.  Finally it opens up above yours, her fangs extending.  Though you flinch in preparation for a bite, none comes - instead venom drips off her teeth and lips, every drop felt as it lands on your tongue.  You gurgle once more in protestation at this torture, but her hand pets the skin of your forehead maternally to remind you of your powerlessness.  Sadistic delight plays across her eyes as she watches your restrained struggles.  As the venom seeps, a heat grows.  Though subtle at first, you soon recognize it: lust.  Raw, burning need.  The poison coursing through you makes little effect on your heart, but a world of difference to your crotch.");
			if (player.hasCock()) outputText("  Worse still, its blood-swollen protuberance is met with no reaction from the drider.");
			...
			outputText("The drider presses you to her crotch");
			if (!player.isTaur()) outputText(", cutting your head loose from the imposed bondage with sharp, hard fingertips");
			...
			if (!player.isTaur()) outputText("land against the ground");
			else outputText("strain your neck suddenly with the dead weight of it");
			...
			outputText("You can tell that the bindings at your limbs are already slackening, but you're so weak, so exhausted - simply being able to breathe seems like a delicacy that had not been afforded you until just this moment.  You can't leave, just yet.  You just need... to rest... a little...");
...
//*Loss Ride & Impregnate Butt (done)
		private function loseVSDriderAndGetFuckedANDAnalImpreggegity():void
		{
			...
			if (player.lust >= player.maxOverLust()) outputText("Unable to resist the desire coursing through your body, you strip off your [armor] and start stroking " + sMultiCockDesc() + ", displaying it in the hope that it will draw the drider's attention, while at the same time just trying to get yourself off and attain release.");
				...
				outputText("member");
				if (player.cockTotal() > 1) outputText("s");
				outputText(" to the air.  She squeezes " + oMultiCockDesc() + " in her hand and pumps it, smiling while you stiffen and twitch precipitously in her grasp.");
			}
			...
			if (monster.nipplesPierced > 0) outputText(", the crossed chains digging into your " + chestDesc());
			...
			outputText("The strange organ keeps forcing its way deeper, slipping the steadily-widening, rib-like segments of its bulk into your " + assholeDescript() + ".  ");
...
//Summary: what goes up must come down
		public function birthSpiderEggsFromAnusITSBLEEDINGYAYYYYY():void
		{
			...
			outputText("Clenching and relaxing, your slime-dripping asshole slowly narrows, gaping less visibly while you shudder and sigh, so happy and blissful that you don't even care about the crackling egg-shells hatching beneath you.  ");
			if (player.hasCock()) outputText(SMultiCockDesc() + " stays limp, but drools a heavy flow of semen, puddling up a nice thick batch of your pleasure alongside your goo-pile.  ");
			if (player.hasVagina()) {
				outputText("Your pussy clenches and ");
				if (player.wetness() < 3) outputText("drips");
				else if (player.wetness() < 4) outputText("dribbles");
				else if (player.wetness() < 5) outputText("gives a tiny squirt");
		...
		public function driderPregVagBirth():void
		{
			...
			outputText("After what seems like hours, you have lost count of the eggs pushed out.  ");
			if (player.isNaga()) outputText("At last, when no more eggs seem to be coming, you flop over onto your back, careful not to crush any of your dormant offspring.");
			else outputText("Looking between your legs you can see another slick, green egg slide from your body to join the sizable pile behind you.  At last, when no more seem to be following it, you flop over onto your back, careful not to crush any of your dormant offspring.");
			...
			if (player.biggestTitSize() >= 2) {
				outputText("One drider girl cautiously makes her way over to you.  Her legs tickle your stomach as she climbs up to one of your " + breastDescript(0) + ".  Grasping the flesh tightly in her tiny hands she begins to suckle at your " + nippleDescript(0) + ".");
				if (player.bRows() > 1) outputText("  Following their sister's lead, more little driders take their place at each of your nipples and begin to drink.");
				else outputText("  Following her sister's lead, another little drider takes her place at your other nipple and begins to drink.");
			...
			outputText("Around you, your many drider children fervently explore their surroundings.  One climbs up your back and perches atop your head.  She raises one hand to her forehead to block the sun's glare and stretches all six of her jewel-like eyes wide, delighting in the wondrous world she surveys.");
			if (player.eyes.type != Eyes.SPIDER) outputText("  Suddenly she flops forward, leaning against your eyebrows and causing you to cross your eyes in an attempt to focus on her.  She seems confused by the way your eyes are different to hers.");
