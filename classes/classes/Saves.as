...
//      // Uncomment to restore deleted saves from backup
//		if (!test.data.exists && CoC.instance.debug) {
//			var test2:Object = SharedObject.getLocal(saveFileNames[i]+"_backup", "/");
//			if (test2.data.exists) {
//				Utils.extend(test.data, test2.data);
//			}
//		}
...
//		saveFile.data.dualWFXP = player.dualWFXP;
		...
		for (i = 0; i < Player.MAX_NUM_ITEM_SLOTS; i++) {
			saveFile.data["itemSlot"+(i+1)] = [];
			saveFile.data["itemSlot"+(i+1)].quantity = player.itemSlots[i].quantity;
			saveFile.data["itemSlot"+(i+1)].id = player.itemSlots[i].itype.id;
			saveFile.data["itemSlot"+(i+1)].unlocked = player.itemSlots[i].unlocked;
		}
...
//				trace("Creating perk : " + ptype);
		...
		for (i = 0; i < Player.MAX_NUM_ITEM_SLOTS; i++) {
			if (saveFile.data["itemSlot"+(i+1)] != undefined) {
				player.itemSlots[i].unlocked = saveFile.data["itemSlot"+(i+1)].unlocked;
				player.itemSlots[i].setItemAndQty(ItemType.lookupItem(
								saveFile.data["itemSlot"+(i+1)].id || saveFile.data["itemSlot"+(i+1)].shortName),
						saveFile.data["itemSlot"+(i+1)].quantity);
			}
		}
		// always unlocked
		player.itemSlots[0].unlocked = true;
		player.itemSlots[1].unlocked = true;
		player.itemSlots[2].unlocked = true;
		player.itemSlots[3].unlocked = true;
		player.itemSlots[4].unlocked = true;
		player.itemSlots[5].unlocked = true;
		for (var key:String in _saveableStates) {
			var ss:SaveableState = _saveableStates[key];
			try {
