...
public function timeChange():Boolean
{
    ...
    if (player.hasStatusEffect(StatusEffects.CampMarble)) {
        ...
        else if (!player.hasStatusEffect(StatusEffects.MarbleHasItem) && marbleScene.marbleAtCamp()) {
            // Off cooldown
            if (!player.hasStatusEffect(StatusEffects.MarbleItemCooldown)) {
                if (rand(10) == 0) {
                    marbleSprite();
    ...
    if (player.hasStatusEffect(StatusEffects.MarbleWithdrawl)) {
        ...
        else if (player.hasStatusEffect(StatusEffects.BottledMilk)) {
            outputText("\nYou no longer feel the symptoms of withdrawal.\n");
            needNext = true;
            player.removeStatusEffect(StatusEffects.MarbleWithdrawl);
            dynStats("tou", 5, "int", 5);
...
public function marbleSprite():void {
    if (flags[kFLAGS.MARBLE_BOVA_LEVEL] < 2)
        ...
        spriteSelect(SpriteDb.s_marble_cow);
    }
}

override public function marbleFollower():Boolean {
	return player.hasStatusEffect(StatusEffects.CampMarble);
}
...
private function forceFeedMarble():void {
	...
	if(player.hasPerk(PerkLib.Feeder)) outputText("You bet this cow-girl loves to get milked and nursed on just like you, but how often does she get to taste the creamy sweetness of milk?  Having her suck on your own leaky tits would be doing her a favor, right?  You approach the defeated cow-girl; her eyes are still locked onto you, wondering what you're going to do next.  Well, not that you can do much with this crowd watching you...\n\n");
...
public function postAddictionFarmExplorings():void {
	...
	if(player.hasPerk(PerkLib.MarbleResistant)) outputText("uncomfortably ");
...
public function standardSex():void {
	...
	if(player.gender == 0) {
		...
		//(player is not wearing fetish gear)
			...
			outputText("You smile in return and begin removing your [armor] from your body.  You start by slowly slipping off your top to reveal your [allbreasts], ");
...
public function marbleSex2Continued(genders:int):void {
	...
	//(player is male)
	if(genders == 1) {
		outputText("Marble leans back and invites you to come.  With a coy grin, you slowly crawl towards her, each movement bringing the two of you closer until you are on top of her body. She puts her hands around you and lies back completely.  You carefully line up your [cock] and thrust into her warm snatch.  ");
...
public function marbleMilkSex():void {
	...
	function dickF():void {
		...
		if(player.cockTotal() > 1) outputText("\"<i>So, which one is your favorite?</i>\"  Marble asks you while taking measure of your [cocks].  Before you have a chance to answer, Marble grabs a hold of your central [cock] and says \"<i>I'll bet it's this one.</i>\"  ");
		outputText("Without much hesitation, Marble lifts herself up, and impales her moist lips upon your [cock].  The two of you gasp as you sink into her interior.  You feel each inch slowly flow inside her until ");
		//[dick(0) is less than 8 inches long]
		if(player.cocks[0].cockLength < 8) outputText("every bit of your [cock] is deep inside.  ");
	...
	function vagF():void {
		...
		//[player has smaller then C breasts]
			...
			outputText("She teases your " + vaginaDescript(0) + " with her fingers for a moment before putting her hand on one of your " + biggestBreastSizeDescript() + ".  \"<i>I'm not sure what to do with teats this small, but I'll do my best. Though I feel kinda sorry for you, sweetie.</i>\"  You open your mouth to make an indignant response, but Marble puts a figure on your lips to shush you. She gives you an intense stare and tells you in no uncertain terms that since you enjoyed her breasts so much, it's only fair that she gets to experience your [allbreasts].  She then sticks out her tongue at you before giving your " + nippleDescript(0) + " a lick and rub down, which she extends to the ");
		...
		if(player.totalBreasts() > 2) outputText(" with each of your " + allBreastsDescript());
		else outputText(" with each of your " + breastDescript(0));
		...
		if(player.biggestLactation() <= 1 && !player.hasFuckableNipples()) outputText("Of course, since you aren't lactating, nothing but your " + nippleDescript(0) + " reaches her lips.  Marble doesn't seem to be put off by this and still gives you a thoroughly enjoyable experience.");
		outputText("\n\n");
		outputText("After a while, Marble pulls back from your " + nippleDescript(0) + " and tells you, \"<i>Sweetie, I know how annoying it is to only have one of your nipples serviced, so I will be sure to finish the job.</i>\"  She dives right into your ");
...
public function marbleStatusChange(affection:Number, addiction:Number, isAddicted:Number = -1):void {
	if(!player.hasStatusEffect(StatusEffects.Marble)) player.createStatusEffect(StatusEffects.Marble,0,0,0,40);
	//Values only change if not brought to conclusion
	if(!player.hasPerk(PerkLib.MarblesMilk) && !player.hasPerk(PerkLib.MarbleResistant)) {
...
private function talkWithMarbleAtCamp():void {
	...
	{
		outputText("Your story about what had happened to Marae seems to have shaken up Marble a little.  Though, you notice that she seems to be getting more and more aroused as you relate your story.  ");
	...
	else if(flags[kFLAGS.MET_MARAE] > 0 && flags[kFLAGS.FACTORY_FOUND] <= 0)
	{
		...
		if(player.level < 3) outputText("\"<i>I think we should help out Marae and shut down that factory she mentioned was in the mountains, but I don't think you're ready to go into the Mountains yet. They can be brutal - get a little more practice, and make sure you've got a good weapon.</i>\"");
	...
	//Player has found factory but not shut it down.
		...
		outputText("\"<i>You still haven't shut down the factory yet, have you?</i>\"  You shake your head.  \"<i>Well then go do it!</i>\"");
	//(player has completed the factory but has not returned to Marae)
		...
		outputText("\"<i>You haven't gone back to Marae yet have you?</i>\"  You shake your head.  \"<i>Well then go see her!  I'm sure she really wants to thank you.</i>\"");
...
private function marbleInfo():void {
	...
	outputText("Once camp improvements have been implemented, you will be able to get her to work on upgrading the camp instead of searching for supplies.");
	doNext(interactWithMarbleAtCamp);
}
...
public function giveMarbleLactaid():void {
	...
	clearOutput();
	outputText("You show Marble the bottle of Lactaid that you found and ask her if she could drink it.  She gives you a knowing smile. \"<i>So you want a special treat?  Ok sweetie, give it here.</i>\"  She takes the bottle from you and easily drinks the liquid.  Within moments, she gasps and ");
	...
	if(!player.isNaga()) {
		...
		function breastsF():void {
			...
			if(player.biggestTitSize() > 3) {
				outputText("pressing them against your own " + biggestBreastSizeDescript() + ".  She giggles slightly as she puts one of them between her mounds and rubs them up and down around it, teasing your " + nippleDescript(0) + " with her own " + marbleNips() + ", a trickle of milk leaking out onto your locked chests.");
			} else {
				outputText("pressing them against your chest.  She rubs them up and down against you as she moves around, making sure she has rubbed each part of your upper body at least three times over.  She is especially happy with moving your head around between her breasts.");
			}
			...
			if(player.cocks.length > 0) {
				...
				if (player.cocks.length > 1) {
					outputText("  Yet still unsatisfied, Marble engulfs and rub the rest of your [cocks] within her ample mounds.  ");
				}
			}
			//If Player has a vagina, Marble titfucks that
			if(player.vaginas.length > 0) {
				outputText("\n\nShe slides down your body to rub and caress your " + vaginaDescript(0) + " with her breasts, sometimes pushing one of her " + marbleNips() + " inside your " + vaginaDescript(0) + " and pulling a small gasp from you.  ");
			...
			if(player.cocks.length > 0) {
				...
				if(player.cumQ() < 25) outputText("  In moments, a small amount of your cum is covering Marble's breasts.");
				else if(player.cumQ() < 250) outputText("  After a few moments, a fair amount of your cum is covering Marble's breasts and face.");
				else if(player.cumQ() < 500) outputText("  You keep cumming and cumming, until eventually Marble is completely drenched in your cum.");
				else outputText("  Your seemingly endless orgasm eventually results in the two of you becoming completely drenched in your cum, as well as a large part of the ground around you.");
			}
...
public function marblePoopsBaybees():void {
	...
	if (pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
		...
		else
		{
			...
			if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) outputText(", but you notice that she does not have a cock of any kind. It seems that trait isn't passed on");
...
public function marbleNightSleepFlavor():Boolean {
	...
	switch (pregnancy.eventTriggered()) {
		...
		case 3: outputText("<b>Marble's belly has gotten a fair bit bigger; she may be pregnant.</b>\n\n"); //8 days in
				break;
		case 4: outputText("<b>Marble's belly has gotten bigger; she is obviously pregnant."); //12 days in
				...
				if (flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] == 0 && pregnancy.type != PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
					flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION]++;
					outputText("  She lets you know that she'll be busy working on a nursery for her child for a while.");
				}
				outputText("</b>\n\n");
				break;
...
private function marbleCuddlin():void {
	outputText("  Before too long, Marble reaches over and ");
	...
	if(!pcPregWithMarblesKids() && !marblePregWithPCKids()) {
		...
		if(player.tallness <= 78) outputText("pulls your head on top of her chest.  She idly strokes your " + hairDescript() + " and says, \"<i>Sweetie, how about I be your pillow for the night?</i>\"  You happily agree, and soon you drift off to the sound of her steady breathing while you lay between her sizable breasts.");
		else outputText("wraps her arms around you, gently pulling you against her.  \"<i>Sweetie, do you mind if we stay like this for a while?</i>\"  There is no way you could refuse, and before long, the two of you drift off to sleep in each other's arms.");
	}
	//Preg-cuddles
	else {
...
private function marbleNightSexIntro(clear:Boolean = true):void {
	...
	else
	{
		...
		//If (Marble is a herm) {
		if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) {
			outputText("gently rub her " + marbleCock() + ".  After a moment, you move your hand down lower and ");
		}
	...
	if(player.gender == 3) {
		if(player.cockArea(x) <= marbleCuntCapacity()) {
			outputText("\"<i>So sweetie, what part will you be using today?</i>\" she asks you.");
...
private function marbleNightSexDudes():void {
	...
	//ORGASMO
	outputText("You gasp as you feel your insides churning and the walls of Marble's sex collapsing tightly down on you.  Once again, you've brought each other to the point of blissful orgasm around the same time.  ");
	...
	if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0) {
		outputText("Trapped between your bodies, Marble's " + marbleCock() + " spasms and sprays the two of you.");
	}
...
private function marbleNightSexChicks():void {
	...
	clearOutput();
	if(flags[kFLAGS.MARBLE_DICK_TYPE] == 0) {
		outputText("You gently lift yourself up as Marble produces her double dildo and slips it inside her womanhood.  Smiling at her, you slowly lower your body and impale your " + vaginaDescript(0) + " onto the artificial rod.  ");
		...
		if(player.cor + player.lib < player.statusEffectv4(StatusEffects.Marble) + 10 || rand(3) == 0) {
			...
			outputText("\"<i>Sweetie!</i>\" Marble gasps as you start to play with her breasts.  ");
			...
			outputText("Within moments, your rapid pistoning on the dildo is pushing the two of you closer and closer to the edge.  ");
		}
	...
	//PC is female, Marble is a herm
		...
		player.slimeFeed();
		outputText("You gently lift yourself up into the air and guide your " + vaginaDescript(0) + " to the tip of her waiting " + marbleCock() + ".  You slowly lower your " + vaginaDescript(0) + " onto her quivering " + marbleCock() + ", gaining inch by inch ");
		//is she satisfied?
		...
		//Marble seems to think you can take more than maybe you should be able to take
			...
			outputText("until you bottom out and can take no more of her.  \"<i>No more?</i>\" Marble asks you a little sadly, \"<i>You sure you can't take a little more?</i>\" she adds as she starts pushing you down onto her " + marbleCock() + ". You are forced to stretch to accommodate more of her and you shudder slightly from the strain.  ");
			//(Message about vagina becoming looser)
			outputText("Marble grins at you and says, \"<i>See?  I knew you that you could fit a little more.</i>\"\n\n");
		}
		outputText("After adjusting to the feeling of her inside you, you can't deny what a wonderful feeling it is to be connected like this.  ");
		//Does Marble take the initiative?
		...
		//no, no, the PC is much hornier than she is
			...
			outputText("Within moments, your rapid pistoning on her " + marbleCock() + " is pushing the two of you closer and closer to the edge.  ");
		}
...
private function marblePreggoChance(preggerMult:Number):void {
	...
	if(player.hasPerk(PerkLib.FertilityPlus)) preggerOdds += 5;
	if(player.hasPerk(PerkLib.FertilityMinus) && player.lib < 25) preggerOdds -= 5;
	//If has 'stud perk' almost always get her pregnant
	if(player.hasPerk(PerkLib.MaraesGiftStud)) preggerOdds += 25;
...
private function marbleNomNoms():void {
	...
	function dickF():void {
		...
		outputText("Marble lifts up her skirt and gives you a good look at her very erect " + marbleCock() + ".  You position yourself overtop of her and take a deep breath, taking in her smell.  ");
		//is the PC overwhelmed by her animalistic smell?
		if(player.hasStatusEffect(StatusEffects.SlimeCraving) || player.lib + player.cor - player.inte > 35) {
...
public function MarbleDigsDraftsYo():void {
	...
	outputText("Marble drops the potion and grabs at her crotch.  Turning away from you, she gives an excited cry that is a mix between a moo and a moan as her body convulses slightly.  After a moment, she turns back to you and shows her new 7\" long and 2\" wide human cock.  She gives you a ");
	if(player.statusEffectv4(StatusEffects.Marble) > 50) outputText("big ");
	else outputText("nervous ");
	outputText("smile.  <b>Marble is now a hermaphrodite.</b>  ");
	doNext(camp.returnToCampUseOneHour);
...
//removes her dick, no other effect.  Can only be given if Marble has a dick.
	...
	outputText("She lifts up her skirt and the two of you watch as her " + marbleCock() + " is absorbed back into her body.  <b>Marble is now purely female.</b>  ");
...
public function marbleOvulatesLikeMadDawg():void {
	...
	if (flags[kFLAGS.MARBLE_NURSERY_CONSTRUCTION] < 100 && pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
		outputText("Marble refuses to take the elixir to speed up her pregnancy until after she has finished the nursery. ");
		doNext(camp.returnToCampUseOneHour);
		return;
	}
	else if (pregnancy.isPregnant) {
		outputText("After taking the elixir, Marble's belly rumbles slightly.  You can both tell that her pregnancy has advanced.  ");
		...
		outputText("After taking the elixir, Marble tells you that her belly feels full.  It sounds like she'll be laying eggs soon.  ");
...
private function marbleAppearance():void {
	...
	if(player.statusEffectv1(StatusEffects.MarbleSpecials) == 0) outputText("She has one virgin bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
	else outputText("She has one bum-hole, placed between her squeezable butt-cheeks where, you presume, it belongs.");
	doNext(interactWithMarbleAtCamp);
}
