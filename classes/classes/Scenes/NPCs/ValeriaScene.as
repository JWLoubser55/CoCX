...
public class ValeriaScene extends NPCAwareContent implements TimeAwareInterface {

		public function ValeriaScene()
		{
...
private function followersValeriaSex(display:Boolean = true):void {
	...
	menu();
	addButton(0, "PenetrateHer", penetrateValeria).hint("Fuck the goo girl with your penis!")
...
public function valeriaGetFucked():void {
	...
	//[if PC has cock: \"<i>
	if(player.hasCock()) outputText("her gooey ass devouring your [cock] as she slides down your belly");
	...
	outputText(".");
	if (player.isGenderless()) gooFlation(false);
	...
	function dickF():void {
		...
		outputText(" and fucking your [cock] with one fluid, passionate motion.  You're both moaning openly now, nearly overwhelmed by a multitude of pleasures.  You can feel your impending orgasm rising, and by the heightening pitch of Valeria's moans, she seems to be just as close to the edge.");
	...
	function sharedEnd():void {
		...
		dynStats("sen", -1);
		pc.HPChange(25 + (player.newGamePlusMod() * 15), false, false);
...
public function valeriaSexDominated(offCamp:Boolean = false):void {
	...
	function dickF():void {
		...
		outputText("\n\nYou gasp with pleasure as she starts to stroke [eachCock], jerking you off as she looms over you, grinning wickedly.  \"<i>Oh, you like that do you?</i>\" the armor-goo asks.  \"<i>Well then, I might just let you get off, too... If you're a good " + player.mf("boy","girl") + "</i>\"  She starts to increase her tempo, making you squirm and writhe as she wanks your [cocks], licking her lips as little bubbles of pre start to form.");
...
public function valeriaAndGooThreeStuff():void {
	...
	outputText("\n\nThe bluish");
	if (monster.bodyColor != "blue") outputText(" " + (monster as GooGirl).gooColor6());
