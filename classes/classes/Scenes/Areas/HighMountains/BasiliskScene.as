...
public class BasiliskScene extends BaseContent
	{
		...
		public function defeatBasilisk():void {
			...
			menu();
			addButtonIfTrue(0, "Tongue", tongueBasiliskSmex, "Not genderless!", player.gender > 0);
			addButtonIfTrue(1, "Ass", defeatBasiliskAndAnal,
				"Req. to have a cock fitting " + monster.analCapacity() + " area and be highly corrupted.",
				player.cockThatFits(monster.analCapacity()) >= 0 && player.cor >= 66 - player.corruptionTolerance);
			addButtonIfTrue(2, "Lay Eggs", driderPCEggLaysBasilisk, "Req. a drider/mantis ovipositor.", player.canOvipositSpider() || player.canOvipositMantis());
			addButtonIfTrue(2, "Lay Eggs", layBeeEggsInABasilisk, "Req. a bee ovipositor.", player.canOvipositBee());
		...
		public function loseToBasilisk():void {
			...
			if(player.hasVagina() && player.pregnancyType != PregnancyStore.PREGNANCY_BASILISK && (player.inHeat || player.hasPerk(PerkLib.Oviposition) || player.hasPerk(PerkLib.BasiliskWomb) || player.pregnancyType == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS))
		...
		private function basiliskHasVagEggStuff():void {
			...
			if (player.pregnancy2Type == PregnancyStore.PREGNANCY_OVIELIXIR_EGGS) {
				outputText("\n\nYour womb gurgles, and you instinctively put a hand on your belly. It seems larger than it usually is, and you feel oddly more tender and motherly than normal.  You shake your head at the thought.  Damn hormones.");
				player.knockUpForce(PregnancyStore.PREGNANCY_BASILISK, PregnancyStore.INCUBATION_BASILISK - 150, 1); //Convert Ovi Elixir eggs to Basilisk eggs
			}
		...
		public function basiliskBirth(womb:int = 0):void {
			spriteSelect(SpriteDb.s_basilisk);
			outputText("\n");
			if(player.hasPerk(PerkLib.BasiliskWomb)) {
				...
				player.cuntChange(20,true,true,false, womb);
				...
				outputText("They seem to quickly adapt to where they have found themselves, running around each other with increasing confidence, and you can see even in the short time you have been watching they have grown, their tender scales hardening as the sun and air beats down on them.  One of them suddenly scuttles like the lizard it is for cover, and you lose it from view underneath a rock.  They are quickly all at it, one after the other dashing and slipping from view.  The last to go is the largest, the first to hatch: it fixes you with its stare before slowly turning and following suit.  You could swear it gives you the smallest of smiles, a child's eager grin, before it goes.  The only evidence you have left of what just happened is a slimy pile of discarded eggshells.\n\n");
		...
		private function basiliskAdvantageHarpy():void {
			...
			//Male/Herm:
			if(player.hasCock() && (!sceneHunter.uniHerms || rand(3) > 0)) {
			...
			//Genderless:
				...
				outputText("The harpy comes to a halt behind you and begins to eagerly run her cold but soft hands over your bottom half, stroking your thighs and squeezing your [butt], as if appraising a piece of meat.  Whilst caressing your neck she runs her hands between your legs and grabs around your groin, searching for something that isn't there.  She lets loose a squawk of pure frustration and wheels around you to glare in your eyes angrily.  The sex-crazed harridan is clearly deeply irritated about finding such a helpless victim only to further discover they lack a cock, which she can abuse.  She shifts her eyes to your mouth, but quickly arrives at the same conclusion you've already come to: unable to move your mouth, you can't even be forced to give oral pleasure.  You feel a bizarre sense of triumph over the creature; you stare into space smugly as the harpy paces in front of you, glaring, thwarted but unwilling to give up her prize.  Perhaps eventually she will leave you alone...?\n\n");
		...
		private function basiliskAdvantageGoblin():void {
			...
			//Unsexed:
				...
				basiliskAdvantageMinotaur_part2();
			}
			//Female:
			else {
				outputText("At the corner of your vision, you see a small, familiar green shape hover into view.  The goblin is so busy sorting through her inventory of drugs that you actually manage to see her before she sees you.  When she does lift her head up and notices the petrified, naked individual in front of her, she is so surprised she drops her satchel.\n\n");
		...
		//used in goblin scene too!
		private function basiliskAdvantageMinotaur_part2():void {
			...
			outputText("Eventually the minotaur's balls swell against your [butt] and with a long, satisfied moo, he reaches his peak, holding you down, so his cum jets forth deep inside you.  ");
