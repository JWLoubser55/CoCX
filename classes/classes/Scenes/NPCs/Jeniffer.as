...
package classes.Scenes.NPCs
{
	...
	public class Jeniffer extends Monster
	{
		public function Specials2():void {
			outputText("The "+short+"'s visage twists into a grimace of irritation, and it swings its hand at you in a vicious backhand.");
			var damage:Number = Math.round(((str + weaponAttack) * 6) - rand(player.tou+player.armorDef));
			//Dodge
		...
		}//or make it some range weapon attack???
		
		public function Specials1():void {
			//Determine if dodged!
			if (player.getEvasionRoll()) {
				outputText(short+"tries to clinch you, but you use your speed to keep just out of reach.\n");
				return;
			}
			var damage:Number = Math.round((str + weaponAttack) - rand(player.tou+player.armorDef));
			if (damage < 0) damage = 0;
			outputText(short+" ducks and jinks, working to close quarters, and clinches you. Unable to get your weapon into play, you can only ");
			...
			else outputText("laugh as her spikes scape uselessly at your armor-clad back");
		...
		public function Jeniffer()
		{
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] < 1) {
				...
				this.tallness = 72;
				initStrTouSpeInte(100, 125, 75, 120);
				initWisLibSensCor(120, 90, 50, 0);
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] == 1) {
				...
				this.tallness = 144;
				initStrTouSpeInte(120, 150, 90, 140);
				initWisLibSensCor(140, 120, 70, 0);
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] == 2) {
				...
				this.long = "You're currently fighting a golemancer controlling personally created golem called J2a. It's thirteen feet tall with general shape of a female but lacking any fine details. It stone body is covered with soulmetal plates in many placed and it looks like using lance to stab enemies.  Only goblin's head isn't protected showing her green skin, pointed ears, and purple hair.";
				this.tallness = 156;
				initStrTouSpeInte(150, 190, 110, 160);
				initWisLibSensCor(160, 150, 90, 0);
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] == 3) {
				...
				this.tallness = 168;
				initStrTouSpeInte(180, 230, 130, 180);
				initWisLibSensCor(180, 180, 110, 0);
				this.weaponAttack = 72;
				this.armorDef = 150;
				this.armorMDef = 150;
				this.bonusHP = 600;//4x armorDef value
				this.bonusLust = 323;
				this.level = 33;
			}
			if (flags[kFLAGS.JENIFFER_LVL_UP] >= 1 && flags[kFLAGS.JENIFFER_LVL_UP] < 4) {
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] >= 4 && flags[kFLAGS.JENIFFER_LVL_UP] < 7) {//lub później niż 7 upgrade
				...
				this.weaponName = "lance";//mecha drill-lance / mecha lance-drill
				this.weaponVerb = "piercing stab";
			...
			if (flags[kFLAGS.JENIFFER_LVL_UP] >= 1) this.createPerk(PerkLib.TankI, 0, 0, 0, 0);
			if (flags[kFLAGS.JENIFFER_LVL_UP] >= 2) this.createPerk(PerkLib.GoliathI, 0, 0, 0, 0);
			if (flags[kFLAGS.JENIFFER_LVL_UP] >= 3) this.createPerk(PerkLib.CheetahI, 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 4) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 5) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 6) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 7) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 8) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 9) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 10) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 11) this.createPerk(PerkLib., 0, 0, 0, 0);
			//if (flags[kFLAGS.JENIFFER_LVL_UP] >= 12) this.createPerk(PerkLib., 0, 0, 0, 0);
			checkMonster();
		}
	}
}
