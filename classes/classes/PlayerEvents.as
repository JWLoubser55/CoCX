...
public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		private function hourlyTimersProc():Boolean {
			...
			needNext = needNext || player.gainPerk(PerkLib.ImprovedVenomGland, flags[kFLAGS.VENOM_TIMES_USED] >= 50, "You feel wonderfully healthy. After using your venom so many times your body finally got acclimated to the presence of your venom gland allowing for increased capacity and production.");
			needNext = needNext || player.gainPerk(PerkLib.ImprovedVenomGlandEx, flags[kFLAGS.VENOM_TIMES_USED] >= 125, "You feel wonderfully healthy. After using your venom so many times your venom gland development reached it next stage. Allowing for increased capacity, production and lowering usage of venom.");
			needNext = needNext || player.gainPerk(PerkLib.ImprovedVenomGlandSu, flags[kFLAGS.VENOM_TIMES_USED] >= 375, "You feel wonderfully healthy. After using your venom so many times your venom gland started to produce more potent venom. Allowing for increased capacity, production and increased effects of venom.");

			needNext = needNext || player.gainPerk(PerkLib.GeneticMemory, flags[kFLAGS.TIMES_TRANSFORMED] >= 25, "Your body behave weirdly as if all the transformation, which you have undergone started to make it unsure about what it truly is. Sometimes you even try to move limbs that are no longer there. Suddenly you realise that no matter how many time you change your body remembers it. Your body developed genetic memory!");
			needNext = needNext || player.gainPerk(PerkLib.TransformationResistance, flags[kFLAGS.TIMES_TRANSFORMED] >= 100, "You feel a strange tingling sensation. It seems as if you've finally adapted to the transformative properties of the food in Mareth and your body has finally built up enough resistance! You suspect that you can still transform but at somewhat diminished rate.");
			needNext = needNext || player.gainPerk(PerkLib.TransformationAcclimation, flags[kFLAGS.TIMES_TRANSFORMED] >= 200, "Due to you're continued consumption of transformative items, you have discovered a way to draw out more of the transformative properties of the food in Mareth.");
		...
		private function hourlyCheckRacialPerks():Boolean {
			...
			needNext = needNext || player.gainOrLosePerk(PerkLib.HydraRegeneration, player.lowerBody == LowerBody.HYDRA, "", "You accidentally cut yourself but to your stupor the wound does not close as fast as it should. Guess you are no longer a hydra enough to benefit from superior regeneration.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.HydraAcidBreath, player.lowerBody == LowerBody.HYDRA, "", "As your lead hydra head vanishes so do your ability to belch acid.");
			//Troll Regeneration
			needNext = needNext || player.gainOrLosePerk(PerkLib.TrollRegeneration, player.isAnyRaceCached(Races.TROLL, Races.GLACIAL_TROLL), "", "You accidentally cut yourself but to your stupor the wound does not close as fast as it should. I appears your lack of troll blood is no longer enough to benefit from superior regeneration.");
			...
			//Blood Affinity
			needNext = needNext || player.gainOrLosePerk(PerkLib.BloodAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.WERESPIDER, Races.DRACULA), "Your ability to hone the power of the red, flowing essence heightens. Hemophilia is a delightful chance for you to revel in the hemorrhage of freshly spilt blood.", "The sight of blood no longer fuels you with the same rousing sensations you once had. Any affinity you carried for the crimson fluid is now lost.");
			//Dark Affinity
			needNext = needNext || player.gainOrLosePerk(PerkLib.DarknessAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.APOPHIS, Races.ANUBIS, Races.DRACULA, Races.LICH) || player.hasPerk(PerkLib.Nightshade), "You are at home while shrouded in darkness, seeing clearly within the shadows as if it were broad daylight.", "The inky black darkness grows hostile toward you once more as your affinity for it is lost.");
			//Compelling Aria
			needNext = needNext || player.gainOrLosePerk(PerkLib.HarpySong, player.isAnyRaceCached(Races.SIREN, Races.HARPY, Races.PHOENIX, Races.THUNDERBIRD) || player.hasMutation(IMutationsLib.HarpyHollowBonesIM), "Your voice sound like magically entrancing music to your ears now, it would seem you have gained the infamous magically compelling voices common to harpies.", "Your voice no longer carries the magical power it used to and thus you are no longer able to use your compelling aria.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.MelkieSong, player.tongue.type == Tongue.MELKIE || player.hasMutation(IMutationsLib.MelkieLungIM), "Your words are notes, your sentence a melody. Your voice is like music to your ears and you realize it is because your body became closer to that of a Melkie adapting even your tongue and voice. Well, you could always go sit on a rock and sing in case some sailor came by.", "Your voice no longer carries the magical power it used to and thus you are no longer able to use your compelling aria.")

			//Alraune perks
			needNext = needNext || player.gainOrLosePerk(PerkLib.AlrauneNectar, player.isAlraune(), "Your flower now produces a delicious nectar the scent of which is sure to draw in partners.", "As you no longer have a pitcher to produce nectar your scent has became more like that of an ordinary human.", function():void {
						player.vaginaType(VaginaClass.ALRAUNE);
					}, function():void {
						player.vaginaType(VaginaClass.HUMAN);
					});
			needNext = needNext || player.gainOrLosePerk(PerkLib.PlantKnowledge, player.isAlraune(), "It would seem you acquired additional skills in herbalism thanks to being a plant yourself.", "No longer a plant you lost some of your innate floral knowledge.");
			//Kamaitachi perks
			needNext = needNext || player.gainOrLosePerk(PerkLib.CursedWound, player.arms.type == Arms.KAMAITACHI, "Your kamaitachi blades shines with a sharp, deadly glow. Those will clearly leaves deadly wounds.", "Lacking a pair of kamaitachi scythe you can no longer inflict your trademark cursed kamaitachi wounds.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.NaturalHerbalism, player.isRaceCached(Races.KAMAITACHI), "Great knowledges flows through your mind as you become more Kamaitachi like. It dawns on you that you have acquired a natural affinity for medicine and herbalism, something your species is famous for, heck you can identify every single plant near your camp by name and species now.", "No longer a Kamaitachi, you seem to have lost your knack for herbs and medicines.");
			//Aquatic Affinity
			needNext = needNext || player.gainOrLosePerk(PerkLib.AquaticAffinity, (InCollection(player.lowerBody,LowerBody.ORCA,LowerBody.SEA_DRAGON) && InCollection(player.arms.type,Arms.ORCA,Arms.SEA_DRAGON) && player.tailType == Tail.ORCA && InCollection(player.ears.type,Ears.ORCA,Ears.ORCA)) || player.isScylla() || player.isKraken() || InCollection(player.tailType,Tail.ARIGEAN_GREEN,Tail.ARIGEAN_RED,Tail.ARIGEAN_YELLOW,Tail.ARIGEAN_PRINCESS),
			...
			//Salamander bullshit
			needNext = needNext || player.gainOrLosePerk(PerkLib.LustingWarrior, player.isRaceCached(Races.SALAMANDER,2) && player.hasStatusEffect(StatusEffects.Overheat),
			...
			//}
			needNext = needNext || player.gainOrLosePerk(PerkLib.BigHandAndFeet, player.lowerBody == LowerBody.YETI && player.arms.type == Arms.YETI, "Geeze, with those oversized hands and feet you should just drop your weapon and punch things out cold. Why do you even need a weapon for anyway?", "Your limbs are now back to their normal size, it might be smart to use a weapon over your bare hands and feet for fighting.");
			//Gazer perks
			needNext = needNext || player.gainOrLosePerk(PerkLib.TrueSeeing, player.eyes.type == Eyes.MONOEYE || player.hasMutation(IMutationsLib.GazerEyesIM), "As part of acquiring an all seeing eye your sight has improved greatly. It would seem you now possess the innate ability to dismiss illusions and falsehood, seeing the world as it truly is.", "Your sight is not what it used to be. No longer having a cyclopean vision you have lost the power of true sight.");
			...
			//Equine Strength
			needNext = needNext || player.gainOrLosePerk(PerkLib.EquineStrength, player.isAnyRaceCached(Races.HORSE, Races.CENTAUR, Races.LAQUINE), "You are enough of a horse-morph or centaur and have gained your special abilities!", "You are no longer enough of a horse-morph or centaur and have lost your special abilities!");
			//Laquine Might
			needNext = needNext || player.gainOrLosePerk(PerkLib.LaquineMight, player.isRaceCached(Races.LAQUINE), "Woa maybe you’ve eaten too many raw carrots you feel your body ripple with power. It might also be you turning into a full Laquine doing this.", "As your body becomes less Laquine like you feel your energy is down both in libido and strength.");
			//Titan Might
			needNext = needNext || player.gainOrLosePerk(PerkLib.TitanicSize,(player.tallness >= 80 && (player.isRaceCached(Races.SCYLLA, 2) || player.isAnyRaceCached(Races.HYDRA, Races.FROSTWYRM, Races.SANDWORM))), "Whoa, you've grown so big its a sheer miracle you don't damage the landscape while moving. That said, your size now contributes to your strength as well.",
			...
			//ABOBA
			needNext = needNext || player.gainPerk(PerkLib.BasiliskResistance, flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] >= 100, (player.perkv1(IMutationsLib.GorgonEyesIM) >= 1)? "You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks! ":"You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks!");
			needNext = needNext || player.losePerk(PerkLib.EnlightenedNinetails, player.perkv4(PerkLib.EnlightenedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.");
			needNext = needNext || player.losePerk(PerkLib.CorruptedNinetails, player.perkv4(PerkLib.CorruptedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.NinetailsKitsuneOfBalance, player.hasPerk(PerkLib.EnlightenedNinetails) && player.perkv4(PerkLib.EnlightenedNinetails) > 0 && player.hasPerk(PerkLib.CorruptedNinetails) && player.perkv4(PerkLib.CorruptedNinetails) > 0 && player.tailType == Tail.FOX && player.tailCount == 9,
			...
			needNext = needNext || player.losePerk(PerkLib.Diapause, !player.hasVagina(), "With the loss of your womb, you lose your kangaroo-like diapause ability.");
			//Small Frame perk
			needNext = needNext || player.gainOrLosePerk(PerkLib.SmallFrame, player.isAnyRaceCached(Races.IMP, Races.MOUSE, Races.DEVIL, Races.AZAZEL) && player.effectiveTallness <= 60, "Due to your small build you have become harder to hit.", "Due to being too tall you no longer qualify.");
			//Small Caster perk
			needNext = needNext || player.gainOrLosePerk(PerkLib.SmallCaster, player.isAnyRaceCached(Races.IMP, Races.DEVIL, Races.AZAZEL) && player.effectiveTallness <= 60, "Your magic becomes more concentrated in your smaller body.", "Your racial alignment does not meet the conditions and as such, the high concentration of mana in your body disperses.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.Immortality, player.isAnyRaceCached(Races.AZAZEL, Races.LICH), "You gain a sense of invulnerability as you are now an Azazel.", "You lose your sense of invulnerability as you are no longer an Azazel.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.Purifier, player.isRaceCached(Races.AZAZEL), "You gain a sense of conviction as you are now an Azazel.", "You lose your sense of conviction as you are no longer an Azazel.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.QueenOfTheFairies, player.isRaceCached(Races.FAIRY, 2), "You sense a change in yourself, a faint calling from the forest. "+PerkLib.QueenOfTheFairies.desc());
			needNext = needNext || player.gainOrLosePerk(PerkLib.WhatIsReality, player.isRaceCached(Races.CHESHIRE), "You gain a sense of invulnerability as you are now an Cheshire.", "You lose your sense of invulnerability as you are no longer an Cheshire.");
			needNext = needNext || player.gainOrLosePerk(PerkLib.VorpalClaw, player.isRaceCached(Races.CHESHIRE));
			//Gain venomancy if utterly toxic
			needNext = needNext || player.gainPerk(PerkLib.Venomancy, player.isRaceCached(Races.APOPHIS) && player.cor > 50, "Your inborn toxicity has reached such a peak that even your spells are now charged with venom.");
			needNext = needNext || player.losePerk(PerkLib.Venomancy, !player.isRaceCached(Races.APOPHIS) && player.cor < 89, "Your mystical powers over poison and toxins have waned.");
