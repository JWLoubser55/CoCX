...
public function automatonEncounter():void {
	...
	if (player.isRaceCached(Races.AUTOMATA)) {
		outputText("While exploring the cave system, you run into what at first glance appears to be a genuine albeith weirdly dressed human. Must be your lucky day human’s are very easy targets for rehabilitation, you throw caution to the wind and run toward her. As you get closer, you realize her skin pigmentation is unusual… like quicksilver. Furthermore, she is surrounded by the bodies of still unconscious dark elves. There's no denying who did it. Wait, could she be? ");
		...
		outputText("… Analyzing… Designation: [race]. Evaluation: Negative. Another unit by matricule G2768 under the name of [name] detected, Engaging social mode for the purpose of social situations… Hello, unit G2768 how is your mission going?”\n\n");
		...
		if (player.hasStatusEffect(StatusEffects.PostAutomatonBeatdown)) {
			if (player.statusEffectv1(StatusEffects.PostAutomatonBeatdown) > 3) {
				if (player.blockingBodyTransformations()) {
					...
					outputText("GENERATING ARTIFICIAL INTELIGENCE PERSONALITY BASED ON RETRIEVED DATA.\n\n");
