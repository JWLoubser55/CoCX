...
public class MinervaPurification extends BaseContent
	{
		...
		private function playerParentDesc(caps:Boolean = false):String {
			switch(player.gender) {
				...
				case Gender.GENDER_HERM: //A bit wonky. I should consider the maleherms.
		...
		public function startPurification():void {
			...
			if (flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] > 0) outputText("\n\nThe close contact and her over-stimulated libido cause her penis to erect itself and begin to press against your body, the harpy/harpies Minerva has kept her looking on with jealousy that she is paying attention to you and not them.");
		...
		public function purificationByMarae():void {
			spriteSelect(SpriteDb.s_minerva_corrupt);
			clearOutput();
			...
			outputText(" Her breasts swell out magnificently, as if to complement her flared hips and be ready to feed all the young they would invite her to bare, stopping at around a full and swollen G-cup. Finally, a second pair of luminescent wings slowly takes form on her back, stretching out to maybe half the span of her original pair.");
			...
			flags[kFLAGS.MINERVA_CORRUPTION_PROGRESS] = 0; //Reset corruption tracker
		...
		public function purificationByJojoPart2():void {
			clearOutput();
			outputText("The climb up the mountains feels longer and harder than before - maybe because Jojo, despite his admirable dedication, clearly isn't an experienced mountain hiker. Finally, however, you reach the tower and head inside, calling out for Minerva to come and meet a friend of yours.");
			...
			outputText("\n\nThe mouse monk starts blushing so vibrantly, that you can see it through the fur on his cheeks. Clearly out of his depth, he manages to mumble, \"<i>The demons are the enemies of all good folk. I was happy that I was able to make some effort to help you.</i>\" He promptly wriggles desperately free of her, making of show of patting himself down and doing his best to not let her see his blushing. \"<i>You have a gentle soul, miss. I am happy to have freed you from corruption's vile grasp.</i>\"");

			outputText("\n\nHe makes a polite bow to the newly purified Siren. \"<i>With my work here done, I must return to... well, I guess it is my home. I do hope that [name] will show it to you. It was very nice to see your home.</i>\" Having excused himself, he heads out the door and away through the mountains. You know he'll be back at camp before you are.");
		...
		public function declineToMakeBaby():void {
			...
			if (player.gender == 1) outputText("her warm hand naughtily reaches down and grabs the bulge in your [armor], before gently squeezing");
			else if (player.gender == 2) outputText("her warm hand reaches down and slips into your [armor] teasingly, her fingers exploring until they find your womanhood and rub it gently, the golden siren giving you a naughty sharky grin. Clearly she can't wait to start her family with you.");
		...
		public function visitTowerPostPurification():void {
			...
			if (player.isPregnant() || minervaScene.pregnancy.isPregnant) chooserArray[chooserArray.length] = 4;
			if (player.cor >= 50 && !flags[kFLAGS.MINERVA_PURE_TALKED_CORRUPTION]) chooserArray = [5]; //FORCE the corruption talk
			//Select a scene!
		...
		private function minervaSinging():void {
			...
			if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0) {
				...
				outputText("\n\nThe golden siren's eyes are closed for now, as she sings her heart out. It's a song the likes of which you have never heard before, sounding at once like both a gentle harmony of songbirds and a soulful hymn. The sound entrances you as you approach the spring, your eyes glued to the near naked form of Minerva as she happily sings her heart out, obviously overflowing with joy that she is pure and free from her burden.");
		...
		private function minervaPregnancyMeet():void {
			...
			if (minervaScene.pregnancy.isPregnant) { //Minerva is pregnant.
				if (player.pregnancyType == PregnancyStore.PREGNANCY_MINERVA || player.pregnancy2Type == PregnancyStore.PREGNANCY_MINERVA) { //Pregnant with sirens.
					...
					outputText(". After some time Minerva finally speaks, her golden eyes locking with yours. \"<i>So, my sweet darling, since you're taking time out of your championing to visit me all the way up here, how shall we spend our time together?\"</i>");
				}
		...
		private function minervaBabiesMeet():void {
			...
			switch(temp) {
				...
				case 1: //Presents
					...
					outputText("\n\nWanting a little time to be with Minerva you send your daughters off with a little pat on the rump and tell them to go and play in the spring, the young herms cheering and running off to play. you stretch and put your daughters presents into your bag before approaching Minerva and sliding your arms around her, your hands playfully grabbing her wide ample rear, your touch pulling a grin and a playful gentle slap from your siren lover. \"<i>Darling, the young ones are so nearby... But, if you wanted some alone time, we could send them to play a little deeper in the tower, so we could have some privacy...</i>\" She purrs.");
					player.gems++;
					statScreenRefresh();
					break;
				default:
					outputText("<b>This text should not happen. Please let Ormael/Aimozg know so he can fix it.</b>");
			}
		}
		...
		private function minervaCorruptionMeet():void {
			...
			dynStats("cor", -3);
			flags[kFLAGS.MINERVA_PURE_TALKED_CORRUPTION] = 1;
		}
		
		public function pureMinervaMenu():void {
			menu();
			addButton(0, "Appearance", pureMinervaAppearance);
			addButton(1, "Talk", minervaScene.minervaTalkSelect);
			if (player.lust >= 33) addButton(2, "Sex", pureMinervaSexMenu);
			addButton(3, "Eat", minervaScene.eatSomethingYouCunt);
			addButton(4, "Drink", minervaScene.getADrinkYouBitch);
		...
		private function pureMinervaAppearance():void {
			clearOutput();
			outputText(images.showImage("minerva-appearance-pure"));
		...
		public function talkingToMinervaAboutBackstory():void {
			...
			if(flags[kFLAGS.MINERVA_BACKSTORY] == 0) {
				...
				if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) {
			...
			//PART 2 - DONE
				...
				outputText("Informing Minerva that you want to sit down and talk with her seems to surprise her, but a gentle smile quickly dawns on her face as she leads you over to a soft patch of moss. \"<i>Really, you want to talk to me? I thought we knew each other pretty well but if you wanted to talk I'd be happy to talk to you. What would you like to know, my love?</i>\" Sitting across from her, you ask to continue her tale about her past. The shark-harpy purses her lips and smiles softly.\"<i>Well.... I suppose if you want to hear it, I would be alright with telling you the story.</i>\" she says before running a hand through her red hair.");
				outputText("\n\nThe red-headed siren began her story again, picking up where she left off. \"<i>Well I think I was at the part with that anemone. That was the start of some bad times in this world but also the start of one of the best times.... so after the incident with the anemone and getting my cock from the resulting birth I fled from the lake; I didn't want to run into any more of the creatures there, so I tried to survive more inland. \"<i> She chuckles and once again looks down at herself. \"<i>It didn't work out too well as you can see. Well... I found this farm and it seemed alright, but I didn't want to take any chances, and I was really, really hungry at the time, so I just stole some of the peppers that the woman grew there, these bulbous ones, they smelled so good, that I scarfed them down like a hungry animal.</i>\" Minerva sighs and squirms a little in her spot, blushing at the memory. \"<i>Silly things, I had no idea at the time they would make me grow balls, and since I ate so many they got really big.</i>\" She seems uncomfortable, but, at the same time, from the look on her face the memory isn't entirely unpleasant.");
			...
			//PART 3 - DONE
				...
				outputText("\n\nAfter a moment or two of reminiscing Minerva opened her eyes again and sighs happily. \"<i>We stayed there at the stream for months. It was just us, mother and daughter. The stream even had fish in it, plenty of fish to feed just the two of us. It seemed safe for us there and while she was growing up we needed to be by the water. She...grew up fast though. In a matter of months she was about an adult's size. Though she was smaller than me, she was the size of the shark-girls, and her father.</i>\" You couldn't help but notice the sadness in her voice.");
				...
				outputText("\n\n\"<i>I was so hungry I didn't care if they changed me. So I dug in, eating the walnut sized seeds one after another until I couldn't eat anymore.</i>\" bringing her wings in Minerva gently strokes her feathered limbs. \"<i> I grew these very soon after, sprouting right from my back as my arms got covered in these sleeve-like feathers. It hurt to grow them, but it didn't matter to me. I was full of food and had a way to get around quicker. Still sore from the growing I tried my beautiful new wings out, taking off into the air to explore for a way to stem my corruption.</i>\" Her wings fluttered and spread out gracefully, she is clearly proud of her white and gold wings.");
				outputText("\n\n\"<i>I must have flown for hours; the seeds I ate getting burned off fairly quickly, leaving me starving....again... By the way, why is normal food so hard to come by around here?</i>\" she asked seriously, as if it had been on her mind for a while. Shrugging your shoulders you really had no answer for her, though you had been wondering that yourself. \"<i>Well, at least I have my peaches here, they don't transform you. So, after about an hour or two I finally could go no further, exhaustion, hunger and corruption catching up with me, I crashed onto a cliffside. I could feel its warm all consuming grasp closing around me, it was so tempting to give in and let it happen. As fate would have it I landed on the rocks just outside this tower, I hadn't seen it while in their air because of the mist,</i>\" smiling gently the siren stroked the soft moss that grows from the ground almost affectionately. You can't help but notice the somewhat bittersweet look on her face, but it's only there for a moment, replaced with a happy grin.");
				outputText("\n\n\"<i>I thought that if I was going to become a monster I at least wanted to have some shelter, so I went inside the tower. That's where I found this place. This... oasis of life, hidden away inside the tower. I guess it was built as some kind of sanctuary though it clearly fell into disrepair. This place turned out to be my....salvation... As soon as I saw fruits on the trees and fresh water in the spring I couldn't help myself. I feasted, my corruption forgotten I just gorged on the fruit and water, and with each bite, each drink, I felt the corruption reseed. My mind and thoughts became more clear, clearer than they had been in a long time. This place is blessed, it didn't get rid of the corruption, but it claimed it, made it shrink. I was happy to be more or less back to normal, but...I knew I could never really leave this tower, if I did, the corruption would just come back. This place, while beautiful, was my prison, my gilded cage.</i>\" She says before looking at you. \"<i>I'm glad though, that I went through all that, if I hadn't I would never have met you, and you never would have saved me. Despite the hardships, I wouldn't go back and change anything,</i>\" she says before hugging you gently.");
		...
		public function talkToMinervaAboutSpring():void {
			...
			outputText("\n\nThinking about it your decide there would be nothing wrong with going for a dip with the curvaceous golden siren. Getting up from the ground you quickly strip off your [armor] and jump into the pool. Seeing how eager you are brings a toothy smile to Minerva's slightly angular face, the purified herm jumping up and eagerly stripping as well, freeing her big bountiful breasts, before hopping into the water with a energetic splash.");
		...
		private function declineSpringSex():void {
			clearOutput();
			outputText("Staying like this for a while the two of you just rest and enjoy each others company, idly talking about a few things before you figure that its time you should go. Putting your [armor] back on you gather your things as Minerva watches, pouting that you have to leave so soon. \"<i>Thank you so much for coming to visit. I had a lot of fun, don't be a stranger you hear, come back and visit again soon!</i>\" she says before sitting up and sighing contently.");
		...
		public function talkToMinervaAboutSharkgirl():void {
			...
			outputText("Informing Minerva that you want to sit down and talk with her seems to surprise her, but a gentle smile quickly dawns on her face, and she leads you over to a soft patch of moss. \"<i>Really, you want to talk to me? I thought we knew each other pretty well but if you wanted to talk I'd be happy to talk to you. What would you like to know, my love?</i>\" remembering your past conversations with Minerva you remember her mentioning that she had birthed a shark girl daughter at one point in her life, it seemed like a sensitive topic with her though. Really wanting to know more you ask to know more about this daughter of Minerva's");
			...
			outputText("\n\n\"<i>The little one would never leave my side, I had never been a mother before then, so I did the best I could, I played with her, kept her well-fed and taught her all that I could, how to feed herself, how to defend herself and be wary of her surroundings at all times, like a sponge she absorbed it all up. I really wish I could have had more time with her...</i>\" The siren says, sadness bleeding out into her smile, clearly wishing she could see her firstborn daughter again.");
			outputText("\n\n\"<i>I'm sure I have told you this already, but there was an incident between us. She grew up so fast, became an adult in only a few months really. It was such a shock to me, I had no idea she would grow that quickly, if I had, I would have taught her not to just let her more primal needs to get the better of her</i>\" She said looking at the oasis and sighing. \"<i>I guess it could've had something to do with her shark-girl nature as well, the ones I encountered always were aggressively sexual creatures especially when it came to breeding.</i>\" Minerva says with thoughtful look on her face before shaking her head.");
		...
		public function talkToMinervaAboutMotherhood():void {
			...
			outputText("With your tale over, your audience claps excitedly, the lonely siren very happy to hear such silly stories. \"<i>Oh that was wonderful! You tell such great stories!</i>\" she says happily as her tail sways between her legs. \"<i>It was a lot of fun to listen to even if they were a bit silly.</i>\" She says with a bright smile before looking down and putting a hand on her toned flat tummy. \"<i>Those damsels and those goblins, if the stories were real, I'm sure you gave them plenty of children.</i>\" ");
			...
			if (minervaScene.minervaRomanced()) outputText("hug her tightly and kiss her cheek, telling her that she will have children one day, that you will make sure of it.");
		...
		public function talkToMinervaAboutTree():void {
			...
			outputText("have been a source of much desired entertainment on my long vigil.</i>\" The nature goddess admits freely. Her usual serene appearance selling a little brighter than you remember. \"<i>We have talked about a great many things. She even told me her tale of how she came to be here in this world.</i>\" ");
			if (flags[kFLAGS.MINERVA_CHILDREN] > 0) {
				...
				outputText(" \"<i>I'm sure its just innocent sisterly affection Marae.</i>\" Minerva replies with a nod before looking to where her young daughters were. After what happened to her first daughter it obviously wouldn't surprise her if the young sirens have a few strange tendencies.");
			}
		...
		private function sleepWithMinerva():void {
			...
			if (player.armor == armors.GOOARMR) outputText("You take your time to get the metal platebody off until you're completely naked save for the goo-girl covering you. ");
		...
		private function minervaACapacity():Number {
			return minervaScene.minervaACapacity();
		}

		private function minervaVCapacity():Number {
			return minervaScene.minervaVCapacity();
		}
		...
		private function pureMinervaSexMenu(clear:Boolean = true):void {
			...
			menu();
			addButtonIfTrue(0, "FuckHerButt", fuckMinervasAsshole,
				"Req. a cock with area smaller than " + minervaACapacity(),
				player.cockThatFits(minervaACapacity()) >= 0);
			addButtonIfTrue(1, "FuckCowgirl", minervaCowgirlSex,
				"Req. a cock with area smaller than " + minervaVCapacity(),
				player.cockThatFits(minervaVCapacity()) >= 0);
			addButtonIfTrue(2, "RestrainFuck", fuckMinervaWithHerHandsBehindHerBack,
				"Req. a cock with area smaller than " + minervaVCapacity(),
				player.cockThatFits(minervaVCapacity()) >= 0);
			addButton(3, "TakeHerDick", chooseVagOrAss);
			addButton(4, "EatHerOut", goDownOnAHermAndLoveItYouDirtySlutYou);
			addButtonIfTrue(5, "Get BJ", letMinervaSuckYouOff,
				"Req. a cock", player.hasCock());
			addButtonIfTrue(6, "GetEatenOut", minervaScene.pcGetsEatenOutByMinerva,
				"Req. a vagina", player.hasVagina(),
				"<b>Note from SH: if you notice any inconsistencies, please report them.</b>");
		...
		private function fuckMinervasAsshole():void {
			...
			outputText(images.showImage("minerva-sex-analpitch-pure"));
			...
			outputText("\n\nYou allow Minerva to hold you against her toned body, recovering your strength from the draining sex. Eventually, though, your strength returns, and you gently pry yourself free of her, telling her that the sex and her company were wonderful, but that you need to return to camp.");
			
			if(!minervaScene.minervaRomanced()) {
				//First time ending:
		...
		private function minervaCowgirlSex():void {
			clearOutput();
			outputText(images.showImage("minerva-sex-cowgirl-pure"));
			...
			outputText("\n\nWith an amorous grin, you slowly remove your clothes and put them aside, giving Minerva a chance to see your naked body. Your " + cockDescript() + " already starting to stiffen with Minerva's touch, you lay back on the soft mossy ground, patiently waiting for the siren to make the first move. You don't have to wait long, soon Minerva's tight tube top and short shorts and pulled off, her full-yet-perky G-cup melons bouncing free of their fabric prison. Clearly excited by the thrill of actually having regular sex you can see her male endowment quickly growing hard between her legs, the forearm length appendage ready to go despite the fact it won't be the main event.. Looking down at you Minerva approaches you, her ample broodmotherly hips swaying wide with every step. Standing over you, she kneels down, crawling over you like some predator, one of her hands stroking your " + cockDescript() + " before sliding up to feel your [chest]. \"<i>I promise not to get too excited,</i>\" she says before grabbing hold of your maleness and starting to stroke it. Bringing her own male organ up against yours, her tentacles encircling your throbbing cock, stinging it with her potent venom to really get you fired up as she teases you. \"<i>This stuff is always floating around in me, that's why I get so damn hard so fast. I hope you like my special venom.</i>\"");
			outputText("\n\nYou groan softly, feeling the venom already coursing through you. Your " + cockDescript() + " surges erect, the venom induced lust making you swell larger than normal, ready for her to do with as she pleases, your limbs numb and useless from the lust-inducing toxins coursing through your body. You are completely helpless and at her mercy... And a part of you likes that.");
			outputText("\n\nMinerva seems to like seeing you helpless like his, her tentacled dick seems to just get harder as her tentacles caress your member, though they don't sting anymore; instead, they tease you with their slick touch. \"<i>You like that, don't you? You're so hard I can feel your need only growing more and more. You want my tight pussy so badly.</i>\" She says with a grin before rising her body up and aligning your " + cockDescript() + " to her surprisingly cool cunt, already her lips are soaked, her slick juices running down your dick as she teases you more. Finally, she ends your torment and aligns your fat bloated tip to her refreshingly wet entrance before pushing down, letting you penetrate with agonizing slowness, her longer tentacles still stroking and caressing your length as she pulls you into her snatch's vice-like grip. Inside, you can feel smaller tentacles starting to caress and squeeze you, clearly another trait she was gifted with from the shark-girls.");
			...
			//continue
			if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) outputText(" You really are trying to get me pregnant all over again, aren't you? My little babies are all swimming in their daddy's sperm; how very naughty!</i>\"");
			//she's ALWAYS ready
			...
			outputText(" the base of your [cock]], stinging you again and again like an overzealous jellyfish, pumping more of her aphrodisiac venom into your genitals in mind-numbing quantities.");
			...
			if (minervaScene.pregnancy.type == PregnancyStore.PREGNANCY_PLAYER) outputText("Her massive childbearing tummy pressing against your body gently as you snuggle.");
			outputText("\n\nYou allow Minerva to hold you against her warm, voluptuous body, recovering your strength from the very draining sex you just had. Eventually, though, your strength returns, and you gently pry yourself free of her, telling her that the sex and her company were wonderful and despite still being very aroused due to all the venom she stung you with, you need to return to camp.");
		...
		private function fuckMinervaWithHerHandsBehindHerBack():void {
			...
			clearOutput();
			flags[kFLAGS.TIMES_MINERVA_SEXED]++;
			outputText(images.showImage("minerva-sex-restrain-pure"));
		...
		private function minervaLapSex():void {
			...
			outputText("\n\nYou allow Minerva to hold you against her soft body, recovering your strength from the sex you just had. Eventually, though, your strength returns, and you gently pry yourself free. Telling her that the sex and her company were wonderful and, despite still being very aroused due to all the venom she stung you with, you need to return to camp.");
		...
		private function getButtFuckedYouSlut():void {
			...
			clearOutput();
			flags[kFLAGS.TIMES_MINERVA_SEXED]++;
			outputText(images.showImage("minerva-sex-analcatch-pure"));
			outputText("Thinking about just what kind of heated, heart-pounding, lusty fun you want to have with your siren lover, your eyes look the shark-like harpy up and down, taking in her features. Her legs, her curvy hips... your eyes travel past the thick growing bulge in her shorts, up past her scarred, toned belly and to the siren's large, luscious breasts before finally reaching her face. Grinning to the golden and white herm, you decide what you want from her.");
		...
		private function goDownOnAHermAndLoveItYouDirtySlutYou():void {
			clearOutput();
			flags[kFLAGS.TIMES_MINERVA_SEXED]++;
			outputText(images.showImage("minerva-sex-eatout-pure"));
		...
		private function noBallsMinervaAndContinuation(clear:Boolean = true):void {
			...
			outputText(images.showImage("minerva-sex-eatout2-pure"));
			...
			//if demonic tongue:
			if(player.tongue.type == Tongue.DEMONIC) outputText(", your perversely prehensile tongue slowly retracting from her quivering quim, rubbing and teasing as it comes out and nearly sending the overly sensitive herm into another climax");
			outputText(".");
		...
		private function letMinervaSuckYouOff():void {
			...
			if(x < 0) x = player.smallestCockIndex();
			clearOutput();
			outputText(images.showImage("minerva-sex-blowjob-pure"));
			flags[kFLAGS.TIMES_MINERVA_SEXED]++;
		...
		public function pregnancyStage1(repeat:Boolean = false):void {
			...
			if (!repeat){
				...
				doYesNo(pureMinervaSexMenu, camp.returnToCampUseOneHour);
			}
		}
		...
		public function pregnancyStage2(repeat:Boolean = false):void {
			clearOutput();
			if (!repeat) {
				outputText("Looking over the ecstatic siren you could swear she is even happier than before, her belly having doubled in size, her gravid form seeming to radiate the joy she feels, her wings fluttering restlessly as if she wants to just take to the air in happiness. Though her belly is growing larger by the day Minerva seems to take it easily, her fertile curvy body seemingly molded and crafted to carry and birth her young with ease, and with such wide hour-glassy hips like hers, childbirth itself should be relatively easier on her. Seeing your eyes wander up and down her body like that brings a blush to her face, the motherly siren looks down at her swollen body and gently rubs the sign of her pregnancy, her touch looking so gentle, like caresses of wonder and reverence.");
				outputText("\n\nWhat with how your last visit went you were sure that she would have immediately tacked you in her joy, unfortunately for you her restlessness and sheer delight could no longer be contained. Letting out a high-pitched squeal before running to you, arms spread out she tackles you once again, grabbing you up in a bone crushing hug before spinning both of you around and around and pulling you into a kiss, a deep hot tongue filled kiss. Her succulent black lips pressed against yours, her long prehensile tongue slithering into your mouth and nearly going down your throat, her hands squeezing you against her voluptuous form. The great swell of her breasts pressing against your [chest] her tail even coming around to wrap itself around your [legs]. The tight, squeezing, joyful embrace lasting what seems like an eternity, the kiss going on and on as Minerva desperately tries to show you how happy she is to be pregnant with your child, or children judging by the size of her stomach.");
		...
		public function pregnancyStage3(repeat:Boolean = false):void {
			...
			if (!repeat || !player.hasCock()) { //because another scene depends on it...
				...
				if (player.hasBalls()) outputText("\n\nSliding between your legs, her hands stroke and caress your " + ballsDescript() + ". gently kneading and massaging the seed filled nuts with tender softness to coax your future climax to its fullest.");
				...
				if (player.hasBalls()) outputText("\n\nDespite how much you just came your massive venom filled testicles are still achingly bloated with seed, Minerva's overzealous stinging demanding that you fill her over and over to sate her desperate hormone driven need.");
				...
				if (player.hasBalls()) outputText(" Thankfully your aching swollen nuts shrink with each geyser like gush you let loose from your massively swollen prick, though if anything it only seems to mitigate the effects of all the siren venom inside you not stop it and soon your balls will merely refill.");
				outputText(" As soon as she feels the seed filling her again Minerva bursts into a seconds orgasm, her wings flaring out as she rams her hips down against your own, her feet sliding underneath you as she clenches her hips and thighs, crushing herself against you as her body squeezes and milks your swollen pulsating prick for every drop you can give. The newfound pressure keeping much of your second load contained within the tight hungry confines of her womb. \"<i>Yes! Yesss! Fill me! Fill me so much! Give our babies all the cream they want! Make them swim in it!</i>\" the wanton herm screams as she cums so hard above you, demanding that you fill her up with everything you can muster.");
				outputText("\n\nThough her womb swells with the immense load Minerva can only hold so much inside, the remainder of your pressurized load gushing out in great spurts from between your fleshy connection, a small pool soaking your hips and thighs as you finally finish your orgasm. Despite how much you have done already neither of you are even close to being done, Minerva's hormone flooded body craving more pleasure, more sex to feed her insatiable craving for your fertile cock cream, your own body still roiling with incredible lust thanks to all the siren venom inside you, your need to rut and breed not allowing you to rest.");
				outputText("\n\nAs if to emphasize how much she wants you Minerva lifts off you with a wet sloppy pop, a rush of thick jizz flows like a waterfall from her soaking cunt as she gets down onto her hands and knees, chest resting on her big milky tits with her arms cradling the massive bulge of her belly. With her round juicy ass presented so wonderfully to you, the taut round peach of gold and cream flesh swaying back and forth as her tail rises and legs spread. \"<i>More lover! Fuck your siren, let out all your lust, breed my hungry golden pussy!</i>\" her words are the last straw that breaks your restraints, grabbing hold of her hips you get up from your position and ram yourself back inside with a wet squish of combined sexual juices as you continue the ravenous rut, the overpowered need to mate and breed could keep you both like this for hours.");
				player.sexReward("vaginalFluids","Dick");
		...
		public function minervaGivesBirth():void {
			...
			if (model.time.hours < 6) {
				...
				outputText("\n\nThe newborn sirens begin to latch onto Minerva's breasts and suckle milk. \"<i>They're great. They'll grow up nicely,</i>\" she says. You give her a kiss on the cheek and let her know that you're going back to your camp. You wave her goodbye and make your way back to camp.");
				doNext(camp.returnToCampUseOneHour);
			}
		}
		...
		public function playerGivesBirth(womb:int = 0):void {
			...
			if (flags[kFLAGS.MINERVA_TOWER_TREE] > 0) outputText(" under the giant oak tree");
			outputText(". \n\n\"<i>You'll be okay. Just take a deep breath,</i>\" she instructs. You do as she requested, taking deep breaths. She slowly removes the bottom half of your [armor] so that your groin is exposed. ");
			if (player.hasCock()) {
