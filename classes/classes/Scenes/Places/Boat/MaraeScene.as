...
public class MaraeScene extends AbstractBoatContent implements TimeAwareInterface {
    ...
    public function canVisit():Boolean {
        return canVisitNormal() || canVisitPure() || canVisitCor();
    }
    ...
    public function canVisitCor():Boolean {
        return flags[kFLAGS.MET_MARAE_CORRUPTED] > 0 && player.gender > 0 && flags[kFLAGS.CORRUPTED_MARAE_KILLED] <= 0;
    }

    public function visitSelector():void {
    ...
    public function enoughPerks():Boolean {
        switch (player.gender) {
            case 1:
                return player.hasPerk(PerkLib.MaraesGiftStud) && player.hasPerk(PerkLib.MaraesGiftProfractory);
            case 2:
                return player.hasPerk(PerkLib.MaraesGiftFertility) && player.hasPerk(PerkLib.MaraesGiftButtslut);
            case 3:
                return player.hasPerk(PerkLib.MaraesGiftStud) && player.hasPerk(PerkLib.MaraesGiftProfractory) && player.hasPerk(PerkLib.MaraesGiftFertility) && player.hasPerk(PerkLib.MaraesGiftButtslut);
            default:
                return true;
        }
    }
        ...
        if (flags[kFLAGS.FACTORY_SHUTDOWN] <= 0) {
			...
			endEncounter();
                    ...
                    //If not
                        ...
                        outputText("She sighs heavily, and you notice the bark of her nipples stiffening.  Her brow creases with something approximating worry as she continues, \"<i>They know of me.  My power originally kept them far from the shores of the lake, but they seek to corrupt me – to make me like them.   They've used magic and industry to trap the pure rains in the clouds around their mountain, starving me, and in this place they spill their tainted sexual fluids.   For... years now, my furthest reaches have been bathed in their vile cum.   While my power is great, I... I cannot resist forever.  My reach has dwindled to little more than this lake.  Parts of me have already fallen, taking the surrounding life with them.  I do not know how much longer I can endure... even now, the desire to mate you rises within me.</i>\"\n\n");
    ...
    public function firstCorruptEncounter():void {
        ...
        if (flags[kFLAGS.MET_MARAE_CORRUPTED] <= 0 || recalling) {
            if (!recalling) {
        ...
        if (player.hasCock()) {
            ...
            if (player.tentacleCocks() < player.cockTotal()) {
                for (var i:int = player.cocks.length - 1; i >= 0; i--) {
                    transformations.CockTentacle(i).applyEffect(false);
                }
            }
            ...
            transformations.CockTentacle(0, 36, 2).applyEffect(false);
        }
    ...
    private function maraeStealLethicite(deliberate:Boolean = false):void {
        ...
        //(FAIL)
            ...
            if (player.isGenderless() && recalling) {
                outputText("Sorry, no genderless recalls here.");
                doNext(recallWakeUp);
            }
            var studCheck:Boolean = player.hasCock() && (!player.hasPerk(PerkLib.MaraesGiftStud) || !player.hasVagina() || recalling);
            var fertCheck:Boolean = (player.hasVagina() || player.gender == 0) && (!player.hasPerk(PerkLib.MaraesGiftFertility) || !player.hasCock() || recalling);
            if (sceneHunter.lossSelect) {
                outputText("Which gift do you want?\n\n");
                menu();
                if (!player.hasCock()) addButtonDisabled(0, "Stud", "You lack the required parts for this one.");
                else addButtonIfTrue(0, "Stud", stud, "You already have it.", studCheck);
                if (!player.hasVagina()) addButtonDisabled(1, "Fertility", "You lack the required parts for this one.");
                else addButtonIfTrue(1, "Fertility", fertility, "You already have it.", fertCheck);

            } else if (studCheck && (!fertCheck || rand(2) == 0)) stud();
            else fertility();
            ...
            function fertility():void {
                ...
                if (!recalling) player.cuntChange(20, true, true, false);
                outputText("\n\n");
    ...
    private function MaraeIIStageII():void {
        ...
        sceneHunter.selectGender(dickF, vagF, null, hermF, 0);
        //====================================================================
        //[Girls]
        ...
        function hermF():void {
            ...
            if (!recalling) player.cuntChange(12, true, true, false);
            if (!recalling) player.buttChange(12, true, true, false);
            outputText("\n\n");
    ...
    private function MaraePt2RoundIIIPrizes(gender:int):void {
        spriteSelect(SpriteDb.s_marae);
        clearOutput();
        //[EPILOGUE]
        //[Dudes]
