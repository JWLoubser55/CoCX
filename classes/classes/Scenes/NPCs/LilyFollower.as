	...
	public class LilyFollower extends NPCAwareContent implements SaveableState {
		...
		public function lilyEncounter():void {
			...
			if (flags[kFLAGS.LILY_LVL_UP] == 1) {
				...
				outputText("\"<i>I found you [master], I'm so happy!</i>\" croons the drider.\n\n");
		...
		public function LilyAfterBattle():void {
			...
			addButton(5, "DomHome", LilySubComeCamp)
				.disableIf(LilySubmissivenessMeter < 80, "Req. 80%+ submissiveness.", "???");
			addButton(6, "Come2Camp", LilyBringCamp)
				.disableIf(LilyAffectionMeter < 50, "Req. 50%+ affection.", "???");
		}
		...
		public function LilyTalk():void {
			...
			menu();
			if (LilyFollowerState) addButton(0, "Back", LilyCampFollower, true);
		...
		public function LilySubComeCamp():void {
			...
			menu();
			addButton(0, "BringIn", LilyEnterCamp);
			addButton(1, "Tyrantia", LilyTyrantia3some1st)
		...
		public function LilyTyrantia3some1st():void {
			...
			outputText("<i>\"Ohh, yes!\"</i> Lily’s drooling steadily, from both her mouth and cunt, her chitin-covered front legs shaking with anticipation. <i>\"Hurry, [master]!\"</i>\n\n");
			...
			outputText("Lily subsides, spreading her legs wide and letting you see every fold of her well-fucked pussy. <i>\"I’m sorry, [master]. Please forgive your...pet.\"</i>\n\n");
			menu();
		...
		public function LilyTyrantia3Lick():void {
			...
			if (!recalling) player.sexReward("saliva","Pussy");
			if (!LilyFollowerState || recalling) LilyTyrantiaAfterFirst();
			else LilyTyrantiaAfterSex();
		}
		...
		public function LilyTyrantiaAfterFirst():void {
			...
			outputText("<i>\"Nope!\"</i> Tyrantia grins. <i>\"And you’re not gonna let your big bad sis stop you from pleasing your [master], are you?\"</i> Lily shakes her head, and looks at you. She’s still spunk-drunk, but visibly pulling herself back together.\n\n");
		...
		public function LilyEnterCamp():void {
			...
			outputText("<i>\"Truly [master]? You want your toy around?\"</i> Lily shakes her upper body at you, making sure her chain rattles. <i>\"What naughty things are you going to do to me?\"</i> She gives you a rather lascivious smile. <i>\"On second thought...Don’t tell me. Make it a surprise...MMmmm!!\"</i> Lily charges off, setting her sights on a tree near the river. <i>\"I’ll make myself a little nest, and then we’ll...Oh, I am going to love it here!\"</i>\n\n");
		...
		public function LilyRape():void {
			...
			outputText("You strip off your [armor]"+(!player.lowerGarment.isNothing?" and [lowergarment]":"")+", revealing your "+(player.hasCock()?"[cock]":"")+(player.gender == 3 ? " and " : "")+(player.hasVagina()?"[pussy]":"")+", and the Drider begins to writhe, struggling to undo the bindings on her legs.\n\n");
		...
		public function LilyBondageCabinWait():void {
			...
			outputText("Calling her a bad girl, you tell her that she’s not supposed to cum until her [master] does. Good girls are supposed to make their partners feel good, too. Accentuating that, you walk over to her drider back half, slapping her rear, hard.\n\n");
		...
		public function LilyBondageCabinTease():void {
			...
			outputText("<i>“I WANT TO CUM!”</i> She wails. <i>“Please, let me cum!”</i> You tut. Not what you were looking for. <i>“[Master], please…I-I-I want to c-cum! Make me…Take me!”</i> Almost sobbing, Lily’s swollen cunt is shaking, the spider bitch is so <i>close</i>.\n\n");
		...
		public function Lily3Somes():void {
			...
			outputText("<i>\"Oh, [master], I love you!\"</i> She cries, rushing over to your cabin. <i>\"I’ll be trussed up for you before you get back! Please don’t keep me waiting…\"</i> She closes the cabin door behind her, and you can all but hear the sound of her webbing being made. Who do you want to be your other partner tonight?\n\n");
			menu();
		...
		public function SandwichSidonie():void {
			...
			outputText("Lily squeals in delight, wiggling her breasts as much as she can to entice your mutual lover. \"O-oh, yes! Sidonie! [master] has great taste!\" Sidonie takes a hold of Lily’s top breasts, kneading them in her strong hands before lining herself up with Lily’s cunt.\n\n");
		...
		public function LilyExhibition():void {
			...
			addButton(2, "Electra/Luna", LilyExElectra)
				.disableIf(!player.hasCock(), "Req. a cock!")
				.disableIf(flags[kFLAGS.ELECTRA_FOLLOWER] <= 1 || player.hasStatusEffect(StatusEffects.ElectraOff)
					|| flags[kFLAGS.LUNA_FOLLOWER] < 4 || player.hasStatusEffect(StatusEffects.LunaOff),
					"You don't have someone for this one... yet?", "???");
			//addButton(3, "Izma/el", LilyExIzma);
			addButton(4, "Joy", LilyExJoy)
				.disableIf(flags[kFLAGS.JOY_COCK_SIZE] >= 1, "Joy can't have a dick!", "???" )
				.disableIf(flags[kFLAGS.JOJO_BIMBO_STATE] != 3, "You don't have someone for this one... yet?", "???");
		...
		public function LilyBondageTreeLilyLegs():void {
			...
			outputText("<i>\"[master], I’ve been a good slut. I d-did what you asked for...so please, can you make me feel good too?\"</i>\n\n");
			lilySubmissiveness(10);
			lilyAffection(10);
			menu();
		...
		public function LilyReward():void {
			...
			else outputText("[breasts]. She takes a nipple in two fingers, rolling the nub around and pulling on it.");
		...
		public function LilyCampFollower(back:Boolean = false):void {
			...
			} else outputText("<i>\"Yes, [name], what did you need?\"</i>\n\n");
			menu();
		...
		public function LearningWithLily():void {
			...
			if (LilyTalked > 4) {
				...
				addButton(14, "Back", LilyCampFollower, true);
			}
			else {
		...
		public function LearningWithLilyYes(book:Number):void {
			...
			switch (book) {
				case 1:
					player.buff("Learning with Lily").withText("Learning with Lily").setStats({"spe.mult":0.4}).forHours(192);
					break;
				case 2:
					player.buff("Learning with Lily").withText("Learning with Lily").setStats({"int.mult":0.4}).forHours(192);
					break;
				case 3:
					player.buff("Learning with Lily").withText("Learning with Lily").setStats({"lib.mult":0.4}).forHours(192);
					break;
				case 4:
					player.buff("Learning with Lily").withText("Learning with Lily").setStats({"str.mult":0.4}).forHours(192);
					break;
				case 5:
		...
		private function LevelingHerself():void {
			...
			if (flags[kFLAGS.LILY_LVL_UP] < 19 && flags[kFLAGS.LILY_DEFEATS_COUNTER] >= flags[kFLAGS.LILY_LVL_UP] + 5) {
				...
				if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers5))
					...
					player.createStatusEffect(StatusEffects.CampSparingNpcsTimers5, 0, addToV2, 0, 0);
				flags[kFLAGS.LILY_DEFEATS_COUNTER] = 0;
				++flags[kFLAGS.LILY_LVL_UP];
			}
		}
	}
}
