...
private function acceptRebeccsPlea(firstTime:Boolean = false, sacrificed:Boolean = false):void {
	...
	//if(player.armorName != "comfortable clothes") outputText("  The farmers almost eagerly help you out of your [armor], supplying you with a set of shabby but comfortable peasant clothes.");
...
public function loseOrSubmitToVapula():void {
	...
	if(player.lactationQ() > 0) outputText("\n\nLost in your feverish state of arousal, you feel your nipples harden and a thin trickle of milk comes out.  A couple of demons spot this new source of fluids and rush to your [allbreasts], wolfishly suckling them.  Their dexterous tongues keep teasing your nipples, stimulating you further as they drink your essence.  Your milk-udders are being roughly groped and licked by these careless creatures, oblivious of everything but your mounds.");
...
public function rapeZeVapula():void {
	if (recalling) {
		monster = new LustyDemons();
		monster.lust = monster.maxLust();
	}
	else flags[kFLAGS.VAPULA_SUBMISSIVENESS] -= 10;
	...
	function dickF():void {
		...
		outputText("\n\nWith lazy nonchalance you grab hold of another demon crawling at your feet, another succubus; although not so tall or big-breasted as Vapula, her provocative curves are a delight to your eyes.  You bend her over and, without ceremony, start working her tight pucker, filling her insides with your [cock].");
		...
		if(player.cockTotal() > 1) outputText("In an unholy sex-frenzy, you want more; you start grabbing creatures by groups and alternatively pump in and out of their asses with your [cock], always hungry for more anal.");
...
private function rapeRebecc(outside:Boolean = false):void {
	...
	function dickF():void {
		...
		outputText("\n\nThe sight of her glorious rear is too tempting a target to resist, and you quickly shove the tip of your [cock] between her plush cheeks, bracing yourself for torrid anal penetration.  She keeps whining, unable to break free of your iron grasp.  In her constant struggles you notice an opening between her buttocks - you thrust inside with all your might, devastating her backdoor entrance, stretching her holes, tearing her insides and grinding against her rugged anal walls in a extremely rough way.");
		//[if cock is too long, >12 inches]
		if(player.cocks[0].cockLength > 12) outputText("  You keep burying more of your [cock] deep inside her rectum until you reach her limit and can't fit more cock in.");
		...
		outputText("\n\nWith a last, satisfied sigh, you fall over, [cock] deeply buried inside Rebecc's butt as you feel her warm, curvy body slowly waggling below you.  ");
...
private function faceDownHordes():void {
	clearOutput();
	outputText("You tell Rebecc to go hide on the plains; you're going to handle the villagers as well as the demon horde on your own.  She cries, \"<i>But [name]!  You stand no chance against this ruthless horde!  Besides, the villagers won't trust you.  They'll think you want to cheat them... they'll tie you up to prevent you from getting away!</i>\"");
...
private function subdueVapula():void {
	...
	addButtonIfTrue(1, "JustFuckEm", rapeZeVapula, "Not aroused enough... or genderless.", player.gender > 0 && player.lust >= 33);
	addButtonIfTrue(2, "Enslave", enslaveVapula, "Not corrupted enough!.. or genderless.", player.gender > 0 && player.cor >= 66 - player.corruptionTolerance);
	addButton(4, "Skip Out", cleanupAfterCombat);
}
//Option: Disband (Z)
...
private function enslaveVapulaAsACuntWielder():void {
	...
	flags[kFLAGS.VAPULA_FOLLOWER] = .5; //[QUE VAPULA NIGHT MEETING]
	enslaveVapulaShared();
}

private function enslaveVapulaShared():void {
