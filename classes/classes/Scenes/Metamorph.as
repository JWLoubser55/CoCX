...
//			}
		...
		private function openPaginatedMenu (title: String, thisMenu: *, currentPage: int, memArray:Array, index:int = -9000): void {
			...
			for each (var genMem: * in pageMems) {
				...
				if (unlocked && !partsInUse) {
					if (enoughSF || (enoughMana && player.hasPerk(PerkLib.Soulless))) addButton(currentButton++, buttonStr, doMetamorph, title, genMem, index).hint("Cost: " + cost + " SF" + (genMem.info ? "\n\n" + genMem.info : "") + " OR " + (player.maxMana() / 10) + " mana (for True Demons)");
					else addButtonDisabled(currentButton++, buttonStr, "Cost: " + cost + " SF (You don't have enough Soulforce for this metamorphosis!)"+ " OR " + (player.maxMana()/10) + " mana (for True Demons)");
				}
