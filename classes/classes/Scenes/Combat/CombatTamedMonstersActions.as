	...
	public class CombatTamedMonstersActions extends BaseCombatContent
	{
		...
		public function tamedMonstersActionMenu(no:Number):void {
			...
			if (no == 5) {
				...
				if (flags[kFLAGS.TAMED_MONSTER_PROTECTING] > 0) addButtonIfTrue(4, "Protect", curry(tamedMonsterProtect, 4), "Tamed monster No.5 has not yet fully recovered.", player.statusEffectv2(StatusEffects.TamedMonster05)>0, "Command tamed monster No.5 to protect you.");
				else addButtonDisabled(4, "Protect", "You already commanded one of the tamed monsters to protect you.");
			}
		...
		public function tamedMonsterAttackMagic(no:Number):void {
			...
			var weapon:Number = 0;
			var dmg:Number = 0;
			if (no == 1) dmg += SceneLib.combat.scalingBonusIntelligenceTamedMonster(1);
			if (no == 2) dmg += SceneLib.combat.scalingBonusIntelligenceTamedMonster(2);
