...
public class FrostGiant extends Monster
	{
		...
		public function giantAttackPunch():void {
			...
			if (player.getEvasionRoll()) {
				...
				if (rand(player.spe + 120) < spe) {
					...
					damage = ((((str + weaponAttack) * 3) + 1500) + rand(600));
					...
					damage = 450 + rand((str + weaponAttack) * 3);
					player.takePhysDamage(damage, true);
				}
			}
		}
		...
		public function giantGrab():void {
			...
			if (player.getEvasionRoll()) {
				...
				var giantDmg:Number = 300 + rand(240);
				HP -= giantDmg;
		...
		public function giantGrabStruggle():void {
			if (rand(100) >= player.strStat.core.value || rand(10) == 0) giantGrabFail();
		...
		public function giantGrabFail(struggle:Boolean = true):void {
			...
			if (struggle) {
				...
				if (player.strStat.core.value >= 100) {
				...
				else if (player.strStat.core.value >= 75 && player.strStat.core.value < 100) {
					...
					player.addCombatBuff('str', -6, "Combat Debuff", "FrostGiantDebuff");
				}
				else if (player.strStat.core.value >= 50 && player.strStat.core.value < 75) {
					...
					player.addCombatBuff('str', -12, "Combat Debuff", "FrostGiantDebuff");
				}
				else if (player.strStat.core.value >= 25 && player.strStat.core.value < 50) {
					...
					if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy") {damage = 300 + rand((str + weaponAttack) * 6);
					player.takePhysDamage(damage, true);
					}
				}
				else if (player.strStat.core.value < 25) {
					...
					if (!player.hasPerk(PerkLib.Juggernaut) && armorPerk != "Heavy") {damage = 600 + rand((str + weaponAttack) * 9);
					player.takePhysDamage(damage, true);
					}
				}
				if (flags[kFLAGS.PC_FETISH] >= 2) {
					outputText("The thought of being constricted turns you on a bit. ");
					player.dynStats("lust", 80);
				}
				outputText("\n\n");
			}
			else {
			...
			switch(rand(6)) {
				...
				case 2: //Taunt
					...
					damage = 1500 + rand((str + weaponAttack) * 3.6);
					player.takePhysDamage(damage, true);
					break;
				case 3:
				case 4: //Ground Pound
					...
					player.removeStatusEffect(StatusEffects.GiantGrabbed);
					damage = 5400 + rand((str + weaponAttack) * 10.8);
					player.takePhysDamage(damage, true);
					break;
				case 5: //Throw
					...
					damage = 1500 + rand((str + weaponAttack) * 7.4);
		...
		public function giantGrabSuccess():void {
			clearOutput();
			if (player.strStat.core.value >= 100) outputText("You roar and force the giant's hand open. He gasps in surprise, using his other hand to close you in, but it's too late by then. You jump to the ground and roll away, readying for your next attack. ");
			if (player.strStat.core.value >= 80 && player.strStat.core.value < 100) outputText("You push, pull, squeeze, squirm and finally you escape the giant's grasp. You drop and roll and make distance before readying your weapon. ");
			if (player.strStat.core.value >= 60 && player.strStat.core.value < 80) outputText("With no small effort, you  pop out from the giant's clench. You run up his arm and jump down his back, making distance. ");
			if (player.strStat.core.value >= 40 && player.strStat.core.value < 60) outputText("Despite the cold, hard confines of the giant's hand, you manage to slip out of his hand and fall to the ground before scrambling up and running from the angry giant. ");
		...
		public function giantBoulderHit(mode:int = 0):void {
			...
			var damage:int = (str * 12.5) + 2500 + rand(250);
		...
		public function FrostGiant()
		{
			...
			this.hairLength = 8;
			initStrTouSpeInte(1680, 1365, 1080, 280);
			initWisLibSensCor(280, 100, 150, -30);
			this.weaponName = "fists";
			this.weaponVerb="punch";
			this.weaponAttack = 297;
			this.armorName = "ice";
			this.armorDef = 1350;
			this.armorMDef = 450;
			this.bonusHP = 15000;
