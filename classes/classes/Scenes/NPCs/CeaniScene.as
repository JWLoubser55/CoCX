...
public function basicarcherytraining2():void
{
	...
	if (flags[kFLAGS.CEANI_ARCHERY_TRAINING] == 3 && player.statusEffectv1(StatusEffects.Kelt) >= 90) {
		...
		outputText("The session ends well, you learned a few tricks but you know all too well that you're still far from your teacher's skill.\n\n");
		...
		outputText("The two of you head to the shooting range. Once there, she quickly teaches you how to position yourself properly and shows you how to throw the javelins. You do so for a few hours moving to recover your weapon every few shots. All the while you have a good show of Ceani sexy glossy skin and JJ cup breasts held by her pink bikini. She has to regularly remind you to keep the eyes on the target and not on the ‘horizon’.\n\n");
...
public function beachInteractionsAfterArcheryTraining():void
{
	...
	if (player.statusEffectv1(StatusEffects.Kindra) > 145 && player.weaponRange == weaponsrange.SHUNHAR && flags[kFLAGS.CEANI_ARCHERY_TRAINING] == 4) {
		...
		outputText("She’s on! Both of you head to the beach, weapon at the ready, and start making brochettes out of the fishes. Both of you eventually come back to Ceani’s seaside camp with your respective catches and begin counting.\n\n");
...
private function ceaniHungry(ocean:Boolean):void {
	...
	addButton(1, "Date", ocean ? oceanInteractionsDate : beachInteractionsDate)
		.disableIf(!player.hasVagina() && player.findCock(1, 15, 36, "length") < 0,
			"Req. a cock with length between 15 and 36 inches or a vagina.");
...
public function ceaniCampMainMenu():void {
	...
	if (rand(2) == 0 && flags[kFLAGS.SAMIRAH_FOLLOWER] > 9) {
		...
		outputText("The two of them notice you and Samirah excuses herself before leaving both of you in private.\n\n");
...
public function ceaniCampMainMenu1():void {
	outputText("As you walk over to her, Ceani greets you with a bright smile. Her tail swishes once or twice behind her.\n\n");
	outputText("\"<i>Good ");
...
public function ceaniAppearance():void {
	spriteSelect(SpriteDb.s_ceani);
	clearOutput();
	outputText("Ceani is an 11-feet-tall Orca morph. Her flowing pink hair are tied on the front in two bangles and left flowing on her back with a larger golden bangle to tie them together at the end. Her orange pupils always sparkle with the shine of someone who is almost always ready to laugh and her somewhat flat orca nose and cute toothy mouth only make her face all the more friendly. Her large ears are alert to sound both on land and underwater, likely she also uses them for directions. She is laying on the sand next to you smiling as you look her up. She wears a pink bikini to cover her privates.\n\n");
	outputText("Her arms and legs looks about human save for the fins on her forearms not to mention her webbed fingers and toes. Her, still wet, skin is black with a white underside that runs all along her body length up to below her nose. She has a pair of white circles under and above her eyes in an orca pattern. From her well shaped ass surges a powerful tail with a dorsal fin that swishes constantly behind her. It is strong enough to launch her several meter above the water level when she swims.\n\n");
	outputText("You don’t know if it’s because of the fact that she is a whale or because she eats fish every day but her firm JJ cup breast could smash most cowgirls flat and it's certainly not because she’s fat as her athletic waistline would tell. Likely it's just because she is this tall and as such needs to maintain proportion. You believe the absence of apparent muscle is mostly due to the fact she needs a linear shape for proper hydrodynamism, through, the fact she can lift an adult tuna with a single hand proves she’s clearly stronger than most human men.\n\n");
...
public function oceanInteractionsDate():void
{
	...
	outputText("\"<i>Sure! but I can’t leave the sea for too long. I prefer to stay close to my food if possible.</i>\"\n\n");
	menu();
	addButton(0, "Underwater", beachInteractionsDateUnderwater1).disableIf(!player.canSwimUnderwater(), "You can't swim underwater!");
...
public function campInteractionsDateAndFuck():void
{
	...
	menu();
	addButton(0, "Deep Sea", beachInteractionsDateUnderwater3).disableIf(!player.canSwimUnderwater(), "You can't swim underwater!");
...
public function beachInteractionsDateUnderwater3():void
{
	...
	outputText("\"<i>O, I have forgotten how different from the stream sea salted waters feel like. Come on [name] I wouldn't want you to be left behind especially since I am in a swimming mood.</i>\"\n\n");
...
public function beachInteractionsDateOnTheBeach3():void
{
	spriteSelect(SpriteDb.s_ceani);
	clearOutput();
	outputText("Both of you go take a stroll on the beach discussing your daily activities. There isn’t anyone aside from you two in the area and soon Ceani realise this.\n\n");
...
public function beachInteractionsDateUnderwater0():void
{
	...
	outputText("Sunbathing? Her? It's not like she will get a bronzage with that black skin, heck, she looks charred enough already. She notices your confusion then sigh looking to the sky.\n\n");
...
public function beachInteractionsDateUnderwater2():void
{
	if (player.hasPerk(PerkLib.AquaticAffinity)) {
		outputText("You and Ceani dive into the waves, both of you racing underwater and playing as you try to be the first to reach the coral barrier.\n\n");
	...
	outputText("\"<i>Hey, there's a place we didn’t go yet. It's pretty far from the coast but I think you ought to see it.</i>\"\n\n");
	outputText("She takes you all the way to what looks to be an underwater cave. As you enter, Ceani picks up a weird item that emit some light by the entrance and show you in. Wow, the rock formation is covered with a crystalline mineral that reflects the light in a bright yellow pattern. There is a small air pocket above that seems  to lead in a large set of caves that likely run’s back up the beach. ");
	...
	if (player.gender == 3) { //additional check
		...
		addButton(0, "Male", underwaterDateMaleVer)
			.disableIf(player.findCock(1, 15, 36, "length") < 0,
				"Req. a cock with length between 15 and 36 inches.");
...
public function underwaterDateMaleVer():void
{
	...
	outputText("Thanks to the low gravity underwater, she manages to slide on your [cock] at a very slow pace making you gasp as she slowly moves up and down using her tail as a lift. Heck you are glad you are underwater otherwise you are not sure you could properly lift her through the fact that her tail is doing most of the work helps. ");
...
{
	outputText("You barely register Ceani hand moving down to your pussy before she actually slides a finger in, making you gasp in surprise. You answer by breaking the kiss and taking in one of Ceani’s tits in your mouth. You begin to suckle on her tit, making her moan as she slides a second digit inside. If you weren't both standing on the ocean floor you would likely be drifting around as you try to pleasure each other. Yea the water is definitely getting warmer and your cheek flush as your pleasure increases.\n\n");
...
public function beachInteractionsDateOnTheBeach0():void
{
	spriteSelect(SpriteDb.s_ceani);
	clearOutput();
	outputText("How about a walk? It's not like she spends that much time out of the water anyway. Both of you take a stroll on the beach");
	beachInteractionsDateOnTheBeach2();
}
...
public function beachInteractionsDateOnTheBeach1():void
{
	spriteSelect(SpriteDb.s_ceani);
	clearOutput();
	outputText("How about a walk? It's not like she spends that much time out of the water anyway.\n\n");
...
public function beachInteractionsDateOnTheBeach2():void
{
	...
	outputText("You both hold hands for a few hours, resuming your walk. All's going well until suddenly, Ceani surprises you by tripping and pulling you to the ground with her. Your face ends up falling right in the middle of her massive JJ cup. You try to lift up, but she holds you still.\n\n");
...
public function beachDateMaleVer():void {
	var x:int = player.findCock(1, 15, 36, "length");
	if (player.cocks[x].cockLength >= 22) {
		outputText("Ceani licks her lips as she eyes your massive [cock], truth be told, finding someone with a cock large enough to fit her pussy up to the cervix must be a difficult task.\n\n");
...
public function beachDateFemaleVer():void {
	outputText("You can’t hold it anymore either and you slowly start to grind your pussy against hers. Your vaginal lips sliding against each other as both of you make blissful smiles.\n\n");
	outputText("\"<i>Mmmm you sure know how to tribe.</i>\"\n\n");
	outputText("Both of you coo as your cunts becomes increasingly slick with fluids. You can feel your juice slowly running around your leg all the way down to the sand below. You start moaning together as your puffy clits slides against each other.\n\n");
	outputText("\"<i>Oooooh [name] you’re making a mess out of my cunny!</i>\"\n\n");
	outputText("You maintain the hug, grinding against each other for a fair minute until Ceani pussy and legs are positively so wet if you didn't know better you'd think she had peed on herself. Your cunt is not in any better state and you feel the telltale of your incoming climax. Soon both of you cum drenching your legs and the ground with girl juice.\n\n");
	...
	outputText("She lay down on the sand letting you sit on her white belly as you move down to her, now more than juicy, snatch. The white of her underbelly runs up to her pussy lips slowly vanishing into pink. Sliding two finger in to spread her pussy lips apart, you dive in with your tongue. Ceani grabs your [ass] and pull your cunt down on her mouth as she starts to lick on your vaginal lips. ");
	...
	outputText("Both of you lick the other for a while and then cum on your respective partner faces. Tired, and barely feeling your legs, you crawl back to her face and kiss her, dozing off to what could be called a very wet dream.\n\n");
...
public function come2campCeani():void
{
	...
	if (camp.FisheryWorkersCount() > 0) {
		...
		outputText("\n\n<b>Before fully settling in your camp, as if remembering something, Ceani pulls a shining shard from her inventory and hands it over to you as a gift. You acquired a Radiant shard!</b>");
...
public function fishingContestWithCeani():void
{
	...
	outputText("She’s on! Both of you head to the stream, fishing weapon at the ready, and start making brochettes out of the fishes. Both of you eventually walk back to camp with your respective catches and begin counting.\n\n");
	outputText("The orca morph is surprised as you, not only caught more fish than her, but also bigger ones! Kindra training did pay off.\n\n");
...
public function sparringWithCeaniWon():void
{
	...
	outputText("\"<i>Wow [name] your way stronger than you look! Had I been any less tough or smaller you could have broke some of my bones. I'll have to be more careful next time. Care to help me back to my bedroll? I think I will need a nap...</i>\"\n\n");
	outputText("You help her back up and walk her off to her bedroll. She thanks you and lays down to rest for the day. It's unlikely that the two of you will be able to spar again before tomorrow anyway.\n\n");
		...
		if (flags[kFLAGS.CEANI_LVL_UP] < 12) {
			if (flags[kFLAGS.CEANI_DEFEATS_COUNTER] >= 14 + flags[kFLAGS.CEANI_LVL_UP]) {
				if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers2))
					...
					player.createStatusEffect(StatusEffects.CampSparingNpcsTimers2, 0, 0,
						player.statusEffectv1(StatusEffects.TrainingNPCsTimersReduction) * flags[kFLAGS.CEANI_DEFEATS_COUNTER], 0);
				flags[kFLAGS.CEANI_DEFEATS_COUNTER] = 0;
				flags[kFLAGS.CEANI_LVL_UP]++;
			}
		}
	}
	cleanupAfterCombat();
...
public function sparringWithCeaniLost():void
{
	...
	pc.HPChange(player.maxHP() * 0.5, false, false);
	cleanupAfterCombat();
}
