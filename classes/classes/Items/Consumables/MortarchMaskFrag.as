...
public class MortarchMaskFrag extends Consumable{

    public function MortarchMaskFrag() {
        var descr:String;
        descr = "Shard of a Mortarch's mask. You have seen them eat the other, down to the mask."
        super("MortarchMaskFrag", "Mortarch Mask fragment", "a Mortarch Mask fragment", 800, descr );
    }

    override public function useItem():Boolean {
        clearOutput();
        if (!player.hasPerk(PerkLib.ExanimationII)) {
            outputText("You examine the mortarch hollow mask fragment. You know that the hollows like to consume them but you're sure there might be a use for it.");
	...
	public function eatIt():void {
		...
		player.addPerkValue(PerkLib.ExanimationII, 1, 40);
