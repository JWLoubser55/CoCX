...
public class MarblePurification extends NPCAwareContent{
	...
	public function giveMarblePureQuestLabova():void
	{
		...
		else if(player.statusEffectv4(StatusEffects.MarbleSpecials) <= 30)
		{
		 	...
		 	outputText("\n\n\"<i>I... don’t feel different in the slightest, sweetie,</i>\" Marble replies, looking as confused as you feel. \"<i>I guess this stuff just isn’t working anymore - I must need something with more potency than that to fully purify myself,</i>\" she muses.");
	...
	public function partTwoOfClaraShowingUpAndBeingACunt():void
	{
		...
		if(SceneLib.camp.companionsCount() >= 3)
		{
			...
			if(flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1)
			{
			 	...
			 	outputText("\n\nYou wait for several minutes, but she doesn’t resurface; deciding that there’s no point in asking anymore, you leave her to sulk on the riverbed.");
			}
	...
	public function loseToClara(combat:Boolean = true):void
	{
		...
		if(player.tallness < 70) outputText("much too big for you.");
	 	...
	 	outputText("  You also find that you aren’t wearing your [armor], and your [weapon] is nowhere to be seen.  ");
	...
	public function cowCuntBadEndPartIV():void
	{
		...
		{
		 	outputText("At this point your [cocks] is rock hard, and Clara gives it a few strokes.  She does seem to lose interest surprisingly quick though.");
		}
	...
	public function cockTooBigForCowCuntageBadEndage():void
	{
		...
		if(player.cockTotal() > 1)
		{
	 	...
	 	outputText(" too big for a virgin like me to fit!  How the hell am I supposed to get that inside me?</i>\"  Clara stands up and starts stomping around the room.  ");
	...
	public function cocklessPCsGetAbusedByCowCunts():void
	{
		...
		else
		{
			...
			else
			{
				...
				//Clara eventually satisfies herself and concludes that oral isn’t all it is cracked up to be.  She has plans for the PC so that they can please her better.
			 	outputText("\n\nThat’s it, Clara is going too fast and hard for you.  You can barely breathe around the mound that is being incessantly ground into your face, and the cow-woman isn’t even bothering to give commands anyway.  Fortunately, Clara doesn’t go for much longer, and lets you free of her arch.  You’re barely conscious by this point.");
			}
	...
	public function defeatClaraCuntInAFight(cheated:Boolean = false):void
	{
		...
		//Raping or punishing Clara in camp with your followers
	 	...
	 	outputText("\n\n\"<i>No sis, you had your chance.  [name] isn't addicted to you, that means that " + player.mf("he","she") + "'s up for grabs.  That's how things work for our race.</i>\"");
		...
		if(SceneLib.camp.loversCount() > 1)
		{
	 	...
	 	outputText("\n\n\"<i>There, you see?  Now I'm sure you understand why me and [name] here belong toogether.  I won't deny the proper order of things, and I'll make sure that " + player.mf("he","she") + " leads a fulfilling life as my milk slave.  " + player.mf("He","She") + "'ll even by my first husband!</i>\"");
	 	...
	 	outputText("\n\n\"<i>Then if [name] loves you so much, why can't " + player.mf("he","she") + " change for your sake?  Why can't they become addicted to show just how much they love you?</i>\"");
	 	outputText("\n\nMarble swallows before saying, \"<i>Do you have any idea of what [name] is trying to do?  " + player.mf("He","She") + " is trying to defeat the demons!  To stop the corruption for good!</i>\"");
	 	...
	 	outputText("\n\n\"<i>Seriously, there is no damn way someone as pathetic as [name] could accomplish that,</i>\" she continues while struggling to her feet.  \"<i>You need to be like a twenty foot giant, champion fighter, god master of magic to pull a stunt like wiping out the demons.  [name] is none of those things.  Do you really think someone who could do that would actually care about a lowly Lacta Bovine like you?</i>\"  Clara pokes her finger into Marble's chest.  \"<i>I guess the saying among our kind is right, 'the first daughter has the sourest milk.'</i>\"");
	...
	public function pcAndMurbleDoubleTeamCowButt():void {
		...
		if (player.cockThatFits(56) >= 0 && flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
			addButton(0, "DoubleTeam", threesomeDoubleTeam).hint("Use both your dicks to fuck Clara");
		if(player.cockThatFits(56) >= 0)
			addButton(1, "FuckHer", threesomeFuckHer).hint("Use your cock, Marble uses vagina");
		if(flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
			addButton(2, "LetMarble", threesomeLetMarble).hint("Let Marble do everything herself");
		addButton(3, "Lesbo", threesomeLesbo).hint("Engage in lesbo fun");
	}
	
	public function threesomeDoubleTeam():void {
		var x:int = player.cockThatFits(56);
		clearOutput();
	...
	public function NTRIsClearlyTheWorstFetishWhyWouldYouWriteThisOMG():void {
		...
		menu();
		if (player.cockThatFits(marbleScene.marbleCuntCapacity()) >= 0)
			addButton(0, "Your Cock", ntrSceneWithArgs, 0);
		if (player.hasVagina() && flags[kFLAGS.MARBLE_DICK_TYPE] > 0)
			addButton(1, "Marble's Cock", ntrSceneWithArgs, 1);
		if (player.hasVagina())
			addButton(2, "Vaginas", ntrSceneWithArgs, 2);
		//backup
	...
	public function ntrSceneWithArgs(type:int):void {
	...
	public function futaMarbleIsAHugeCowToCowCuntAndStuffsCowCuntsCuntFullOfCowCock():void
	{
		clearOutput();
	 	...
	 	outputText("\n\nA loud, \"<i>MOOOO!</i>\" abruptly rises in crescendo out of the random moans, gasps, and gibbering mess that use to be Clara.  However, her sister doesn't stop with that orgasm and continues to pound away, bringing back the babbling wreck for a few more moments.  Then, Marble's own orgasm rises out in her characteristic gasp and long low moan.  A much more subdued moo than the younger sister's.");
		...
		//round two!
		menu();
		addButton(0, "Next", pcAndMurbleDoubleTeamCowButt);
	}
	...
	public function dealingWithCowCuntPostPurification():void
	{
        ...
        spriteSelect(SpriteDb.s_clara);
	 	...
	 	outputText("\n\nShe is still laying on the ground, tied up next to the firepit.  Fittingly, a couple of discarded cups lay next to her, and her tea pot lays overturned at her feet.  Your return does not go unnoticed.  \"<i>Finally remembered me, huh?  You damn evil people, do you have any idea what kind of person I am?</i>\"");
	...
	public function letGoOfMarblesCowCuntSister():void
	{
		clearOutput();
	 	...
	 	outputText("\n\nYou and Marble untie Clara from her bonds, give her stuff to her, and escort her out of the camp.  She doesn't even look at either of you until she is at the edge and you tell her to leave and never come back.  She gives you one look, a look of pure rage and anger, of one who is swearing vengeance, then as soon as it started, it is over.  She steps out of the camp, and warps away into the world.");
	...
	public function nursingFromPureMarble():void
	{
		...
		{
		 	outputText("Feeling a bit thirsty, you ask Marble if there's any chance she has some milk for you.  She smiles at you and tells you she was worried you'd never ask, then leads you to a familiar, secluded part of the rocks around the camp.");
    ...
    */
	...
	public function pureMarbleDecidesToBeLessOfABitch():void
	{
        ...
        if (player.cor <= 40)
            ...
            outputText("I'm... sorry. I'm still afraid of how this taint will affect you, but... you didn't leave me after you'd got rid of the addiction, and even helped me to free myself from all of my corruption. I shouldn't have left you alone, too. And I can't live without you, [name].");
