...
public function encounterKiha():void {
	...
	if((player.cor < (33 + player.corruptionTolerance) || player.hasPerk(PerkLib.FalseLight)) && flags[kFLAGS.KIHA_CORRUPTION_BITCH] == 1) {
	...
	//PLOT FIGHT TIME!
	if(((player.cor < 33 + (player.corruptionTolerance)) || player.hasPerk(PerkLib.FalseLight)) && flags[kFLAGS.KIHA_AFFECTION_LEVEL] == 0 && flags[kFLAGS.KIHA_TALK_STAGE] >= 3 && player.gender > 0) {
...
internal function kihaVictoryIntroduction():void {
	...
	if(!mocking) {
		if ((player.cor <= (33 + player.corruptionTolerance)) || (sceneHunter.other && player.cor < (33 + player.corruptionTolerance)) || player.hasPerk(PerkLib.FalseLight)) { //Conversation Only - Emotional Rape! (40- Corruption!)
