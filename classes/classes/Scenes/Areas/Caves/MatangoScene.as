...
public function losingToMantango():void {
	...
	if (rand(4) == 0) {
		...
		if (player.hasVagina() && player.vaginaType() != VaginaClass.SHROOM) transformations.VaginaShroom().applyEffect();
		if (player.clitLength < 10) player.clitLength = 10;
		if (player.hasCock()) transformations.CockShroom().applyEffect();
