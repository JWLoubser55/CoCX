...
public function encounterKiha():void {
	...
	if((player.cor < 66 + player.corruptionTolerance || player.hasPerk(PerkLib.FalseLight)) && flags[kFLAGS.KIHA_CORRUPTION_BITCH] == 1) {
	...
	//PLOT FIGHT TIME!
	if(((player.cor < 66 + player.corruptionTolerance) || player.hasPerk(PerkLib.FalseLight)) && flags[kFLAGS.KIHA_AFFECTION_LEVEL] == 0 && flags[kFLAGS.KIHA_TALK_STAGE] >= 3 && player.gender > 0) {
...
internal function meetKihaAndFight():void {
	clearOutput();
	outputText("You step closer and proclaim that you go where you please.  Kiha snorts and says, \"<i>Cute.  Sadly, misplaced confidence will only make this hurt that much more.</i>\"");
	//(START COMBAT)
	startCombat(new Kiha());
}
//[Ask Why]
...
private function tellKihaTributeWasYourIdea():void {
	...
	outputText("The dragon-girl yanks her axe from her back and twists her hands, making the edge glow orange in the murky swamp air.  \"<i>You should've kept your mouth shut, worm.  The offer is off the table now!  The only thing I'll trade you is pain!</i>\"");
	startCombat(new Kiha());
}
...
internal function kihaVictoryIntroduction():void {
	...
	if (player.lust < 33) {
		outputText("\n\nSadly, you're not aroused enough to fuck her.");
	} else {
		menu();
		...
		addButton(2, "FuckHerPussy", victoryDickKiha).hint("Penetrate Kiha vaginally.")
			.disableIf(player.cockThatFits(monster.vaginalCapacity()) < 0, "Req. a cock fitting "+monster.vaginalCapacity()+" area!");
	...
	if(!mocking) {
		if (player.cor <= 40 + player.corruptionTolerance || sceneHunter.other && player.cor < 66 + player.corruptionTolerance || player.hasPerk(PerkLib.FalseLight)) { //Conversation Only - Emotional Rape! (40- Corruption!)
...
internal function kihaLossIntro():void {
	...
	if (!(mocking || monster.lust >= 50 || sceneHunter.other && player.lib >= 80 || player.biggestLactation() >= 2)) {
		...
		cleanupAfterCombat();
	} else if (player.biggestLactation() >= 2 && (!useMenu || player.isGenderless())) kihaMilkTitHumiliation();
	else if (player.isGenderless()) kihaGenderlessBeating();
	else if (!useMenu) {
		if (player.gender == 1) kihaRapesMen();
			...
			[0, "Dick", kihaRapesMen, "Req. a cock.", player.hasCock()],
			[1, "Vagina", kihaRapesLittleGirlsISawItOnTheNews, "Req. a vagina.", player.hasVagina()],
			[2, "Both", kihaRapesHerms, "Req. to be a herm.", player.isHerm()],
			[3, "Breasts!", kihaMilkTitHumiliation, "Req. to have higher levels of lactation.", player.biggestLactation() >= 2],
		],
		"Seems like the dragoness warrior is a bit aroused by your actions and won't let you go just now... maybe you can just submit to her and beg to use something specific?\n\n"
	);
}
...
private function kihaMilkTitHumiliation():void {
	...
	outputText("The scaly woman leans down to tug on your " + nippleDescript(0) + "s in turn, laughing every time a spurt of milk erupts from one of your tits.  Each rough squeeze sends shivers of erotic sensation through your body, and you quickly grow aroused from the rough treatment.  Kiha keeps going, ");
	if(player.hasCock()) outputText("not noticing the stiffening in your loins");
	else if(player.hasVagina()) outputText("not noticing the scent of your growing need");
	else outputText("not noticing the flush of your skin");
	...
	outputText("You trudge back to camp, replaying the erotic torture over and over in your over-sexed mind.  You just can't shake it, and once you get safe, you wind up spending hours masturbating, mooing over and over while you play with your body and further abuse your milky tits.  ");
...
private function kihaRapesMen():void {
	...
	if(x < 0) x = 0;
	sharedRapeIntro();
	if (!(player.isNaga() || player.tailType == Tail.LIZARD)) sceneHunter.print("You could have more fun if you had a naga/lizard tail.")
	outputText("You try to rise, but the dragon girl whips around, her long, thick tail delivering a subduing crack against the side of your head, dropping you back onto your [butt].  Triumphantly standing over you, the scaled woman eyes you with a panting breathlessness that betrays the coldness in her gaze.  She squints, arcing an imperious eyebrow as she looks you up and down, drawing her hands back to shoulder level and barely brushing against her breasts.  Her tail lashes back and forth between her legs, wrapping around her calves as she lifts a clawed foot and rests it lightly upon your stiffening groin.  Apparently, your fight left her wanting more than just your gems.  Her flexible toes squeeze your [cocks] through your [armor], and she snorts in derision.  \"<i>This is it, huh?  Pathetic.  I'm going to have to do all the work if I have any hope of you satisfying me.</i>\"\n\n");
	...
	if(player.hasBalls()) outputText(", resting the heel of her foot against the swell of your [balls] and applying just enough pressure to feel the cum churning within");
	...
	if(player.hasBalls()) outputText(", your [balls] practically leaping in over-burdened joy as their heavy load rushes through your flesh");
...
private function kihaRapesHerms():void {
	...
	outputText("The heat of her closeness and probing tail begin to excite you, [cocks] slowly stiffening against your abdomen.  Noticing your arousal, the dragoness pulls her tail from your mouth and curls it back behind her again.  \"<i>You'd love to stuff my tight box with every inch you've got, wouldn't you?</i>\" she taunts, pressing her hands to the ground behind her. Leaning back, she derisively stares down the bridge of her nose and curls her upper lip to display her jagged fangs.  ");
	//NAGA OR SLIME:
	if(player.isNaga() || player.isGoo()) outputText("Sliding backward slightly");
	//LEGS:
	else outputText("Hooking a leg under one of yours");
	outputText(", Kiha repositions herself so that her ashen-hued vulva presses moistly against your " + vaginaDescript(0) + ", the plush lips of her nethers lapping at yours in a torrid embrace.  Rocking against you in an angular motion, the dragon's sweltering sex grinds against your own sensitive flesh, small shocks of overstimulation arcing through your hips as her stiff, pink pearl brushes against your " + clitDescript() + ".  You mirror her motions, rubbing into the girl with increasing urgency as your flesh engorges, nearly forgetting the throbbing imperative of your [cocks].\n\n");
	...
	if(player.hasItem(consumables.INCUBID) || player.hasItem(consumables.P_DRAFT)) {
		...
		else player.consumeItem(consumables.INCUBID);
		//[Incubus Draft cont.:
...
private function tsundereMasturbationChristmasCarol():void {
	...
	else outputText("distinct lack of genitals");
	outputText(".\n\n");
	...
	//If Vagina
	else if(player.hasVagina()) outputText("\"<i>What do you think you're going to do with that pathetic, dripping pussy, huh?  I swear, if you make me l-lick that thing...</i>\"");
	//If Genderless
	...
	if(player.hasCock()) {
		outputText(multiCockDescriptLight() + " against her soft flesh, ");
	...
	else outputText(nippleDescript(0) + "s against the hard ridges of her back, your tongue softly licking her smooth skin.");
...
private function kihaVictoryPomfTail():void {
	...
	outputText("The gear lands behind you in a bed of moss");
	if(silly()) outputText(" with a pomf =3");
	else outputText(".");
	outputText("  Now nude, you sway your " + hipDescript() + " back and forth sensually, forcing Kiha to watch your body before you use her.  ");
	if(player.biggestTitSize() <= 4) outputText("She looks away and blushes as she says, \"<i>It's not like I'd find such tiny breasts attractive!</i>\"  Her mannerisms seem to indicate that she does.");
	else if(player.biggestTitSize() <= 12) outputText("She sighs and says, \"<i>As if I'd be impressed by udders like those!</i>\"");
	else outputText("She laughs and taunts, \"<i>How do you even walk with ridiculous hooters like those?  Go on, moo, cow!</i>\"");
	...
	outputText("Kiha's eyes well up with tears while her eyebrows stay locked in a perpetually angry glare.  At the same time, you begin to hear fluids dripping down to the ground.  You surreptitiously glance below and confirm that your initial thought was correct - she's gotten much wetter after having to suck on your pussy-");
	...
	if(player.wetness() <= 3) outputText("grows wet");
	else if(player.wetness() <= 4) outputText("quickly grows slimy");
	else outputText("immediately becomes soaked from your constant drippings");
	outputText(".\n\n");

	outputText("Satisfied that it is sufficiently lubricated, you squeeze the now-slippery tail and point its nubby tip at your entrance.  It bumps your " + clitDescript() + " as you wrangle with the struggling hind-limb, only turning you on all the more.  Feeling the tip circling your tunnel's opening, you push it up and seat the tentacle-like tail deep inside yourself.  ");
	player.cuntChange(1,true,false,true);
	outputText("Gods she's so... warm, and the ridges, they're stroking every inch of your interior, even your g-spot!  Your [legs] wobble unsteadily, but spurred by lust, you keep an iron grip on your new-found toy.  Kiha doesn't stop resisting either.\n\n");
	...
	else if(player.wetness() < 4) outputText("Leaking");
	else if(player.wetness() < 5) outputText("Gushing");
...
private function victoryDickKiha():void {
	...
	var y:int = player.cockThatFits2(monster.analCapacity());
	clearOutput();
	...
	outputText("Pausing for a moment, you set to removing your [armor] and unleashing " + sMultiCockDesc() + " ");
	if(player.hasVagina()) outputText("and " + vaginaDescript() + " ");
	...
	function multiF():void {
		outputText("That perfect, dripping entrance beckons for your " + cockDescript(x) + ", but your other penis");
		...
		var z:int = player.findCockWithTypeNotIn([x, y], CockTypesEnum.TENTACLE);
		if(player.isNaga() || player.tailType == Tail.LIZARD || z >= 0) {
			...
			outputText("\n\n");
		} else sceneHunter.print("Check failed: Naga/Lizard tail OR tentacock!");
		...
		if(player.cockTotal() > 2) {
			outputText("  You even coat her belly and tits with seed thanks to your extra penis");
			if(player.cockTotal() > 3) outputText("es");
			outputText(".");
		}
		outputText("\n\n");
		...
		outputText("You sigh and pull back, noting that Kiha's orgasm still seems to be ongoing.  She doesn't even react to the sudden vacancies in her pussy and asshole.  Maybe this will teach her a lesson about underestimating her opponents, but you doubt it.  You get dressed and leave, smiling at the sound of lurid orgasmic panting behind you.");
		player.sexReward("vaginalFluids", "Dick");
	...
	function singleF():void {
		...
		outputText("You push yourself in all the way to the hilt and unload, firing jet after jet of thick cream into the dragon's dribbling pussy, only making it leak even more.  ");
		if(player.cumQ() >= 500) outputText("A chubby bulge appears on her taut, defined belly, the only visible sign of your copious cum production.  ");
...
private function kihaRapesLittleGirlsISawItOnTheNews():void {
	...
	outputText("You shudder in ecstasy tinged with fear and redouble your efforts, slurping down the now steadily flowing juices of your reptilian tormentor like a woman dying of thirst who is suddenly confronted with a fresh spring of water. With another moan, Kiha looses a gasp of heated air that nearly feels like agonizingly wonderful flames into your aching snatch and you nearly go over the edge, your juices evaporating into a lewd puff of lusty perfume that wafts into the dragoness' nostrils. \"<i>Yes, cum for me,</i>\" she coos, as she grinds her pussy against your [face], flooding your mouth and nose with her own smoldering fragrance.\n\n");

	outputText("Unable and unwilling to do anything but obey, you climax with a shuddering tensing of your muscles that vibrates the dragon lying atop you.");
	if(player.wetness() >= 5) outputText("  Your juices spray into her face like cum from a cock, your quantity of lubricants unable to resist cascading from the pressure of your orgasm.");
...
private function rapeKihaWithWORDS():void {
	...
	//Conversation Level 4: Requires Low Corruption, Repeating for now (Zed)
		...
		outputText("You sit down next to Kiha and start to talk again.  While she initially gives the barest hint of a smile, she soon begins to sulk.  No matter what you say or do you can't seem to bring her out of it.  Perhaps for now, there's simply no more to discuss with her.");
	}
...
private function analRapuzulaKiha():void {
	...
	var y:int = player.cockThatFits2(monster.analCapacity());
	clearOutput();
	if (y < 0) sceneHunter.print("Maybe you could use another fitting dick?");
	...
	if(player.cockArea(x) >= 20) outputText("You quickly shed your [armor], letting your [cocks] out to breathe in the warm, swampy air.  Her red eyes widen slightly, but she quickly regains her composure, suddenly all confidence despite her obvious loss.  \"<i>Oh? What do you think you're gonna do with THAT, huh?  Just ravish me like some kind of sex-crazed demon? Is that your genius plan?</i>\"\n\n");
	else outputText("You quickly shed your [armor], letting your [cocks] out to breathe in the warm, swampy air. Her red eyes widen slightly, but she quickly regains her composure, suddenly all confidence despite her obvious loss.  \"<i>Hah, look at that pathetic thing.  You might have gotten me a little hot, but I really doubt you could please anyone with that puny piece of equipment.</i>\"\n\n");
	...
	outputText("As soon as your fingers touch her skin, her tail whips around your [cocks], wrapping up your groin");
	if(player.hasBalls()) outputText(" and threatening to crush your " + ballsDescriptLight());
	...
	if(player.cor < 33) outputText("You move your hands from her two mounds and down her sides, resting just above her hips. Hoping that this will work, you begin to tickle the stubborn girl.  ");
	else if(player.cor < 66) outputText("Since she seems to be concentrating on your hands, you frantically try to think of a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and nibble on the tip of her ear.  ");
	else outputText("Since she seems to be concentrating on your hands at the moment, your mind races trying to find a solution that doesn't involve them.  An idea forms and, deciding it's now or never, you lean forward and slowly slide your tongue across her cheek.  ");
	...
	function multiF():void {
		...
		else if(player.hasVagina()) outputText("; your " + vaginaDescript() + " pulses with pleasure, soaking your thighs with girl-cum as your cocks unload");
		outputText(".\n\n");

		outputText("The dragoness slumps and falls to the ground, drawing your spent shaft from her ass's depths.  It appears that she has fainted from overwhelming pain, pleasure, and exhaustion. Deciding that you're done here, you retrieve your [armor] and leave the girl in a mixed puddle of your cum and hers.  Perhaps now she'll treat you with a little more respect.  You return to camp.");
		player.sexReward("Default", "Dick",true,false);
		cleanupAfterCombat();
	}
	function singleF():void {
		...
		outputText("Grunting, you thrust one last time and spill your seed into her, filling her rectum with sticky cum.  ");
		if(player.hasVagina()) outputText("Your " + vaginaDescript() + " pulses with pleasure, but it's an empty orgasm, leaving you wishing you had something to fill that aching void.  Still, you cannot help but marvel at the superior feeling of cumming with a multitude of sexual organs.  ");
