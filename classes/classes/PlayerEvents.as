...
public class PlayerEvents extends BaseContent implements TimeAwareInterface {
		...
		private function hourlyTimersProc():Boolean {
			...
			needNext = player.gainPerk(PerkLib.ImprovedVenomGland, flags[kFLAGS.VENOM_TIMES_USED] >= 50, "You feel wonderfully healthy. After using your venom so many times your body finally got acclimated to the presence of your venom gland allowing for increased capacity and production.") || needNext;
			needNext = player.gainPerk(PerkLib.ImprovedVenomGlandEx, flags[kFLAGS.VENOM_TIMES_USED] >= 125, "You feel wonderfully healthy. After using your venom so many times your venom gland development reached it next stage. Allowing for increased capacity, production and lowering usage of venom.") || needNext;
			needNext = player.gainPerk(PerkLib.ImprovedVenomGlandSu, flags[kFLAGS.VENOM_TIMES_USED] >= 375, "You feel wonderfully healthy. After using your venom so many times your venom gland started to produce more potent venom. Allowing for increased capacity, production and increased effects of venom.") || needNext;

			needNext = player.gainPerk(PerkLib.GeneticMemory, flags[kFLAGS.TIMES_TRANSFORMED] >= 25, "Your body behave weirdly as if all the transformation, which you have undergone started to make it unsure about what it truly is. Sometimes you even try to move limbs that are no longer there. Suddenly you realise that no matter how many time you change your body remembers it. Your body developed genetic memory!") || needNext;
			needNext = player.gainPerk(PerkLib.TransformationResistance, flags[kFLAGS.TIMES_TRANSFORMED] >= 100, "You feel a strange tingling sensation. It seems as if you've finally adapted to the transformative properties of the food in Mareth and your body has finally built up enough resistance! You suspect that you can still transform but at somewhat diminished rate.") || needNext;
			needNext = player.gainPerk(PerkLib.TransformationAcclimation, flags[kFLAGS.TIMES_TRANSFORMED] >= 200, "Due to you're continued consumption of transformative items, you have discovered a way to draw out more of the transformative properties of the food in Mareth.") || needNext;
		...
		private function hourlyCheckRacialPerks():Boolean {
			...
			if (player.hasPerk(PerkLib.Ethereal) && player.armor.name != "nothing" && (player.armorPerk == "Medium" || player.isInHeavyArmor() || player.isInAyoArmor)) {
			...
			needNext = player.gainOrLosePerk(PerkLib.EasterBunnyBalls, player.isRaceCached(Races.EASTERBUNNY) || player.perkv1(IMutationsLib.EasterBunnyEggBagIM) >= 1, "", "Something changes in your balls you can feel them as if they stopped growing. Guess you're no longer enough of a easter bunny to produce eggs.") || needNext;
			...
			//Blood Affinity
			needNext = player.gainOrLosePerk(PerkLib.BloodAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.WERESPIDER, Races.DRACULA), "Your ability to hone the power of the red, flowing essence heightens. Hemophilia is a delightful chance for you to revel in the hemorrhage of freshly spilt blood.", "The sight of blood no longer fuels you with the same rousing sensations you once had. Any affinity you carried for the crimson fluid is now lost.") || needNext;
			//Dark Affinity
			needNext = player.gainOrLosePerk(PerkLib.DarknessAffinity, player.isAnyRaceCached(Races.VAMPIRE, Races.APOPHIS, Races.ANUBIS, Races.DRACULA, Races.LICH, Races.BANSHEE) || player.hasPerk(PerkLib.Nightshade) || (player.hasMutation(IMutationsLib.UndeadMetabolismIM) && player.perkv1(IMutationsLib.UndeadMetabolismIM) >= 3), "You are at home while shrouded in darkness, seeing clearly within the shadows as if it were broad daylight.", "The inky black darkness grows hostile toward you once more as your affinity for it is lost.") || needNext;
			//Acid Affinity
			needNext = player.gainOrLosePerk(PerkLib.AcidAffinity, player.isAnyRaceCached(Races.CAVEWYRM, Races.HYDRA, Races.CAVESTALKER, Races.SLIME, Races.SANDWORM) || player.hasPerk(PerkLib.ExanimationIII),
					"You retch violently suddenly feeling as if about to vomit before a flow of smoking green digestive fluid gushes out of your mouth drenching the ground. You watch in amazement as grass and other foliage matter are swiftly dissolved by your smoking fluids. Looks like you gained the ability to produce strong acid naturally, this is going to take some time getting used to. Coincidently you are resilient to acid now.",
					"You feel a fundamental change in your guts as your stomach which formerly produced deadly acids calms down the fluid now back to those equivalent of an ordinary human.") || needNext;
			//Compelling Aria
			...
			needNext = player.gainOrLosePerk(PerkLib.PlantKnowledge, player.isAlraune(), "It would seem you acquired additional skills in herbalism thanks to being a plant yourself.", "No longer a plant you lost some of your innate floral knowledge.") || needNext;
			//Kamaitachi perks
			needNext = player.gainOrLosePerk(PerkLib.CursedWound, player.arms.type == Arms.KAMAITACHI, "Your kamaitachi blades shines with a sharp, deadly glow. Those will clearly leaves deadly wounds.", "Lacking a pair of kamaitachi scythe you can no longer inflict your trademark cursed kamaitachi wounds.") || needNext;
			needNext = player.gainOrLosePerk(PerkLib.NaturalHerbalism, player.isRaceCached(Races.KAMAITACHI), "Great knowledges flows through your mind as you become more Kamaitachi like. It dawns on you that you have acquired a natural affinity for medicine and herbalism, something your species is famous for, heck you can identify every single plant near your camp by name and species now.", "No longer a Kamaitachi, you seem to have lost your knack for herbs and medicines.") || needNext;
			//Aquatic Affinity
			needNext = player.gainOrLosePerk(PerkLib.AquaticAffinity, (InCollection(player.lowerBody,LowerBody.ORCA,LowerBody.SEA_DRAGON) && InCollection(player.arms.type,Arms.ORCA,Arms.SEA_DRAGON) && player.tailType == Tail.ORCA && InCollection(player.ears.type,Ears.ORCA,Ears.ORCA)) || player.isScylla() || player.isKraken() || InCollection(player.tailType,Tail.ARIGEAN_GREEN,Tail.ARIGEAN_RED,Tail.ARIGEAN_YELLOW,Tail.ARIGEAN_PRINCESS),
					...
					"As you lose the respiratory organ to breath underwater it also becomes obvious that you will drown if attempting to breath water in. You will need to get items or transform to breath underwater again."+(player.rearBody.type == RearBody.ORCA_BLOWHOLE ? "\nIt dawns on you that you didn't breath for a full hour. When you realize this you relax your blowhole and take in some air. Well,wow it seems you can now hold in your oxygen for very lengthy period. This will be perfect for underwater explorations.":"")) || needNext;
			//Salamander bullshit
			...
			//}
			needNext = player.gainOrLosePerk(PerkLib.BigHandAndFeet, player.lowerBody == LowerBody.YETI && player.arms.type == Arms.YETI, "Geeze, with those oversized hands and feet you should just drop your weapon and punch things out cold. Why do you even need a weapon for anyway?", "Your limbs are now back to their normal size, it might be smart to use a weapon over your bare hands and feet for fighting.") || needNext;
			//Gazer perks
			needNext = player.gainOrLosePerk(PerkLib.TrueSeeing, player.eyes.type == Eyes.MONOEYE || player.hasMutation(IMutationsLib.GazerEyesIM), "As part of acquiring an all seeing eye your sight has improved greatly. It would seem you now possess the innate ability to dismiss illusions and falsehood, seeing the world as it truly is.", "Your sight is not what it used to be. No longer having a cyclopean vision you have lost the power of true sight.") || needNext;
			...
			//Elven Sense
			needNext = player.losePerk(PerkLib.ElvenSense, (player.eyes.type != Eyes.ELF || player.ears.type != Ears.ELVEN) && player.hasPerk(PerkLib.ElvenSense) && !player.perkv1(IMutationsLib.ElvishPeripheralNervSysIM) >= 1 && !player.hasPerk(PerkLib.BlessingOfTheAncestorTree), "You feels yourself less aware of your surroundings. Heck your vision seems less keen then it used to be. Most likely because you no longer possess the senses of an elf.") || needNext;
			...
			//Flawless Body
			else if (player.hasPerk(PerkLib.FlawlessBody) && ((player.lowerBody != LowerBody.ELF && player.lowerBody != LowerBody.SPECTRAL_SHROUD) || (player.arms.type != Arms.ELF && player.arms.type != Arms.SPECTRAL_CLAWS) || !player.hasPlainSkinOnly() || player.skinAdj != "flawless")) {
			...
			//Wild Drake
			needNext = player.gainOrLosePerk(PerkLib.WildDrake, player.isAnyRaceCached(Races.DRAGON, Races.FAERIEDRAGON, Races.YGGDRASIL, Races.FROSTWYRM, Races.JABBERWOCKY, Races.SEA_DRAGON, Races.VOUIVRE, Races.DRAGONNE), "Your draconic blood boil with contained power as your ability to unleash breath attacks increase to match that of a great drake.", "As your body becomes less draconic you feel the might of the drakes within you wanes.") || needNext;
			//Hinezumi Coat
			...
			//ABOBA
			needNext = player.gainPerk(PerkLib.BasiliskResistance, flags[kFLAGS.BASILISK_RESISTANCE_TRACKER] >= 100, (player.perkv1(IMutationsLib.GorgonEyesIM) >= 1)? "You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks! ":"You notice that you feel a bit stiff and your skin is a bit harder.  Something clicks in your mind as you finally unlock the potential to protect yourself from the goddamn basilisks!") || needNext;
			needNext = player.losePerk(PerkLib.EnlightenedNinetails, player.perkv4(PerkLib.EnlightenedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.") || needNext;
			needNext = player.losePerk(PerkLib.CorruptedNinetails, player.perkv4(PerkLib.CorruptedNinetails) == 0 && (player.tailType != Tail.FOX || player.tailCount < 9), "Without your tails, the magic power they once granted withers and dies, vanishing completely.") || needNext;
			needNext = player.gainOrLosePerk(PerkLib.NinetailsKitsuneOfBalance, player.hasPerk(PerkLib.EnlightenedNinetails) && player.perkv4(PerkLib.EnlightenedNinetails) > 0 && player.hasPerk(PerkLib.CorruptedNinetails) && player.perkv4(PerkLib.CorruptedNinetails) > 0 && player.tailType == Tail.FOX && player.tailCount == 9,
					...
					"Without your tails, the balance is disturbed and you lose your insights into the third path.", player.perkv4(PerkLib.NinetailsKitsuneOfBalance) == 0) || needNext;
			needNext = player.losePerk(PerkLib.Diapause, !player.hasVagina(), "With the loss of your womb, you lose your kangaroo-like diapause ability.") || needNext;
			//Small Frame perk
			needNext = player.gainOrLosePerk(PerkLib.SmallFrame, player.isAnyRaceCached(Races.IMP, Races.MOUSE, Races.DEVIL, Races.AZAZEL) && player.tallness <= 72, "Due to your small build you have become harder to hit.", "Due to being too tall you no longer qualify.") || needNext;
			//Small Caster perk
			needNext = player.gainOrLosePerk(PerkLib.SmallCaster, player.isAnyRaceCached(Races.IMP, Races.DEVIL, Races.AZAZEL) && player.tallness <= 72, "Your magic becomes more concentrated in your smaller body.", "Your racial alignment does not meet the conditions and as such, the high concentration of mana in your body disperses.") || needNext;
			needNext = player.gainOrLosePerk(PerkLib.Immortality, player.isAnyRaceCached(Races.AZAZEL, Races.LICH), "You gain a sense of invulnerability as you are now an Azazel.", "You lose your sense of invulnerability as you are no longer an Azazel.") || needNext;
			needNext = player.gainOrLosePerk(PerkLib.Purifier, player.isRaceCached(Races.AZAZEL), "You gain a sense of conviction as you are now an Azazel.", "You lose your sense of conviction as you are no longer an Azazel.") || needNext;
			...
			if (player.isAnyRaceCached(Races.BANSHEE, Races.POLTERGEIST) && !player.hasPerk(PerkLib.Ethereal)) {
				if (player.isRaceCached(Races.BANSHEE)) {
					outputText("\nAs you become more and more ghostlike your relationship with life lessens. Your body has become closer to the composition of an immaterial unbound soul. You start to fade into an ethereal state but never vanish completely, stuck between the metaphysical realm and sustained by your strong soul. While you can still manifest and interact with physical objects to an extent, doing such requires a sustained effort from your soul. This strange state of existence ");
