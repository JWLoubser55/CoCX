...
public class KihaFollower extends NPCAwareContent implements TimeAwareInterface, SaveableState {
    ...
    public static var KihaTownState:int; //0 for unseen, 1 for seen
    ...
    public static var Lily3SomeState:int; //0 for no, 1 for yes, 2 for disabled forever //TODO: unused?
    //
    public static var TalkedAfterLethice:Boolean;
    public static var CanVisitKids:Boolean;
    ...
    public function resetState():void {
        DergKidnapped = 0;
        KihaTownState = 0;
        FlameSpreaderKillCount = 0;
        FlameSpreaderBossKilled = false;
        ProposalStatus = 0;
        TyrantiaRelationshipStatus = 0;
        SidonieSeen = false;
        BelisaRelationshipStatus = 0;
        //
        TalkedAfterLethice = false;
        CanVisitKids = false;
    ...
    public function saveToObject():Object {
        return {
            "DergKidnapped": DergKidnapped,
            "KihaTownState": KihaTownState,
            "FlameSpreaderKillCount": FlameSpreaderKillCount,
            "FlameSpreaderBossKilled": FlameSpreaderBossKilled,
            "ProposalStatus": ProposalStatus,
            "TyrantiaRelationshipStatus": TyrantiaRelationshipStatus,
            "SidonieSeen": SidonieSeen,
            "BelisaRelationshipStatus": BelisaRelationshipStatus,
            "TalkedAfterLethice": TalkedAfterLethice,
    ...
    public function loadFromObject(o:Object, ignoreErrors:Boolean):void {
        if (o) {
            DergKidnapped = o["DergKidnapped"];
            KihaTownState = o["KihaTownState"];
            FlameSpreaderKillCount = o["FlameSpreaderKillCount"];
            FlameSpreaderBossKilled = o["FlameSpreaderBossKilled"];
            ...
            CanVisitKids = "CanVisitKids" in o ? o["CanVisitKids"] : false;
    ...
    override public function followerKiha():Boolean {
        return flags[kFLAGS.KIHA_FOLLOWER] > 0 && flags[kFLAGS.KIHA_CORRUPTION_BITCH] != 1;
    }
    ...
    internal function kihaFriendlyGreeting(output:Boolean = true):void {
        ...
        if (output && flags[kFLAGS.KIHA_TALK_STAGE] == 6 && (player.cor <= 30 + player.corruptionTolerance || sceneHunter.other && player.cor < 66 + player.corruptionTolerance)) {
            ...
            outputText("[pg]You are.  She is.  All of you - anyone who hasn't given in to corruption.  Anyone with the will and the strength to fight back.  You tell her of everyone you've met on your travels, the few stalwart souls that still resist Lethice's hordes.");
    ...
    private function jumpDaBitches():void {
        ...
        outputText("[pg]After what she just tried to pull, you tell her to sit down and shut up.  Behind you, Hel squirms around until you grab her arm and drag her over to sit by Kiha.  \"<i>[name]!  I was trying to help you!</i>\"  You just scowl and grab both scaly girls by the hair, dragging them up to kneel in front of you.  They squirm and struggle in your grasp, until you give both of them a hand slap.  That shuts them up for the moment.");
        ...
        if (player.cockTotal() > 1) outputText("[pg]Deciding to use Hel's mouth a bit more, you grab your [cock 2] out of your armor and press its head against Hel's lips.  She starts to protest, but just like you did with Kiha, as soon as she opens her mouth to complain you ram your secondary cock in, burying its length in her face.  She gags and gasps, but you just grab both girls by the hair and start to fellate yourself with them, ramming their faces down your cocks until you're a spit-slicked mess, until both girls have become completely compliant, simply allowing you to use them.");
    ...
    public function kihaAdmitsSheLikesYourWang():void {
        clearOutput();
        if (flags[kFLAGS.KIHA_ADMITTED_WARM_FEELINZ] == 0 || recalling) {
            if (!recalling) flags[kFLAGS.KIHA_ADMITTED_WARM_FEELINZ] = 1;
    ...
    private function lovinHugKiha():void {
        ...
        if (!recalling) flags[kFLAGS.KIHA_AFFECTION_LEVEL] = 2;
        if (!recalling) flags[kFLAGS.KIHA_AFFECTION] = 0;
    ...
    //Required to clear the output properly if SH is disabled… clumsy, yeah. -SH
    private function lovingHugSelector():void {
        clearOutput();
        sceneHunter.selectGender(lovingHugDickings, lovingHugsGirlFuckSex, lovingHugsForRetards);
    }
...
private function warmLoverKihaIntro(output:Boolean = true):void {
	...
	if (followerKiha()) {
		if (output) {
            if (flags[kFLAGS.LETHICE_DEFEATED] && !TalkedAfterLethice) {
                KihaAfterLethice();
                return;
            } else if (emberScene.followerEmber() && !EmberInteractionHappened && rand(10) == 0) {
                emberInteraction();
                return;
			...
			else if (model.time.days % 15 == 0 && flags[kFLAGS.KIHA_PREGNANCY_POTENTIAL] == 0) kihasFirstClutch();
		...
		addButton(8, "Dominate", dominateKiha)
			.hint("Try to turn the tables and dominate her instead! Will she agree, or… ?")
			.disableIf(player.isGenderless(), "Not for genderless!");
		if (sceneHunter.mockFights)
			addButton(9, "Mock-Rape", mockRapeKiha)
				.hint("Do things as in the old days and challenge her for a fight! The loser gets fucked!")
    ...
    private function hangOutWithKiha():void {
        menu();
        addButton(0, "Walk", walk);
        addButton(1, "Dinner", dinner);
        addButton(2, "LookAtSky", sky);
        ...
        function sky():void {
            ...
            if (!followerKiha()) {
                ...
                else outputText("night in the wastelands.  The stars are out, as they always are at night in your part of Mareth, a thousand of little pin-pricks in the heavens");
                outputText(".");
            }
    ...
    private function itsGood():void {
        clearOutput();
        outputText("You give Kiha a little wink and tell her it's great.  She breaks out into a big, dopey grin as you explain your delight at the fine, wood-smoked texture and delightful juiciness of the flash-cooked meat.  Kiha takes your compliments to heart, declaring, \"<i>O-of course it's good; just the fact that </i>I<i> cooked it ought to make it obvious!</i>\"");
    ...
    private function kihaSexMenu(allowBack:Boolean = true):void {
        menu();
        addButton(0, "Anal", savinTheAnalForKiha)
            .disableIf(player.cockThatFits(94) < 0, "You're too big to fuck her ass."
                + "\n\n<b>Req. a dick fitting 94 area.</b>");
        addButton(1, "Get HJ", kihaPlaysWithBigassCocksFemDomAhoy)
            .disableIf(!player.hasCock() || player.biggestCockArea() < 150,
                "Your penis isn't ridiculously large enough for her to take care of it with her hands and feet."
                + "\n\n<b>Req. a dick larger than 150 area.</b>");
        addButton(5, "Girl69", kihaGirlGirlSex).disableIf(!player.hasVagina(), "<b>Req. a vagina.</b>");
        //Follower-only scenes
        if (followerKiha()) {
            addButton(2, "FuckVag", fuckKihasVagInCamp)
                .disableIf(player.cockThatFits(67) < 0, "Kiha's vagina is too small and tight for you to take."
                    + "\n\n<b>Req. a dick fitting 67 area.</b>");
            //some shit for lusty fuck.
            var lustyReject:String = !player.hasCock() ? "You can't fuck her without a cock… " :
                player.smallestCockArea() >= 200 ? "Where are you going to put THAT?! You're just too big for her!" :
                    player.minLust() > 50 || player.lib > 80 ? "You have a hunch that if your dick were bigger you'd be able to really go town on her with your incredible libido." :
                        "If you had a big enough dick and a lust draft you could really go to town on her.";
            lustyReject += "\n\n<b>Requires a dick between 40 and 200 area. Also, lust draft OR high libibo / min. lust.</b>";
            addButton(3, "LustyFuck", boneTheShitOutofKihaHolesWithHorsecock)
                .disableIf(player.findCock(1, 40, 200) < 0
                    || player.minLust() < 50 && player.lib < 80 && !player.hasItem(consumables.L_DRAFT), lustyReject);
            addButton(4, "TentacleFuck", fuckKihaWithATentacle)
                .disableIf(player.countCocksWithType(CockTypesEnum.TENTACLE, 18, -1, "length") == 0,
                    "<b>Req. at least one tentacle cock longer than 18 inches.</b>");
            addButton(6, "Give I.Draft", giveKihaIncubusDraft)
                .hint("You could slip her an incubi draft and let her fuck your ass with it.")
                .disableIf(!player.hasItem(consumables.INCUBID) && !player.hasItem(consumables.P_DRAFT),
                    "If you had an incubi draft, you could have her grow a dick for you to take in the ass, at least for a while.");
            addButton(7, "GroPlusTits", ghostboobiesKiha).hint(player.hasPerk(PerkLib.Incorporeality) ?
                ...
                .disableIf(!player.hasItem(consumables.GROPLUS), "If you had some Gro+, you could ask her about making her breasts bigger.");
    ...
    private function boneTheShitOutofKihaHolesWithHorsecock():void {
        ...
        var x:Number = player.findCockWithType(CockTypesEnum.HORSE, 1, 40, 200);
        if (x < 0) {
            sceneHunter.print("A bit more details for horsecocks.");
            x = player.findCock(1, 40, 200);
        }
        var x1:Number = x + 1;
        ...
        else if (player.hasBalls()) outputText("Her other fondles your [balls], steadily increasing your desire for release.  ");
        ...
        if (player.cocks[x].cockType == CockTypesEnum.HORSE) outputText("her free hand eagerly caressing you inside your own sheath with inhuman warmth");
            ...
            if (player.hasBalls()) outputText("her tongue working as much of your shaft as possible with inhuman warmth.");
        ...
        if (player.hasBalls()) outputText("  Your [balls] smear and shudder against her chin, so tight, hot, and ready to explode.");
        outputText("[pg]Kiha ");
        if (player.hasBalls()) outputText("squeezes your [sack]");
        ...
        if (!player.isTaur()) outputText("You run a hand over her rump, stopping only to give it familiar slaps as x1");
        else outputText("Y");
    ...
    private function kihaPlaysWithBigassCocksFemDomAhoy():void {
        ...
        if (player.hasBalls()) outputText("Your balls clench and relax in time with the waves of pleasure, and y");
        else outputText("Y");
    ...
    private function kihaGirlGirlSex():void {
        ...
        outputText("[pg]Mouths stuffed in each other's twats, you lie with your lover, lazily lapping at fragrant girl-honey while your bodies shiver from aftershocks of bliss.  Kiha admits, \"<i>Okay, you're - lick - not too bad - lick - at this.</i>\"  You swat her rump and stroke her happily swaying tail before thanking her.");
        player.sexReward("vaginalFluids", "Lips");
    ...
    private function fuckKihasVagInCamp():void {
        ...
        if (!player.isScaleCovered()) outputText("leave long, red scratches on your back.");
    ...
    private function fuckKihaWithATentacle():void {
        ...
        for (var i:int = 1; i <= 4; ++i)
            x1[i] = player.findCockWithType(CockTypesEnum.TENTACLE, 1, 18, -1, "length") + 1;
        if (x1[4] <= 0) sceneHunter.print("You could use more.")
        ...
        if (player.countCocksWithType(CockTypesEnum.TENTACLE) > 1) outputText("those monstrous things");
        ...
        if (player.hasBalls()) outputText("churn and quiver");
    ...
    private function ghostboobiesKiha():void {
        ...
        if (!player.hasPerk(PerkLib.Incorporeality)) {
            outputText("[pg]Shaking your head in disappointment, you shuffle away, replacing the Gro+ in your pack with a sigh.  Maybe someday.");
            sceneHunter.print("What if you were able to ASSUME DIRECT CONTROL? Maybe… possess her, like a ghost?");
            doNext(playerMenu);
            return;
        }
    ...
    internal function kihaBitchesOutCorruptPCs():void {
        ...
        if (!followerKiha()) {
            ...
            simpleChoices("Fight", kihaScene.meetKihaAndFight, "", null, "", null, "", null, "Leave", leaveKihaAfterCorruptionBitch);
        } else {
    ...
    private function dominateKiha():void {
        clearOutput();
    ...
    //[Fight for position]
    private function fightForDominanceWithDragonCunnies(mock:Boolean = false):void {
        clearOutput();
        ...
        if (mock) outputText("Kiha flies up and slams down into the mossy earth with enough force to spray chunks of dirt everywhere.  She stands and growls, \"<i>Just because you beat me before doesn't mean you've defeated me.  I'll NEVER BE DEFEATED!</i>\"\n\n");
        else {
    ...
    internal function pcWinsDomFight():void {
        ...
        sceneHunter.print("I won't spoil the results - there's 2 parts of the scene, each randomized. Three endings for each.")
        ...
        if (select == 0 || !player.hasVagina()) {
            ...
            if (rand(2) == 0) outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and cum all over your face!  You'll lap it all up like a good little " + player.mf("boy", "girl") + " to bo- UGH!</i>\"");
            //Kiha response one (requires cock):
            else outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh… </i>\"");
        }
        //PC's vagina is chosen/ has vagina only:
            ...
            //Kiha response one (requires cock):
            if (player.hasCock() && rand(2) == 0) outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and make you beg to plant your seed in my womb!  Just wa-OOhhOhhh… </i>\"");
            //Kiha response one (requires vagina):
            else outputText("[pg]\"<i>Ha!  The Champion still has some fight in [him]!  Keep me here if you can, wimp!  I WILL pin you down and cum all over your face!  You'll lap it all up like a good little " + player.mf("boy", "girl") + " to bo- UGH!</i>\"");
        }
    ...
    //Mock-fight: repurposed alt dom-fight dialogues, leading to original rape!
    private function mockRapeKiha():void {
        clearOutput();
        ...
        outputText("[pg]You see what's going on: being nice about this isn't going to get you anywhere.  You need to show some dominance if you want her to listen.  \"<i>HEY!  LOOK AT ME WHEN I'M TALKING TO YOU!</i>\"  You snarl, advancing towards the stubborn woman.  Attentively, she returns your gaze.  \"<i>Yes, [name]?  What is it?</i>\"  She inquires, apparently having \"<i>forgotten</i>\" what you just asked her.  You sternly tell her that she needs to learn how to be a bottom in the relationship and take a good fuck from you sometimes, rather than expecting you to play along with her whims.  \"<i>Aw, that's cute; you're thinking you have any say in the matter,</i>\" she replies, facetiously.  This makes you angry.  You do have a say in the matter, you tell her, and she will listen.  Kiha grins. \"<i>You're all talk; just be a good " + player.mf("boy", "girl") + " and let me do my thing during sex.  Otherwise, I might have to hurt you.  I wouldn't want to hurt my </i>friend<i> now, would I?</i>\"");
        menu();
        addButton(0, "PleaseNo..", noRapePls);
        addButton(1, "FIGHT!!!", fightForDominanceWithDragonCunnies, true);
    }
        ...
        if (flags[kFLAGS.SPARRABLE_NPCS_TRAINING] == 2) {
            ...
            if (flags[kFLAGS.KIHA_LVL_UP] < 17 && flags[kFLAGS.KIHA_DEFEATS_COUNTER] >= flags[kFLAGS.KIHA_LVL_UP] + 7) {
                ...
                if (player.hasStatusEffect(StatusEffects.CampSparingNpcsTimers1))
                    ...
                    player.createStatusEffect(StatusEffects.CampSparingNpcsTimers1, 0, 0, addToV3, 0);
    ...
    public function KihaAfterLethice():void {
        ...
        outputText("You sigh, wrapping your arms around your dragoness lover and pulling her into you. She gasps, but doesn’t pull away. \"<i>Couldn’t get enough of me, could you?</i>\" She shakes herself. \"<i>Look… We don’t have time for this, not while Lethice rules Mareth.</i>\"\n\n");
        outputText("This almost gets a laugh from you, but you hold her tighter. You tell Kiha that if that’s the case, you have all the time on Mareth. \"<i>Wh-what?</i>\" Kiha sputters, fire singing your [hair]. \"<i>Don’t joke like that!</i>\" She pushes you away, and her eyes bore into you. \"<i>Don’t joke about stuff like that. It’s not funny.</i>\" She stands, looking at you with a combination of hurt and anger. She spreads her wings, crouching down as if to take off.\n\n");
        outputText("Before she can take off, you yell that you weren’t joking. She freezes, and at that moment, you describe Lethice’s fortress entrance, where the secret passage was. Kiha slowly turns around, her wings falling limp. You ask if she’d like to hear it all, and she closes her mouth, simply nodding once. You sit on a log by the campfire, and Kiha sits down beside you, leaning into you as you describe the inside of the fortress, the Basilisks, then the series of Lethice’s elites you tore through on your way to the keep. You describe Lethice to her, in all her profane glory, and how you put her down, not once, but twice, after she’d gorged herself on the souls of others.\n\n");
    ...
    public function KihaAfterLethiceSpeak():void {
        ...
        outputText("\"<i>I don’t know.</i>\" She whispers this, and you say nothing else, letting her bury her face into you.\n\n");
        menu();
    ...
    public function KihaProceed():void {
        clearOutput();
        outputText("\"<i>… I don’t know what to say.</i>\" Kiha says, pulling back from you. Her eyes shine with emotion, and more than a few tears trickle down her face. \"<i>I… Thought I’d be… </i>\" Kiha’s almost completely still, which is unlike the dragon-girl you’ve come to know. \"<i>I thought I’d be happier, [name].</i>\" She shudders, her voice low and shaky. \"<i>I… </i>\"\n\n");
        ...
        outputText("\"<i>Yes. I mean No! I mean… </i>\" Kiha growls, looking down at the ground again. You sigh, telling her that if she’s having trouble explaining it, she has the time to think. \"<i>… I honestly don’t know.</i>\" Kiha runs a single claw along your back as she speaks. \"<i>I used to think that having that bitch gone would make me happy. But now? I can’t… It just feels numb.</i>\" At the rather confused look on your face, she shivers. \"<i>Look… I trained my ass off for years. Learning how to use the powers they gave me, fighting, killing… There was nobody I could trust.</i>\" Kiha breathes. \"<i>And to have you just come back and tell me she’s gone? Like nothing happened?!</i>\" Kiha glares into your eyes, her face extremely close to yours.\n\n");
        outputText("\"<i>What if she’d taken you, idiot?! What if any one of those monsters had gotten a hold of you?</i>\" The fire comes back to Kiha’s eyes for a moment, but goes away just as quickly. \"<i>What do you think I’d have done if you never came back?</i>\" Kiha’s dusky cheeks are bright red, tears running down her face.\n\n");
        outputText("You put your palms on Kiha’s cheeks, closing your eyes and giving your dusky dragoness a smile. You gently remind Kiha that you were able to best her, many times. She huffs angrily at that, a puff of smoke escaping her lips. \"<i>Well, yes, but I-</i>\" You shush her again, putting a finger on her lips. You tell Kiha that she doesn’t need to impress you, or anyone. You tell her that Lethice is no longer a threat to Mareth, or to her.\n\n");
        outputText("\"<i>… You never really needed my protection on those dates, did you?</i>\" Kiha seems to be struggling with that, and you chuckle gently, kissing her nose. You tell her that you didn’t, not really, but that it was a thoughtful gesture. You ask her if it made her happy to ‘protect’ you, and she nods.\n\n");
        outputText("\"<i>… So what now?</i>\" Kiha seems lost, looking at you with confusion in her eyes. \"<i>If the bitch is dead… What do I do now?</i>\" You remind her that even with Lethice’s death, there are still other demons and imps running around. This seems to perk her up, and she gives you a slight smile.\n\n");
        outputText("\"<i>I can’t believe I tried to push you away.</i>\" Kiha whispers this, not realizing you can still hear her. You tell Kiha that it’s okay. You understand. She blinks, her dusky skin flushing with embarrassment. \"<i>I mean, I… I… You… </i>\" She seems to deflate. \"<i>Forget it.</i>\"\n\n");
    ...
    public function KihaTalkFamily():void {
        clearOutput();
        outputText("You ask Kiha if she’d ever thought about having kids. “<i>Before, or after the demons fucked my shit up?</i>” Kiha snorts. “<i>Honestly, not a lot of time to think about that, [name]</i>”.\n\n");
        outputText("Taking her hand, you tell her that you have thought about it. Kiha’s dusky cheeks flush, and you notice her knees turning inwards slightly.\n\n");
        outputText("“<i>O-oh… </i>” Kiha’s dusky cheeks flush bright red. “<i>W-well I’m not surprised, idiot. Of course, you’d want me.</i>” She seems happy with your forwardness. “<i>… I’m not sure if I want them or not… But I can’t think of anyone I’d rather have them with.</i>” Realising what she’s said, Kiha blushes, turning away. “<i>N-not that it’s saying much! I J-just don’t have anyone else… I… </i>” Kiha realises she’s making it worse. “<i>I-idiot! Just because I like you in bed, d-doesn’t… Mean… </i>” Kiha shudders, taking off before she embarasses herself further.\n\n");
    ...
    public function KihaTalkEmber():void {
        clearOutput();
        outputText("“<i>What about her?</i>”  Kiha asks. “<i>She’s… Look. She’s stuck up. Calls me" + (totalKihaChildren() > 0 ? " and the kids" : "") + " fake dragons. It’s really starting to piss me off.</i>” You wince, making a comment about draconic pride getting in the way of good things. Kiha blinks, giving you a suspicious look.");
        doNext(playerMenu);
    }

    public function KihaTalkTyrantia():void {
        clearOutput();
        outputText("\"<i>Oh, the giant spider girl.</i>\" Kiha folds her arms, sitting down. \"<i>… She’s just as much of an idiot as you sometimes. She’s massive, strong and tough. That armor isn’t for show… But underneath that, she’s a lot softer than she looks. She’s constantly looking for people to be around. It’s kind of sad, really.</i>\"\n\n");
        outputText("You remind Kiha that she decided to join you at the camp, so maybe she’s more like the giant spider-girl than she realizes. \"<i>… Idiot.</i>\" Kiha scoffs. \"<i>… She’s the kind of woman who clings to others because she can’t stand on her own.</i>\" Kiha’s turned her back on you, but you can see her wings shaking.  \"<i>… She’s a good person though, under all the armor. Brave and loyal. The fact that she isn't completely gone… Is a testement to her determination.</i>\"\n\n");
    ...
    public function KihaTalkDiva():void {
        clearOutput();
        outputText("\"<i>You know what’s worse than demons torturing you, invading your holes and turning you forcibly into a dragon? Listening to her trying to talk fancy. All the ‘thous’ and… Nobody talks like that anymore! We get it, you’re a fancy pants noble, you don’t need to act like the cheesiest stereotype possible!</i>\" Kiha tosses her hands into the air, flickers of fire pooling in her nostrils. \"<i>And then there’s the blood… </i>\" Kiha shudders. \"<i>Look, I eat raw meat sometimes, and even I think drinking blood out of a fancy glass in front of everyone is just… weird.</i>\" She gives you an odd look. \"<i>Anything else you wanted?</i>\"\n\n");
        doNext(playerMenu);
    }
    ...
    public function KihaProposal():void {
        ...
        outputText("\"<i>Wh-what, can’t I do something nice for you without it being a b-big thing?</i>\" Kiha hugs herself, tail wagging nervously, and you shake your head. You tell Kiha that she’s never done this before… But that you appreciate that she did. This gets an immediate reaction. She seems to lose a bit of her nervousness, and she smirks a little. \"<i>You’d better, doofus.</i>\" She saunters over to you, hips swaying, and leans in. She offers you her hand, and you take it. Kiha leans in, pressing her soft, dusky DDs against you. You lean in, exchanging a warm kiss. She pulls back after a few seconds, trailing a finger along your shoulder, before pointing at a wooden table set up behind where she’d been standing.\n\n");
        outputText("On the table is a home-cooked meal the likes of which you haven’t seen in a long time. Meat cut thinly into strips, marbled fat glistening in the torchlight, some sort of bluish tuber, a bottle of wine you recognize from Tel Adre, and fresh vegetables. You stare at the meal in surprise, and then back to Kiha. Her smile grows, and she wraps her tail around your leg. \"<i>What is it, my idiot?</i>\" Shaking yourself, you tell Kiha that you never expected her to cook something so… wholesome.\n\n");
        outputText("\"<i>You mean anything but flame-breath charred meat.</i>\" She chuckles at that, the laughter not quite reaching her eyes. \"<i>… I’ve cooked a meal before, [name]… But I haven’t really… had a reason to make the effort in a long time.</i>\" With that, she motions towards the food. \"<i>Come on, before it gets cold.</i>\"\n\n");
        ...
        outputText("\"<i>They’re called Etobar.</i>\" Kiha swallows a mouthful, washing it down with some wine. \"<i>They grow in the foothills, between the mountain and plains.</i>\" You nod, and begin asking her about the rest of the meal. Kiha’s tail wags a little as you ask, and she pours you another glass.\n\n");
        outputText("\"<i>Well, the meat came from the plains as well. The gnolls keep a herd of flightless birds under lock and key as a reserve. They don’t let outsiders get to them voluntarily, but… </i>\" Kiha gives you a little, wicked grin. \"<i>Those scavengers caused us enough problems back then. Now? I can wipe out an entire tribe.</i>\" At the look on your face, she chuckles. \"<i>Oh, I didn’t fight them, [name]. I didn’t need to. When I demanded one of their birds, they gave it up.</i>\" She puts a hand on yours.\n\n");
        outputText("\"<i>The tubers were from the caves. We almost never got them in the village, but when we did, it was a bidding war to bring those roots home.</i>\" You ask her about the butter, and Kiha just gives you a slightly nauseated look. You decide not to pursue (or think about) the dairy anymore. With a little time to think, you note that Kiha must have flown quite some time to find all these ingredients. \"<i>… It wouldn’t be as special if it wasn’t difficult.</i>\" Kiha says this simply, as if it’s obvious. \"<i>… I did have a little help, but… I wanted to make this special.</i>\"\n\n");
        ...
        outputText("\"<i>You say that like this is over, my idiot.</i>\" Kiha’s eyes are half-lidded, her black lips in a lazy smile. \"<i>Come on.</i>\" She stands, walking over to your side of the table. \"<i>We’ve got somewhere to be.</i>\"\n\n");
        //If player has large wings
        if (player.canFly() && player.hasPhysicalWings()) {
            outputText("Kiha bends her knees, taking off with a single flap. She turns her head around to watch you, and she beckons, spreading her legs enticingly before slamming them shut and flapping her wings again. \"<i>Come on!</i>\"\n\n");
            ...
            outputText("Kiha holds her arms out, as if for a hug. \"<i>Come here, my idiot.</i>\" Her voice is full of warmth, and you step in, hugging your dragoness tight. Her soft DDs press snugly against your [chest], and you can feel her nipples harden at your touch. Kiha’s tail wraps around your waist, and she whispers into your ear as you bring your lips to her neck. \"<i>Hold on tight.</i>\"\n\n");
            outputText("With almost no warning, Kiha spreads her wings, lifting you both into the sky. Instinctively, you hold on tighter, and she smiles, her arms under your shoulders. \"<i>My Idiot.</i>\" She carries you up, up, high into the clouds, and you can no longer see Mareth, or the moon… or anything, really, except for what’s right in front of you.\n\n");
            outputText("Your heart leaps into your throat as Kiha rests her head on your shoulder, wingbeats steady, and nibbles your ear. The air around you is cold, but her surprisingly soft touch easily keeps the cold at bay. ");
            if (player.hasCock()) outputText("Your [cock] hardens, pressing against Kiha’s thigh, ");
            if (player.hasVagina()) outputText((player.hasCock() ? "y" : "Y") + "ou begin to drool from your lower lips, ");
            outputText("the dangerous altitude somehow arousing when you’re cuddling with your dragoness. Kiha notices your arousal, sighing happily.\n\n");
            outputText("\"<i>My, you’re eager, aren’t you, my idiot?</i>\" Kiha’s voice is tender, but she chides you, putting a hand on your ass. \"<i>Do you want to have your way with me? To make me cum midair, dropping us to the ground together?</i>\" She shakes her head. \"<i>Hold on, my dumbass. I’ll take us down.</i>\" Kiha spreads her wings, holding you horizontally as she glides down. As you get closer to Mareth, you see the plains and swamp, the mountains… even a few bits of light that could be a town, or a city. Your awe silences you, and Kiha rubs your cheek with one hand.\n\n");
        ...
        outputText("Kiha snuggles in beside you, her head on your shoulder. Her dusky thighs rub against your [leg] and she sighs, letting a bit of flame run past your face. \"<i>You know where we are, lover?</i>\" You nod, telling Kiha that it’s where she used to live, before she moved in with you. This gets a nod and a happy rumble from Kiha, and she holds your arm in both of hers.\n\n");
        outputText("A few minutes pass, the two of you quiet, before Kiha speaks up again. \"<i>Do you remember what you told me, when I said that everything had gone to shit? I told you the entire world was being corrupted, that Lethice had already won… </i>\" Kiha looks up, into the night sky, and points at the stars. \"<i>You told me to look up there. That the demons couldn’t destroy the heavens.</i>\" She sighs. \"<i>I thought you were an idiot at the time… But now?</i>\" You turn your head to look at Kiha’s smiling face. \"<i>I can’t even look up at night anymore… without remembering that.</i>\"\n\n");
        outputText("You tell Kiha that you’re happy that your words got through to her. \"<i>Yeah… Me too.</i>\" She looks back up at the stars, and you can see a slightly nervous look in her eyes. \"<i>[name]… I’ve been thinking a lot… about how to pay you back for all you’ve done for me.</i>\" You open your mouth to say something, but Kiha puts a finger over your mouth. \"<i>Shush, let me talk… You wanted me to, after all.</i>\" Kiha looks back at the sky above. \"<i>I… Know you would have helped anyone. It’s who you are… But it’s always felt different with me. You went so far out of your way… </i>\" Kiha closes her eyes, hugging your arm as tight as she can. \"<i>I know I can be selfish, and cold… But you didn’t care.</i>\" Kiha’s voice is soft, and you have to strain to hear her.\n\n");
        outputText("Kiha sits up, and you do the same. As you watch, she digs her claws into the mattress beside her, and comes up with a small velvet box. \"<i>… I… Don’t know how to say this, but I want you to stay with me. I want to be with you… Forever.</i>\" Kiha opens the box, and a ring sits, nestled inside. A gold band, smooth and shining, supports a few tiny, red gems cut into the shape of little suns. The centerpiece, however, is breathtaking. It’s a black diamond with specks of white inside, and each little white speck seems to glow with its own light. Shocked, you watch as Kiha rises to one knee, looking you dead in the eyes, and blushes profusely.\n\n");
        outputText("\"<i>W-will you marry me?</i>\" Her tail wags behind her, and her eyes moisten. You’ve never seen Kiha this nervous before.\n\n");

        menu();
        addButton(1, "YES!", KihaProposalYes).disableIf(!SceneLib.templeofdivine.canMarry(), "You need to repair the temple first!");
        addButton(2, "Repair 1st", KihaProposalRepair);
    ...
    public function KihaProposalYes():void {
        ...
        outputText("Kiha melts into your embrace, her soft scales heating up as she brings her hand to the back of your head. Suddenly breathless, your hearts pounding, she keeps your lips against hers, eyes closed over tears of joy. You roll off the mattress together, tumbling over one another, eyes and faces all you can see… Until you roll too far off the dry spot, splashing into the swampy water of the swamp.\n\n");
        ...
        outputText("\"<i>… It’s an old piece of magic, from before. When I had this made… I didn’t want to just get you a fancy trinket.</i>\" Kiha looks deep into your eyes. \"<i>This ring has a few enchantments on it. While you wear it, fire won’t hurt you, and… It has a strength enchantment woven into it.</i>\" Kiha blushes. \"<i>… But that isn’t what you noticed.</i>\" You nod, asking her if this is what you think it is.\n\n");
    ...
    public function KihaProposalRepair():void {
        clearOutput();
        outputText("You clutch at your [chest], your heart suddenly aching. You tell Kiha that you want this. You want her to be yours, forever. Her face lights up, her tail wagging ferociously… and her smile quickly fades. \"<i>… But… ?</i>\" Kiha folds her wings, looking down and away. \"<i>… No. I should have known. I should have KNOWN this was too good to be true. Why would you ever-</i>\" You stop her rant, wrapping your arms around Kiha’s waist and pulling her close. \"<i>What are you doing, [name]?! Let me go!</i>\"\n\n");
        ...
        outputText("\"<i>So… It’s a yes?</i>\" You half-laugh, calling Kiha an idiot in love, and you close the ring case, wrapping your hands around Kiha’s. You tell her to hold onto the ring, because she’ll need it when you find the right venue.\n\n");
        outputText("\"<i>… That’s… a yes.</i>\" Kiha says dumbly, then blinks, it seems to finally click in her head. \"<i>That’s a YES!</i>\" She tackles you, laughing, and you laugh right back, taking her soft cheeks in your palms. \"<i>But no, my idiot. I won’t hold onto this ring. You will.</i>\" She takes the ring from its little box, and you smile, holding your hand out for her. Kiha slides the exquisite piece onto your hand… and you feel warm the moment it touches your skin. Your muscles feel tighter, your skin… tougher. You feel the heat around you rising… but it doesn’t affect you.\n\n");
        ...
        outputText("\"<i>… I’m sorry, my idiot… </i>\" You feel embarrassment, a little fear… What is this? \"<i>… It’s an old piece of magic, from before. When I had this made… I didn’t want to just get you a fancy trinket.</i>\" Kiha looks deep into your eyes. \"<i>This ring has a few enchantments on it. While you wear it, fire won’t hurt you, and… It has a strength enchantment woven into it.</i>\" Kiha blushes. \"<i>… But that isn’t what you noticed.</i>\" You nod, asking her if this is what you think it is.\n\n");
        outputText("\"<i>It’s an emotional link.</i>\" Kiha shivers, despite herself. \"<i>If you don’t want that, you can turn that part off… Or I could remove it, but… </i>\" You shush your dragoness, holding her close. You tell Kiha that it’s a wonderful gift. The ring floods with warmth, and Kiha lets loose a sound halfway between a squeal of delight and a roar, tackling you with gusto.\n\n");
        outputText("You walk back to camp together, reveling in each other’s company. You can feel her emotional reactions to everything, from her hand in yours (glee) to the cawing of some swamp birds (annoyance and hunger).\n\n");
    ...
    public function KihaMarriedHug():void {
        ...
        outputText("\"<i>What?</i>\" She asks, and you tighten your grip, ever so slightly. Not to be outdone, you feel Kiha’s claws against your back as she also grips you harder. You tell her that it’s nothing, and she shifts nervously, resting her head on your shoulder.\n\n");
        ...
        outputText("\"<i>Wh-what are you doing?</i>\" Kiha asks, her low, hushed tone very different from her normal, domineering personality. You tell Kiha that you’re admiring her wings. You say that despite how strong they are, they seem so delicate like this. \"<i>If you say so.</i>\" Kiha shudders as you lean in, kissing the side of her neck. \"<i>Really? You’re into my wings now?</i>\" She doesn’t have any real force behind those words, however, her tail caressing your thigh. \"<i>Pervert.</i>\"\n\n");
        ...
        outputText("\"<i>Idiot.</i>\" Kiha’s voice is low, but tender. She slowly folds her wings, and you put your hands on her hips"+(player.tallness < 72 ? ". You look into her dark red eyes, and she puts a hand on your cheek" : "")+". You jokingly remind Kiha that she chose to be here, with you. At this, she rolls her eyes, popping you on the thigh with her tail.\n\n");
    ...
    public function KihaLoveRecip():void {
        ...
        outputText("\"<i>So… You did do it for me, at least a little.</i>\" Kiha seems to have mixed feelings about that. You tell Kiha that she doesn’t owe you anything. That you would have done a lot more to ensure that Kiha could move forward. \"<i>… What are you saying?</i>\" Kiha’s voice is soft, and when she inhales, it’s like a little gasp.\n\n");
        ...
        outputText("Your dusky dragoness’s eyes moisten at that, and she turns her head so you can’t see her tears. \"<i>… Damn it. Why is this so hard?</i>\" You give her a small smile. You tell Kiha that you’ve been asking yourself that question for quite some time now. She’s shocked, and you seize that moment, bringing your lips to hers.\n\n");
        ...
        outputText("<i>“I hope you realise what comes next.”</i> Kiha murmurs, eyes half-lidded"+(player.isNaked()?"":", her slavering muff rubbing against the front of your [armor]")+". <i>“Because you’re not going anywhere.”</i> Your dragoness exhales a puff of fire, just enough to send a burst of warmth into your chest. <i>“Lover.”</i>\n\n");
        ProposalStatus = 1;
        doNext(kihaSexMenu);
    }
    ...
    public function KihaWedding():void {
        ...
        outputText("\"<i> [Name], champion from beyond this world, Slayer of demons, chosen warrior of Marae: Do you take Kiha to be yours, for better or for worse? To love, lose and be as one, until the skyfire sets on your last day?</i>\"\n\n");
        ...
        outputText("\"<i>Do you, Kiha of the Flaming axe, Slayer of demons, first of the Dragonoids, take [Name] to be yours, for better or for worse? To love, lose and be as one, until the skyfire sets on your last day?</i>\" Kiha stops dead cold, entire body stock still. She gulps, then takes a deep breath. When she talks, her voice is soft, far removed from her usual, brassy tone.\n\n");
        outputText("\"<i>This… I never… </i>\" Kiha struggles, before straightening her spine, spreading her wings wide and looking you in the eyes… You think. She still has the veil on, covering her entire face from view. \"<i>When I first met you, I was unimpressed.</i>\" This gets some chuckles, and she rolls her shoulder. \"<i>… I’d escaped from… A hell. No other way to put it… To find nothing better outside the demon’s lab. The world was different, everything trying to rape or kill, nothing left of the world I’d grown up in. I adapted quickly, killing everything I had to, trying to build my strength… Until you barged into my territory, not a care in the world. I tried to chase you off, Divines know I tried.</i>\"\n\n");
        outputText("Your guests are silent, but Kiha keeps going. \"<i>You’d fight me, never giving up… but refusing to take advantage, like everything else. You reminded me of better times, of… Well, of before. I didn’t want to remember… But I… couldn’t keep you away.</i>\" She sniffs, getting a few ‘aww’s from the more sensitive guests. \"<i>… I thought you were just an idiot, a fool looking to get yourself killed… Until those Driders came… And instead of leaving me to my fate, like anyone else would have done… You saved me. Fought alongside me… </i>\" Kiha’s grip on your hands tightens, and she takes a deep breath. \"<i>I swear, until the skyfire sets on my final day, I’m yours.</i>\"\n\n");
        outputText("\"<i>Let the divines witness this union, the vows spoken. Let those with us bear witness to the merging of hearts. Any who doubt, let them speak now, or be silent.</i>\" Nobody dares to even breathe, and you flip Kiha's veil up, revealing her eyes, shimmering with emotion. You lean in, wrapping your arms around her… And you hear a sudden bang, tackling Kiha to the ground as a shard of wood embeds itself in the wall behind you. You look back at the temple door, which has been flung from its hinges, blue fire licking the shattered wood. Your guests gasp, ducking behind the pews as the stone of the temple shakes. Above the door, the stone shatters, and through the ruined door frame walks a massive, masculine dragonoid.\n\n");
        ...
        outputText("He inhales through his draconic snout, exhaling blue flames as he surveys the church, guests… and his eyes fixate on your bride. He gives Kiha a lascivious grin, equal parts lustful and angry, and speaks in a jovial tone.\"<i>Oh, Kiha, my darling, it’s been too long!</i>\" He declares, spreading his hands wide. \"<i>I knew you loved me, but I honestly didn’t expect you so soon!</i>\"\n\n");
        outputText("Kiha flicks her veil up, and to your surprise, her eyes are wide, a single tear falling down her face. \"<i>… No.</i>\" She looks over at you, then back to him. \"<i>Magnar? I thought you were dead.</i>\" She clenches her fists. \"<i>What did they do to you?</i>\"\n\n");
        outputText("The giant dragonoid, Magnar, you’re assuming, chuckles. \"<i>Oh, nothing to worry about, Kiha. After all, you’ll be joining me, right?</i>\" His face loses some of the amusement. \"<i>You said you’d come back for me, your best friend.</i>\"\n\n");
        outputText("Kiha looks up, into the dragonoid’s eyes… and she sees the black joy there, the lust in his gaze as he eyes her breasts. \"<i>They changed you, Mags. I’m sorry I couldn’t come back… I was too late.</i>\"\n\n");
        outputText("\"<i>For once, you’re right… But it’s okay, stubborn scales. You couldn’t come then… But I’ll make you come now. Now and forever. You’ll be late many times… Mmm… </i>\" His black, scaled dong rises, tip nearly as high as your bride’s face. \"<i>You’ll be mine, Kiha. As you were always meant to be.</i>\"\n\n");
        ...
        outputText("The amusement drops from his gaze, and the giant looks at you, blue flames rising from his nose. \"<i>… So this wasn’t a joke, then… Or a way to get my attention.</i>\" He levels his maul at you, rage in his reptilian eyes. \"<i>Fine then, my love. Witness the death of your "+player.mf("bride","groom")+". Watch me crush them… and they can watch me consummate our love after.</i>\"\n\n");
    ...
    public function KihaWeddingVictory():void {
        ...
        outputText("Your bride, Kiha, steps past you, her expression unreadable and her axe slung over one shoulder. She approaches the fallen dragon, breathing heavily. \"<i>Magnar… Is that you?</i>\" You can feel her emotions through the ring, rage, regret and sadness churning, burning.\n\n");
        outputText("\"<i>You recognize me, love.</i>\" He chuckles. \"<i>You… They took you first… But I knew… </i>\" He coughs up blood. \"<i>You always… were looking after me, back home… But now I’m strong… I can protect you.</i>\" His toothy grin comes back. \"<i>We can be together… Like we promised on the hill.</i>\" Despite his condition, you can tell he’s watching her, taking in every facet of her naked body.\n\n");
        outputText("Kiha closes her eyes, and you can feel her sadness, her regrets, and… a spark of anger. \"<i>… Magnar… We were children. When I said we’d be together, that wasn’t what was meant.</i>\" She steps back as, despite his injuries, the mortally wounded black dragon stands, maw shaking with rage.\n\n");
        outputText("\"<i>You stand here, naked, drooling from your cunt… and say such things?</i>\" He roars, stumbling towards your bride. \"<i>I loved you, Kiha!</i>\"\n\n");
        ...
        outputText("\"<i>… You were a brother to me.</i>\" Kiha whispers. \"<i>But you died when the demons came.</i>\" She brings her axe down, flaming steel embedding itself inches deep in the corrupted dragon’s skull. Your bride stands over the body for a few moments, then shakes herself, spinning on one foot. Leaving her axe behind, she runs over to you, putting both hands on your shoulders.\n\n");
        outputText("\"<i>[Name], are you alright?!</i>\" Kiha gives you a rather aggressive once-over, eyes wide. Her fear rushes through the ring, a sick, guilty feeling. Your injuries aren’t too bad, but she still insists on taking a look at them, shakily sitting you down on one of the unbroken pews.\n\n");
        if (player.hasStatusEffect(StatusEffects.MagnarDominated)) {
            if (player.statusEffectv1(StatusEffects.MagnarDominated) >= 1){
                ...
                outputText("She looks down, seeing ");
                if (!player.armor.isNothing)
                    outputText("your [undergarment] displaced, ");
                if(!player.lowerGarment.isNothing)
                    outputText("the lower parts of your [armor] tossed to the side. ");
            }
            if (player.statusEffectv1(StatusEffects.MagnarDominated) >= 4) {
                outputText("Kiha wraps her shaking arms around you, sick guilt written all over her.\n\n");
                outputText("\"<i>[name], I’m so sorry. I wasn’t there, and he… Are you… ?</i>\" Kiha’s tears fall down her face. \"<i>We planned all this… and he… </i>\" ");
            }
            if (player.statusEffectv1(StatusEffects.MagnarDominated) >= 6) {
                outputText("She sniffs, the blackened spooge on the floor giving off a sickly sweet smell. \"<i>Is that… ?</i>\" You nod once, and your dragoness wraps her tail around your waist, talons digging furrows into the stone below her feet.\n\n");
            }
        }
        outputText("Kiha’s soft arms shake, and she rests her head on your shoulders. \"<i>I’m sorry… I’m sorry… </i>\" She whispers, over and over. You can feel her hot tears running down your back. You wrap your arms around your panicking dragon-girl, shushing her, making comforting sounds as your guests trickle back in. Kiha’s friend gives you a look, like \"<i>Can I do something?</i>\" You nod, flicking your head over to a large tablecloth. She nods, gently removing some of the food and bringing the covering over. She throws it over the two of you, and you hold Kiha close as the darkness embraces you.\n\n");
        outputText("After a few minutes, you gently call for her, whispering her name. Kiha reluctantly raises her head, swollen eyes meeting yours. \"<i>I love you.</i>\" You’ve never heard Kiha as vulnerable as she is now, slightly glowing yellow eyes the only thing lighting the darkness. You touch the side of her face, giving her a little smile. She almost laughs, a bittersweet huff that warms the small, dark space you’re sharing. You can feel from the ring, the sadness fading, her anger still burning bright, but the pain is dulling, the shock from the altarside brawl fading away. Kiha’s breathing steadies, her heartbeat slows, and you feel some of the happiness come back to her.\n\n");
        outputText("\"<i>… So what now?</i>\" She whispers, hands still shaking. \"<i>Do we… go forward?</i>\" You almost laugh. Of course, you will. You tell Kiha that today was meant to be the happiest day of your lives, and that you’re no quitter. Rapist giant dragon or no, you fully intend on making today special.\n\n");
        outputText("\"<i>Alright.</i>\" Kiha’s voice regains most of her vigor almost immediately, and she kisses your neck. \"<i>So… I’ll go get changed back, you clean up, and we… ?!</i>\" You stop her with a kiss, asking her if the dress really matters to her. You tell Kiha that you intend to marry her, your dragoness. Naked and beautiful is how you met her, after all. Kiha gives you a happy hug, tail wagging behind her. \"<i>You’re damn right I’m beautiful naked.</i>\"\n\n");
        ...
        outputText("You’re snapped from your reverie by a light punch to your shoulder. Kiha sits down beside you, eyes shining. \"<i>Hey.</i>\" She takes your hand, and with free far hand, she takes a forkful of the food on her plate, holding it up to your mouth.\n\n");
        ...
        outputText("\"<i>Finally.</i>\" Kiha says, voice low, barely more than a whisper. \"<i>Are you not enjoying the meal?</i>\" You tell Kiha that you have your eyes on something more enticing than dinner, and she gives you a slow, languid wink, lazily bringing another bite to you. Not one to pass up food, you lean forward, wrapping your lips around the bite. \"<i>I’m starving.</i>\" Kiha opens her mouth expectantly. You gamely hold the bite out, but as Kiha bites down, you pull it back, bringing your own head forward and kissing her cheek. The sun begins to set, and the guests start to leave. The dragonoids say goodbye to Kiha, and you watch your wife hug her friend, the wedding dress over one shoulder. The last of your guests leave, and Kiha takes one of the last remaining bottles of wine. She uncorks it, sitting down beside you, then takes a long swig.\n\n");
        outputText("\"<i>I don’t know where you want to go, my idiot</i>\", she purrs, her tail wrapping around your waist. “<i>But I can tell you right now… You aren’t leaving my sight until the sun comes back up.</i>” She walks her fingers up your chest, and you can feel… Wow… Kiha is extremely turned on right now. It’s affecting you through your wedding band. You aren’t going anywhere.\n\n");
        outputText("Kiha takes your hand, and you entwine your fingers with hers, her soft breasts pressed against you. Kiha’s eyes are half-lidded, and she flicks her head up back towards the temple. You gamely follow, still holding your bride’s hand, as she leads you past the altar, into a back room, where to your surprise, Kiha seems to have snuck an entire bed and several bottles of Lust Draft. She pops your [ass] with her tail, and as you turn back towards her, she gives you a scorching, almost hungry gaze as she slides an iron bar home, locking the door with a resounding thud.\n\n");
    ...
    public function KihaMaleWeddingNight():void {
        clearOutput();
        outputText("You slip her grip and grab a hold on your dragoness’s toned ass with one hand, guiding her down towards your [cock]. You rub your shaft up her thigh, and as Kiha inhales slightly, looking right into your eyes, you guide her hips down, her swollen, drooling lips parting on your [cockhead]. Kiha shivers, both her hands on either side of your head as you slowly slip into her burning, soaking pussy. She shudders, letting her legs and arms release, her weight now pressing comfortably on you. For a moment, you lie, your rock-hard [cock] inside, the inside of her thighs on your [legs], not moving, neither of you willing to break the moment.\n\n");
        outputText("Kiha looks down into your eyes, smiling and sticking her reptilian tongue out, tickling your lips. “<i>Well? You’ve got me… Now… </i>” She wiggles her hips, sending shocks of pleasure down your rod. “<i>Time to lay the dragon.</i>”\n\n");
        outputText("You surge up, twisting your hips. Kiha lets out a moan, wings spreading further out as she slams down in time with you, burying you to the hilt inside her. You start slow, but deep, bringing yourself almost out with each thrust before burying yourself back as deep as you’ll go. It only takes a few thrusts before you can feel your dragoness shudder, trying to pick up the pace. She rests her palms on your shoulders, but you grab her wrists, playfully pushing them aside. With a little “<i>Eep!</i>” You never expected from Kiha, she falls, and you wrap your arms around her, waist and shoulder, pinning her to you as you pick up the pace, fucking your dragon-wife in earnest.\n\n");
        outputText("“<i>Th-that was… dirty… </i>” Kiha mutters, but her blush and smile gives her away. “<i>Mm… </i>” She pants, pulling herself up with a moist schluck. Your lower halves are already soaked, and she grins, sitting up and over you, her D-cups bouncing as you keep moving. Kiha folds her wings, her tail shaking as she buries your [cock] into her.\n\n");
        outputText("“<i>Mm-m-Make… me cum!</i>” Kiha wails as you pull her back down, her pillowy breasts smooshing into you. You take Kiha’s moaning lips and cover them with your own, her cries muffled in your mouth. Her tongue finds yours, and you grab both her palmable asscheeks, kneading them as she takes your head in hers, all but forcing you to look your lusty wife in the eyes as her walls clamp down on you. Kiha tries to hold back her voice, but she lets out a guttural moan, hips rolling as she covers your shaft in her love-juices. Near your own orgasm, you keep fucking her, and Kiha’s cries grow louder, her love-tunnel heating up even more. You cum, jamming your [cock] in, holding Kiha’s gaze as your load enters her. She squirms a little, shaking as you both revel in the feeling.\n\n");
        outputText("Kiha’s gaze quickly focuses, however, and as she pulls herself off of you, she lets the warm, white fluid drip down her inner thighs. Kiha shakes her head at the sight, turning her back on you and bending over, giving you a clear view of her swollen pussy lips, trickling your cum down past her knees. Kiha winks back at you, raising her tail, and your [cock] stands at attention. You grin, and Kiha braces herself against the wall, letting a bit of her fire from her mouth.\n\n");
        outputText("You wrap your hands around Kiha’s dusky neck, and your dragoness moans, letting her tongue loll out. Hands against the wall, your dragoness pants, back arched and knees shaking as you pound her red-hot chasm. Her tail wraps around your waist, pulling you back in and smacking your hips into hers. With a cry, and a gush of femcum, Kiha’s legs nearly collapse, and she leans into the wall, tits against the stone. With each slam of your hips, you can feel your dragoness shudder, her furnacelike twat leaking.\n\n");
        outputText("Kiha looks back at you, her reptilian eyes glazed over with lust, and she throws herself backwards, catching you by surprise and bowling you over. She gleefully follows you to the floor, gasping as she impales herself on your rod. Without missing a beat, Kiha starts pumping herself on your shaft, slowly rotating so she can face you.\n\n");
        outputText("“<i>Come on, my dumbass… </i>” She purrs, running her clawed hands gently up your hips, then up to your [breasts], slowing down the pace as she sensually moves, sliding her legs down outside yours, resting more of her upper body onto you. “<i>Cum for me. Cum again!</i>” She leans in, nibbling on your neck, and as she clamps down, her rings of sweet flesh trapping your [cock], you thrust up, letting a groan loose. Your [cock] twitches, and Kiha moans, arching her back and slapping her tail against the ground as you paint her womb white.\n\n");
        outputText("Kiha drops, her knees on either side of you, your [cock] still inside her. She bites her lip, her walls tightening again around you. She looks down into your eyes, spreading her wings as your mixed juices trickle down onto your thighs. “<i>Mmm… Not a bad start.</i>” She licks her lips, resting her weight onto your pelvis. “<i>But I’m nowhere near done.</i>” Her tail wraps around one of the bottles of Lust Draft, and she chugs half of it, offering you the other. You grin, downing it, and you feel your cock stiffen inside her. You give Kiha a little thrust, and she exhales a warm breath, slitted eyes moist. She bears down, pinning your shoulders against the floor, and begins to ride you, grinding her hips into yours with wild abandon. You bite your lip, letting Kiha have her fun… \n\n");
        outputText("You barely remember much of the night after that… But a few memories stick out.\n\n");
        outputText("… Holding Kiha’s wings as you thrust up into her, rubbing the membranes of her fragile-looking bat wings… \n\n");
        outputText("… Her tail in between your asscheeks as you suckle her dark-skinned breast, the tip slowly spreading your puckered asshole… \n\n");
        outputText("… Kiha, one eye closed from the load you blew on her face, shoving your length down her throat with abandon… \n\n");
    ...
    public function KihaWeddingBeaten():void {
        clearOutput();
        outputText("Unable to stand, you collapse to the floor. Your wedding ruined, your body spent, all you can do is watch as he towers over you. “<i>An admirable fight, champion… But now it’s over.</i>” He raises the hammer high, and all you can do is watch as the maul’s head comes hurtling towards you… \n\n");
        outputText("Kiha roars, slamming shoulder-first into your foe. Flames seething from her maw, your dragoness lifts her massive axe, standing between you and him. “<i>No, it’s not.</i>” She declares proudly, holding her head high. “<i>Because I’m still standing.</i>” Your heart hammers in your chest, and for a moment, you feel relief, and a stirring of love in your chest. Kiha’s a warrior, and you were able to get a number of hard hits in. Surely she can finish what you started… !\n\n");
        outputText("What you forgot, and what Kiha never really acknowledged, was that you’re stronger than her. She fights bravely, getting a few good hits in with her axe, nearly chopping off a wing, breaking his scales in places… But to your horror, his massive hand lashes out, grabbing your dragoness’s slender neck. Her axe is next, quickly slapped out of her free hand, and from there he takes Kiha down to the floor, covering her in his bulk.\n\n");
        outputText("To your surprise, he stops there, turning his head back to you, amused as Kiha struggles to get him off her. “<i>Let this be your lesson. Kiha is mine.</i>” As you watch, horrified, he spreads Kiha’s legs with his tail, rubbing her pussy lips with his tool. You struggle to move, but you’re forced to watch as Kiha’s stomach bulges, your wife-to-be crying out as he forces his shaft into her. “<i>Hmm… I missed you, in the pens. But that’s okay. Soon we’ll be together.</i>”\n\n");
        outputText("“<i>N-no!</i>” Kiha tears up, her dusky cheeks as red as her scales. She breathes fire in his face, whips him with her tail, but nothing can stop him. You turn away, trying to protect your bride’s dignity… or what’s left of it. You hear her cry out for help, for you… but her wails stop, moans taking their place. You hear the grunts and the slow drip, drip. Kiha screams an orgasmic cry, and you open your eyes to see her against the wall, tail hanging limp, tongue out in a slutty face as black cum drips from her quim.\n\n");
    ...
    public function KihaWeddingDominated():void {
        clearOutput();
        outputText("Your [tongue] lolls out from your head, and you give your dominant lover an orgasmic moan. Your [breasts] shake with each thrust, and he turns you over, pinning you back-first against the wall, his lust-inducing breath blurring your vision. You idly pick at his barrel chest, letting him do the work as he hits the deepest parts of your womanhood. “<i>Cum for me, little bitch. Cum for me and be mine. Join your little dragoness at my side.</i>” A small part of you screams out in your head. This was supposed to be your wedding night! Kiha’s the love of your life! You can’t just… let this… Massive dragon dick wipe… \n\n");
        outputText("Maybe it’s for the best. Maybe such a big, strong dragon was what you wanted. Kiha’s great, but she could never fill you like this. You cum, the massive, twitching cock inside you too delicious to endure. Your entire body goes limp, and he rewards you, nipping your neck with his teeth, his low, growling whisper in your ear as he keeps fucking you.\n\n");
        outputText("“<i>That’s right, little champion. Now you just lie there. Let my liquid love do its work</i>”. He cums again, sending a pulse of heat through your entire body. Your eyes roll up into the back of your head, and you can barely hear an echo of a dismayed cry. [“<i>Name], no!</i>” You turn your head, a dreamy smile on your face, as Kiha leaps into battle. You try to stand, you do… But your muscles won’t listen to you. Your eyes close.\n\n");
        outputText("You open your eyes, still barely able to feel your legs. Your mouth is dry, and as you raise your head, you hear a wet slapping sound, and the moans of a woman desperately trying to hold herself back.\n\n");
        outputText("Your red-skinned dragoness, hands tied behind her back, is being fucked from behind by that massive dragon-demon. Kiha is clearly beaten, bruises covering her chest and arms. She tries to hold a moan back, failing spectacularly as he pulls her head up so her eyes meet his. She flaps her wings weakly, but all that gets her is a particularly hard thrust. Kiha tears up, kicking at his shins, but the sticky, black fluid is already dripping down her legs.\n\n");
        outputText("“<i>Oh, look my lovely. Your champion’s awoken. She’s quite good looking, isn’t she?</i>” Kiha’s eyes widen, and she looks over at you. The black dragon slaps her ass hard, getting another moan from your draconic lover. You groan, trying to move… but your muscles are still slack, and you can’t feel your legs. Your thighs are still sticky with his cum, and as he glares down, red eyes glowing, your heart races, fear and arousal rushing through you. “<i>Aw, look. She’s trying to come over here.</i>” He chuckles. “<i>Sorry Kiha, but I’m going to have to make this quick now.</i>”\n\n");
        outputText("He picks Kiha up effortlessly, walking her over to you and slamming her face-down onto the floor beside you. Kiha’s dusky cheeks, bright red with shame… and arousal, fill your field of view. Helpless, unable to move, you watch as the demon toys with Kiha, filling her with his aphrodisiac cum over and over, making her squeal until her eyes are half-lidded. “<i>… Please… </i>” You hear her whisper. “<i>Not… [name]. I don’t want to… </i>” Kiha’s tears begin to fall. “<i>I… </i>” Kiha’s back arches, and she lets out a wail, collapsing to the floor beside you. The black dragon picks you up, slathering his cock on your stomach.\n\n");
        outputText("“<i>Sleep, my little dragoness. I’ll wake you when you’re nice and ready.</i>” You’re unable to comprehend his meaning, eyes already closed. You wake on a soft, massive bed, with a mirror right in front of you. Opening your eyes, you blink in shock, sitting bolt upright. You rush over to the mirror, your head swimming with the sudden movement. You feel dizzy almost… Thinking is hard.\n\n");
        outputText("You are a 6 ft dragon-morph, with blue scales, DD cup breasts, and soft, pliant muscles. Between your legs is a single, slavering pussy, already dripping wet and raring to go. You poke yourself, only to feel silvery shocks of pleasure through your entire body. You’ve become extremely sensitive.\n\n");
    ...
    public function KihaKidnapped():void {
        ...
        menu();
        addButton(1, "Rescue", KihaRescue).disableIf(!player.hasStatusEffect(StatusEffects.FieryBand), "Missing Starfire Band - You need Kiha's ring to find her!", "Resque");
        addButton(2, "Camp", playerMenu);
    }
    ...
    public function KihaRescue():void {
        ...
        outputText("\"<i>Hnngh… </i>\" You recognize that voice. Silently padding your way down, you can hear the drip… drip… \" getting louder. An embroidered curtain leads into the next room, and you slip your head past, peeking in.\n\n");
        ...
        outputText("\"<i>Kiha, you won’t keep your strength up if you don’t eat.</i>\" He says this teasingly, but you can hear the rumble in his voice. You hear a spitting sound, the sound of metal on stone, and he moves to one side.\n\n");
        outputText("Kiha’s there, wrists clamped and bound to the wall. Her feet, well off the ground, aren’t bound, but the way she slumps… She’s lost a few pounds, and you can see the telltale black ooze dripping down her legs. She’s shaking, tail poised between the black dragon and her swollen, damp lips. \"<i>… I will never love you.</i>\" Kiha spits. \"<i>Not now… not ever.</i>\" He brings a hand in, slapping aside her tail, and sinking three fingers deep inside her. Kiha flails, trying to dislodge him, but he grins, stepping in, grabbing one leg and lifting it up. Your dragoness wails, cumming almost immediately.\n\n");
        outputText("\"<i>Mind, no. Soul, no. I see that now.</i>\" The beast growls, rage dripping in his gaze. \"<i>I showed you the bed you could be sleeping in… I wanted to court you properly.</i>\" He grins, a dark aura spilling from his wings. \"<i>But… Those things can be dealt with. You’re already sensitive to me, I made sure of that.</i>\" He laughs, and Kiha turns her head, trying to look away as her cheeks flush. \"<i>But the mind… It can be weakened. The demons made sure of that. All it takes is a little drink.</i>\" You watch in horror as he grabs a bottle labelled \"<i>Bimbo Liqueur</i>\" off the nearby shelf. \"<i>And from there… Well, your soul can be mine too. And I will greatly enjoy the process needed to take it.</i>\"\n\n");
    ...
    public function KihaRescueVictory():void {
        ...
        outputText("The walk home is uneventful. You run into only a few creatures of Mareth, and none seem willing to cross you right now. Instead of her nest, however, you bring Kiha to your "+(flags[kFLAGS.CAMP_BUILT_CABIN] > 0?"cabin":"tent")+", laying your dragoness to rest in your "+(flags[kFLAGS.CAMP_BUILT_CABIN] > 0?"bed":"bedroll")+". You lie down next to her, and Kiha wraps her uninjured arm around you, a small hum escaping her.\n\n");
        outputText("\"<i>My Hero</i>\", she says, with no small amusement in her voice. You jokingly tell her that next time it’s her turn to save you, and she smiles sleepily. \"<i>So… The usual… Then.</i>\" Kiha’s grip softens, but she nuzzles in close, her surprisingly soft muscles warming you to the bone. Reaching into your wedding ring, you can feel warmth, happiness… and comfort. Kiha feels safe, for the first time since you found her, and for the life of you, you can’t find any reason to disturb that. You relax, letting the physical and mental warmth of your dragon wife lull you to sleep.\n\n");
        outputText("\"<i>Good Night… My "+player.mf("Husband","Wife")+"</i>\" Kiha murmurs. \"<i>My brave… Idiot.</i>\"\n\n");
    ...
    private function TalkWithKiha():void {
		...
		menu();
        if (totalKihaChildren() == 0) addButton(1, "Family", KihaTalkFamily);
        if (totalKihaChildren() > 0) addButton (1, "Kids", KihaTalkKids); //TODO
        if (KihaTownState == 1 && ProposalStatus < 1) addButton(2, "Town", KihaTalkTown);
        if (KihaTownState == 0) addButton(2, "Home", KihaTalkHome);
        ...
        if (ProposalStatus == 3) {
            addButton(5, "WeddingReady", KihaWedding2)
    ...
    private function visitKids():void {
        clearOutput();
        outputText("Kiha gives you a small smile as you ask about them. She takes your hand, squeezing it hard, and the two of you walk to Kiha’s little cave. \n"
            + "\n"
            + "Inside, "+totalKihaChildren()+" young dragonoids mill about. Mareth’s quick growth matured their bodies, but evidently not their minds, entirely. A few spurts of fire roar from your children’s mouths as they rush over, spreading their wings and tackling you. \n"
            + "\n"
            + "Your children bowl you over, and you find yourself laughing at their enthusiasm. Kiha quickly begins helping you, pulling your scaly children off of you.\n");
        doNext(playerMenu);
    }

    private function KihaTalkCampmates():void {
        menu();
        addButton(0, "Ember", KihaTalkEmber)
            .disableIf(!emberScene.followerEmber(), "", "???");
    ...
    public function KihaTalkHome():void {
        clearOutput();
        if (DemonLab.MainAreaComplete == 0) {
        ...
        outputText("Kiha sighs, hugging herself and digging her claws into her shoulders. She’s too stubborn to cry, but her sides are shaking, her tail lashing into a nearby root.\n\n");
        outputText("\n\n");
        KihaTownState = 1;
        menu();
        addButton(1, "Comfort", KihaFirstTimeTownComfort);
        addButton(2, "Question", KihaFirstTimeTownQuestion);
    }

    public function KihaFirstTimeTownComfort():void {
        clearOutput();
        outputText("You slide closer to your dragon lover, resting your arm on her shoulder. Kiha sighs, leaning into you. She’s shaking, and you hold her close. Her scales are soft, and her surprisingly light body heats up at your touch. Kiha hugs you back, turning her head away from the sight, to bury her head into your [breasts].\n\n");
        outputText("You ask Kiha if she’s alright, and she hesitates. You see her struggling, and hold her closer, as if your arms can chase away her past, the horror she’s clearly still trying to deal with. “<i>… No.</i>” She eventually says, her voice weak. “<i>No, I’m not.</i>” Kiha looks back at the spot, her back tensing up. “<i>… I honestly don’t know how you do it, [name].</i>” She shivers. “<i>You watch the spot where your home sits, just out of reach… But I… </i>” She clenches her fists. “<i>This is the first time I’ve come back here since… since they came.</i>”\n\n");
        outputText("You sit there in silence for a time, before talking again. You ask her why she decided to come back now, of all times. “<i>… You should know that, my idiot.</i>” Kiha wraps an arm around you. “<i>… I thought… It’d be easier with you around.</i>” You ask her what she means by that, and she gives you one of her familiar glares. “<i>You… You know!</i>” She slugs you lightly in the gut. “<i>I… thought… maybe it’d help, coming back… </i>”\n\n");
        outputText(" You tell Kiha that you don’t like seeing her so sad… But that you’re happy she’s opening up like this. “<i>W-well, I trust you, dumbass! And I-if I want… </i>” She looks away, at the grass between her talons. “<i>… I wanted you to know… Why I am the way I am.</i>”\n\n");
        doNext(KihaFirstTimeTownQuestion);
    }

    public function KihaFirstTimeTownQuestion():void {
        clearOutput();
        outputText("You gently put a hand on Kiha’s shoulder. She looks at you, then back at the spot. Gently, slowly, you ask her about the flattened land, the burned wood. Kiha inhales deeply, clearly worried.\n\n");
        outputText("“<i>Yeah… This was my hometown.</i>” Kiha answers this shakily, her red eyes moist. “<i>It was a small village. We were between the Desert and Plains, so… Traders would pass through often</i>”. Kiha shivers, despite the very warm air. “<i>… When they came, we thought they were traders… But they… well… </i>” She shudders. “<i>Give me a second, okay?</i>”\n\n");
        outputText("You pat Kiha’s thigh, then rest your hand on the ground next to her, leaning back, waiting for your lover to be ready. Kiha holds back her tears,  watching that spot for what seems like hours before looking back towards you.\n\n");
        outputText("“<i>Thank you… for waiting.</i>” Kiha whispers, leaning into you. “<i>… I thought this would be easier with you, but… It’s not.</i>”\n\n");
        outputText("You look Kiha in the eyes, giving her a comforting smile. Putting a hand on her cheek, you bring your hands to her face, cutting off her view of the surrounding area. Before Kiha can say anything, you’re already speaking. You tell Kiha that you won’t judge her, that you already know she’s been through a lot. She opens her mouth, and you put a finger over her lips. You tell your dragoness that you’re here for her. This puts a smile on Kiha’s face, despite her tears, and she takes your hands in hers.\n\n");
        outputText("“<i>W-well… This hill was my favorite place to go as a kid.</i>” She looks up into the old apple tree. “<i>It’s a great view, and I used to climb this old thing, get high up into the branches.</i>” She snuggles in closer to you, her soft, D-cup breasts pressing against you. “<i>There was almost always an apple or two in the highest branches that nobody else could get… But the view… Well… That was the real reason I’d come here.</i>” She waves at the surrounding area, and you nod appreciatively. It is a nice view. “<i>We were a halfway town. Inns, farms and hunting were the town’s main three trades. We’d get traders coming through almost every day.</i>” She smiles, lost in her own memories.\n\n");
        outputText("You instinctively wipe a tear from her eye as Kiha looks down at the ground. “<i>When they came, we thought that they were traders like the rest… But they… It started with the innkeeper’s daughter. They tied her down and… </i>” Kiha shudders, and you rub her cheek. “<i>Well… They took most of us alive… We thought we were spared to be slaves. Some of us submitted, others were turned into more of them… I was one of the few brave enough to try and escape.</i>”\n\n");
        outputText("You ask if that’s when they… made her into this. Kiha nods, and her red eyes burn with repressed anger.\n\n");
        outputText("“<i>Filawin was our town's inkeeper… He’d always let people stay, even if they couldn't pay up front. His soft heart made him the heart and soul of our town… So when he found a dragoness, barely clinging to life, he took her in. We’d heard rumblings of the dragons being slain, but we hadn’t believed them… Not until we saw her, wings torn and her legs… </i>” Kiha’s voice trails off. “<i>He and his son, Magnar, they tried to hide her in the inn’s wine cellar. Dumb idea, really.</i>” She shudders. “<i>They got the information, who’d hidden them, easily. It was bandied about the town.</i>” Kiha’s eyes are haunted as she stares at one of the scorched timbers. “<i>None of them died quick. And they made us watch.</i>” She looks down at the ground, and you feel the rage coming off her. Kiha shivers, despite the warm climate.\n\n");
        outputText("Kiha looks at you, tears on her face, but warmth in her eyes. “<i>… They had been trying to create dragons, enslave them, and conquer Mareth.</i>” Looking down at herself, she almost laughs, a bitter, harsh sound. <i>\"They got the first part, at least.</i>\"\n\n");
        outputText("You rest your head on Kiha's shoulder, trying to calm her down. After a few minutes, you ask her about what happened. For a few moments, Kiha is stone-still, muscles tense and teeth grinding.\n\n");
        outputText("<i>\"They took us to some lab. I don't know where it is now, but it's somewhere in the mountains, hidden away. Fuckers thought that they could just do whatever they wanted to us… So when their imps came in to have their playtime… I fucking killed them.</i>\"\n\n");
        outputText("Kiha shudders violently, shrugging you off. \"<i>… Look… I don't know what happened to them. To any of them. They took me away, took their revenge for the imps, and then… There was heat. So much heat… \"</i> you put a hand on Kiha's shoulder. She doesn't need to go on.\n\n");
        outputText("Kiha nuzzles your hand, shivering. \"<i>There was an alarm, blaring. I remember… Just… <b> feeling </b> that was the time. So I bit off the demon dick… that was… </i>\" Kiha shudders in disgust. <i>\"Well… In my mouth… and I just… Tore open his throat and ran. I remember a bunch of… creatures. With long claws, with nothing between their legs. Demons everywhere… but they didn't seem to notice me in the chaos.</i>\" Kiha looks down in shame. \"<i> I just… Ran. A warrior of my village, now a mighty dragon, and… I just… Ran and hid. There was no fight, no glory, just… </i> Kiha hangs her head in shame.\n\n");
        outputText("You wrap your arms around Kiha, holding her close as the dragoness shivers. It takes some time before she can speak again, but she continues. <i>\" So… That's why I want to kill Lethice… But… I don't know if I'll ever be able to go back to that place… even if I kill her.Even if I'm strong enough to wipe her out… </i>\"\n\n");
    ...
    public function emberInteraction():void {
        ...
        function emberStart():void {
            ...
            addButton(2, "Ember's side", emberSide);
            addButton(3, "Ladies!", ladies);
        }

        function explain():void {
            clearOutput();
            outputText("With a calm, firm tone, you tell the two dragons to let you talk. Their gazes snap to you, and you begin with Ember, telling her about how you found Kiha in the swamp. You explain what happened between you two, and why you won’t tolerate Ember belittling Kiha or her struggles. Your dragoness gives Ember a smirk, and you turn to Kiha next. \n"
                + "\n"
                + "You tell Kiha about Ember’s egg, how you found and hatched Ember from it. You remind Kiha that you need every ally you can get to defeat Lethice, and despite being recently hatched, Ember is decidedly powerful enough to kill most demons. Now it’s Ember’s turn to act haughty. \n"
                + "\n"
    ...
    //No buttons or clears
    public function tyrantiaInteraction(fromTyrantia:Boolean):void {
        outputText("You hear a familiar roar coming from the training area. As you get near the ring, you see your dragoness flying rings around a familiar, steel-clad giantess. Kiha exhales fire, but Tyrantia doesn’t even seem affected by the flames. Kiha dives in, oversized axe swinging, but Tyrantia parries with the haft of her spear. \n"
            + "\n"
