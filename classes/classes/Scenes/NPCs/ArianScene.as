...
*/
	...
	public static var ArianTotalKidsNum:Number;

	public function stateObjectName():String {
		return "ArianFollower";
	}
	...
	public function loadFromObject(o:Object, ignoreErrors:Boolean):void {
		if (o) {
			ArianSonsNum = o["ArianSonsNum"];
			ArianDaughtersNum = o["ArianDaughtersNum"];
			ArianHermKidsNum = o["ArianHermKidsNum"];
			ArianTotalKidsNum = o["ArianTotalKidsNum"];
		} else {
		...
		public function timeChange():Boolean
		{
			...
			if (player.hasStatusEffect(StatusEffects.ArianEggTimer) && player.statusEffectv1(StatusEffects.ArianEggTimer) > 0) player.addStatusValue(StatusEffects.ArianEggTimer, 1, -1);
			return false;
		}
		...
		public function timeChangeLarge():Boolean {
			if (arianFollower()) {
				...
				if (player.hasStatusEffect(StatusEffects.ArianEggTimer) && player.statusEffectv1(StatusEffects.ArianEggTimer) == 0) {
					player.removeStatusEffect(StatusEffects.ArianEggTimer);
					ArianHatching();
					return true;
				}
			}
...
private function doublePenetrateArian():void {
	...
	player.sexReward("vaginalFluids","Dick");
	if (arianFollower()) ArianPregChance();
	dynStats("sen", -2);
	doNext(camp.returnToCampUseOneHour);
}
...
public function ArianPregChampCarries1():void {
	...
	outputText("You decide to humor Arian, especially if [arian ey] can see so early. You bare your stomach, and he steps gingerly in, placing a scaly hand on you. Arian closes [arian em] eyes, and you can feel a little tingle as a pulse of mana flows through you.\n\n");
...
public function ArianEggLaying():void {
    ...
    outputText("She pushes " + eggco + " eggs out, before finally collapsing back into her chair. For a few minutes, she passes out, and you decide to move her to her bed.\n\n");
	...
	ArianHermKidsNum += eggco;
	player.createStatusEffect(StatusEffects.ArianEggTimer, 24, 0, 0, 0)
	doNext(playerMenu);
}

public function ArianHatching():void {
	spriteSelect(SpriteDb.s_arian);
	outputText("You hear an excited yell from Arian’s tent, and [arian ey] pokes [arian eir] head out, motioning wildly. \"<i>[name]! Come quick!</i>\" You rush over to Arian, and [arian ey] pulls you into [arian eir] tent.\n\n");
	outputText("You stand beside Arian, not knowing what to do, and [arian ey] points at the incubation basket, where you hear a slight cracking sound. \"<i>They’re hatching!</i>\" Arian says, rather unnecessarily, grabbing your hand, hopping up and down with excitement.\n\n");
	outputText("Cracks form on the largest egg, and a piece comes flying off as a white-scaled little head pokes out, closing its eyes against the light. The little lizan makes a high-pitched chirping noise, and you see a clawed hand breach the shell, sending little pieces out. Arian steps forward, slowly, as the little one stretches out their spine, chirping as they break the egg wide open. Arian extends [arian eir] hand, and they look up, sheer wonder in their slitted eyes as Arian picks them up, cradling them in [arian eir] arms.\n\n");
	...
	ArianSonsNum += 1;
	ArianDaughtersNum += 1;
	if (ArianHermKidsNum > 3) {
		if (rand(2) == 0) ArianSonsNum += 1;
		else ArianDaughtersNum += 1;
	}
	if (ArianHermKidsNum > 2) {
		if (rand(2) == 0) ArianSonsNum += 1;
		else ArianDaughtersNum += 1;
	}
	ArianHermKidsNum = 0;
