...
public function KitsuneShrine():void {
    ...
    if (tailz == 9 && flags[kFLAGS.AYANE_FOLLOWER == 0]) {
		...
		outputText("\n\nThe kitsune approaches and bows to you in reverence. \"<i>You have acquired near-deific status, [name]. As a priestess of Taoth, it would be an honor to serve as your attendant. That is, if you would allow me to follow and assist you.</i>\"");
...
private function AyaneStayAtCamp():void {
	...
	outputText("\n\nAyane bows her head, then heads back to the forest to gather her things. You follow, and she packs up her possessions in an odd-looking bag. As you watch, she packs her bedroll up and puts it in... and the bag doesn't seem to change at all. She smiles at your surprise. \"<i> Surprised? This bag is much larger on the inside. Convenient, is it not?\"</i> She finishes, and you lead your new acolyte back to camp.");
	...
	switch(player.gender) {
		...
		case 3:
			...
			if (player.femininity >= 60 || player.femininity >= 30 && player.breastRows[0].breastRating > 1
				...
				addButton(0, "Bath (F)", HaveAGirlBath).hint("Have a bath with the girls.");
			else addButtonDisabled(0, "Bath (F)", "You're too masculine to be considered female. Perhaps, if you have a big enough rack, they'll reconsider?");
            //boys
			if (player.femininity <= 30 || player.femininity <= 60 && player.breastRows[0].breastRating < 2 || player.longestCockLength() > 20)
				addButton(1, "Bath (M)", HaveABoysBath).hint("Have a bath with the boys.");
...
public function HaveABoysBath():void {
	...
	if (player.gender == 3) {
		...
		if (player.cor < 33)
			...
			outputText("You quickly notice their glances and give your boys a show of your [breasts] and [butt].\n\n");
	}
	...
	if (flags[kFLAGS.KONSTANTIN_FOLLOWER] >= 2 && !player.hasStatusEffect(StatusEffects.KonstantinOff)) outputText("Sitting near you, Konstantin relaxes and stretches his limbs. His large and thick build dwarfs that of the other guys. While chit-chatting, he puts one arm around you in a friendly manner, and with the other, he shamelessly washes his gigantic manhood, earning more than a few stares and some blushes from the other boys.\n\n");
	...
	if (camp.loversHotBathCount() > 0) {
		outputText("You discuss your goals with the other guys when a girl's voice rings out from outside the spring.\n\n");
...
public function PeepingTom3():void {
	...
	if (isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0) outputText("Isabella sings as she bathes, which is not surprising. You assume they’re folk songs from her homeland.\n\n");
	...
	if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0) {
		if (flags[kFLAGS.IZMA_NO_COCK] == 0) outputText("While not entirely a girl, Izma is womanly enough that she's still accepted in the bath, her crotch well hidden behind a cloth.\n\n");
		else outputText("While she was not always a girl, Izma is womanly enough that she's still accepted in the bath.\n\n");
	}
	if (flags[kFLAGS.SAMIRAH_FOLLOWER] > 9) outputText("As a naga, Samirah takes a lot of space in the bath. You're glad you made it big enough to hold as many people as possible.\n\n");
	if (flags[kFLAGS.NADIA_FOLLOWER] >= 6 && !player.hasStatusEffect(StatusEffects.NadiaOff)) outputText("Nadia is resting next to a set of medicinal and aromatic oils she regularly sprays in the bath.\n\n");
	...
	if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff)) outputText("Luna is enjoying a break, relaxing for once, as she is not on duty" + (flags[kFLAGS.LUNA_FOLLOWER] > 6 ? ", though for a few split seconds, you imagined her doing the doggy paddle in the water" : "") + ".\n\n");
	...
	if (flags[kFLAGS.SIDONIE_FOLLOWER] >= 1) {
		outputText("Sitting on one side of the spring, spacious enough to accommodate her larger frame, Sidonie relaxes among the girls. Most of them give her not-so-subtle glances, some at her hefty bosom, but most directed at the thick equine member dangling between her legs. Not caring too much about their looks, she simply lies back and enjoys the bath.\n\n");
	}
	...
	if (emberScene.followerEmber() && (flags[kFLAGS.EMBER_GENDER] == 2 || flags[kFLAGS.EMBER_GENDER] == 3)) {
		outputText("Something suddenly grabs you and lifts you by the leg. You peer up at Ember, and it’s clear she’s angry.\n\n");
...
public function HaveAGirlBath():void {
	clearOutput();
	outputText("You sigh in relief as you remove your clothes and head to the bath with the other girls.\n\n");
    if (player.gender == 3) {
        ...
        outputText("\n\nWhile relaxing in the warm water, you notice that the girls pay <i>really</i> close attention to your [cock].");
        if (player.cor < 33)
            outputText("  You feel shy under their hungry gazes, but try not to show your nervousness, attempting to cover your dick with your hands.\n\n");
        else
            outputText("  You quickly notice their looks and even give a few occasional strokes to your dick, bringing it to full mast. Your efforts pay off — some of the girls stop chatting with each other and openly stare at you.\n\n");
    }
	if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) outputText("Amily is sitting in the left corner. Naturally, she’s a little shy about her modest breasts.\n\n");
	if (flags[kFLAGS.JOJO_BIMBO_STATE] == 3 && flags[kFLAGS.JOY_COCK_SIZE] < 1) outputText("Joy is quite silent for once, you expected her to constantly blabber lewd comments about everyone's nudity.\n\n");
	if (followerKiha()) outputText("Kiha definitely doesn’t have a size complex, with her huge breasts leaving most of the other girls in the shade.\n\n");
	...
	if (player.hasStatusEffect(StatusEffects.CampMarble) && flags[kFLAGS.FOLLOWER_AT_FARM_MARBLE] == 0) outputText("Marble easily has the largest pair here, beating all of the other girls without contest, They’re probably the milkiest too.\n\n");
	if (isabellaFollower() && flags[kFLAGS.FOLLOWER_AT_FARM_ISABELLA] == 0 && !player.hasStatusEffect(StatusEffects.SophieOff)) outputText("Isabella sings as she bathes, which is not surprising. You assume they’re folk songs from her homeland.\n\n");
	...
	if (flags[kFLAGS.NADIA_FOLLOWER] >= 6 && !player.hasStatusEffect(StatusEffects.NadiaOff)) outputText("Nadia is resting next to a set of medicinal and aromatic oils she regularly sprays in the bath. Can’t say this is not welcome.\n\n");
	//if (flags[kFLAGS.MICHIKO_FOLLOWER] >= 1) Michiko
	if (flags[kFLAGS.MITZI_RECRUITED] >= 4) outputText("Your resident goblin Mitzi saunters over, looking excited. She strips what little clothing she wears then climbs into the warm waters. Her large tits help her stay afloat as she lays back and relaxes, letting out a sigh of relief.\n\n");
	if (flags[kFLAGS.EXCELLIA_RECRUITED] >= 33) outputText("Excellia comes over to enjoy the soothing waters. She climbs in, letting out a content [exc moo] as she lays back, letting the warm waters wash over her.\n\n");
	if (flags[kFLAGS.LUNA_FOLLOWER] >= 4 && !player.hasStatusEffect(StatusEffects.LunaOff)) outputText("Luna is enjoying a break, relaxing for once, as she is not on duty" + (flags[kFLAGS.LUNA_FOLLOWER] > 6 ? ". You notice that she kicks off when she thinks nobody's watching, doing paddling like a dog in the water for a few moments. As she does, a childlike smile grows on her face." : "") + ".\n\n");
	if (flags[kFLAGS.IZMA_FOLLOWER_STATUS] == 1 && flags[kFLAGS.FOLLOWER_AT_FARM_IZMA] == 0) {
		if (flags[kFLAGS.IZMA_NO_COCK] == 0) outputText("While not entirely a girl, Izma is womanly enough that she's' still accepted in the bath, her crotch well hidden behind a cloth.\n\n");
		else outputText("While she was not always a girl, Izma is womanly enough that she's still accepted in the bath.\n\n");
	}
	...
	if (amilyScene.amilyFollower() && !amilyScene.amilyCorrupt()) {
		...
		outputText(". This friendly competition continues for a few minutes, before escalating as everyone starts groping at each other to compare their sizes. Perhaps it’s the water temperature, but soon groping is no longer enough, and caresses and kisses begin to be exchanged all over the pool. ");
		outputText("A hand trails to your pussy, and in the confusion of the moment, you join in, starting to finger your neighbor. The communal bath quickly takes a lewder turn as everyone starts playing with each other, devolving into a heated orgy. Amidst the chaos, you kiss your nearest neighbor and fully surrender to the heat of the moment.\n\n");
	}
	if (flags[kFLAGS.FUCK_FLOWER_LEVEL] >= 4 || flags[kFLAGS.FLOWER_LEVEL] >= 4) outputText("At some point during the orgy, you hear Holli yelling that it’s unfair that she’s rooted in place at the other edge of the camp while everyone is enjoying themselves.\n\n");
...
public function HeavenTribulationThunderDoom():void {
	...
	outputText("Whatever the reason was, you did not fail this tribulation. If you ever reincarnate, you might have a second chance.\n\n");
	//[GAME OVER]
	EventParser.gameOver();
}
public function HclassHTintro():void {
	spriteSelect(null);
	outputText("\nAn expanse of ink-black clouds form from nowhere, engulfing the land in near-total darkness. You stand, looking up into the artificial night. You see a part of the clouds, directly above you, and from it, crimson lightning splits the sky, carving a trench in the ground in front of you. The unnatural lightning spreads, jagged webs across the black sky, the thunder roaring constantly in your ears. Your heart beats faster, your [skin] crawling with each crack. The wind howls, and you break into a cold sweat. Your first tribulation has arrived.\n");
...
public function HclassHTbeaten():void {
	...
	outputText("<b>You're now an Early Soul Sprite.\nYou can now cultivate the Daos of Elements.\n");
...
public function GclassHTintro():void {
	spriteSelect(null);
	outputText("\nAn expanse of ink-black clouds form out of nowhere, engulfing the land in near-total darkness. You stand, looking up into the artificial night. You see a part of the clouds, directly above you, and from it, crimson lightning splits the sky, carving a trench in the ground in front of you. The unnatural lightning spreads, jagged webs across the black sky, the thunder roaring constantly in your ears. Your heart beats faster, your [skin] crawling with each crack. The wind howls, and you break into a cold sweat. Your second tribulation has arrived.\n");
	startCombat(new GclassHeavenTribulation());
}
public function GclassHTbeaten():void {
	clearOutput();
	outputText("After your attack, the clouds part, the clear sky showing through. The hole widens, clouds scattered every which way by your counterattacks. Looking upward with contempt, you feel pride welling up within you. Your body feels lighter, stronger, your SoulForce pumping like blood through you. You kneel, feeling your SoulForce breaking through blockages in your body"+(player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 3 ? " and soul":"")+". You feel a silent 'pop' inside your body as your cultivation base makes a breakthrough.\n\n");
	outputText("<b>You're now an Early Soul Elder.\nYou can now fly freely.\n");
	if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 0) {
		var BTB:Number = 1;
		if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 1) BTB += 2;
		if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 2) BTB += 3;
...
public function GclassHTsurvived():void {
	clearOutput();
	outputText("You survived. You're in rough shape, but things could be worse. The Tribulation clouds disperse now that your trial has ended. You also feel a silent 'pop' inside your body as your cultivation base makes a breakthrough.\n\n");
	outputText("<b>You're now an Early Soul Elder.\nYou can now fly freely.\n");
	outputText("(Gained Perk: G class Heaven Tribulation Survivor)</b>\n");
	player.createPerk(PerkLib.GclassHeavenTribulationSurvivor, 0, 0, 0, 0);
	player.createPerk(PerkLib.SoulElder, 0, 0, 0, 0);
...
public function FclassHTbeaten():void {
	...
	outputText("<b>You're now an Early Soul Overlord.\nYou can now create a clone, which could serve as a main body replacement, in case something bad happens to your main body.\n");
	if (player.statusEffectv1(StatusEffects.HeavenTribulationCR) > 0) {
		var BTBa:Number = 1;
...
public function FFclassHTintro():void {
	spriteSelect(null);
	outputText("\nAn expanse of ink-black clouds form from nowhere, engulfing the land in near-total darkness. You stand, looking up into the artificial night. You see a part of the clouds, directly above you, and from it, crimson lightning splits the sky, carving a trench in the ground in front of you. The unnatural lightning spreads, jagged webs across the black sky, the thunder roaring constantly in your ears. Your heart beats faster, your [skin] crawling with each crack. The wind howls, and you break into a cold sweat. Your fourth tribulation has arrived.\n");
...
public function EclassHTintro():void {
	spriteSelect(null);
	outputText("\nAn expanse of ink-black clouds form from nowhere, engulfing the land in near-total darkness. You stand, looking up into the artificial night. You see a part of the clouds, directly above you, and from it, crimson lightning splits the sky, carving a trench in the ground in front of you. The unnatural lightning spreads, jagged webs across the black sky, the thunder roaring constantly in your ears. Your heart beats faster, your [skin] crawling with each crack. The wind howls, and you break into a cold sweat. Your fifth tribulation has arrived.\n");
...
public function goblinsBirthScene(womb:int = 0):void {
	...
	if (daughtersCount > 1) outputText("Each new baby you pop out is a new orgasm, and by the end of it your tongue is panting out from pleasure. ");
...
private function goblinsBirthScene2(womb:int = 0):void {
	...
	if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] == daughtersCount) {
		...
		outputText("\"<i>Morning, Mom. So, I was thinking about your setup here and decided that, instead of running around aimlessly in the wild, I might as well stay around your camp. You might actually need my help to get your work going.</i>\"\n\n");
...
public function PCGoblinDaughters():void {
	...
	if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 9 && player.hasStatusEffect(StatusEffects.PCDaughtersWorkshop)) {
		outputText("You check up on your goblin daughters at the workshop. " + flags[kFLAGS.ELDEST_GOBLIN_DAUGHTER] + " sees you approaching, and hollers an order at her younger sisters. They scramble, and she comes over to greet you.\n\n");
		outputText("\"<i>Sup Mom, how ya doin'? Can I interest you into our newest project, or are you looking for something else?</i>\"\n\n");
	}
		...
		outputText("\"<i>Sup Mom, how ya doin'? Can I interest you into our newest project? You know… if there were more of us we could actually build a workshop.</i>\"\n\n");
	}
	else {
	...
	if (flags[kFLAGS.PC_GOBLIN_DAUGHTERS] > 9 && player.hasStatusEffect(StatusEffects.PCDaughtersWorkshop)) {
		...
		else addButtonDisabled(2, "Engineering", "You need to be a goblin for this.");
...
public function PCGoblinDaughtersBuilingWorkshop():void {
	clearOutput();
	outputText("You wake up this morning and notice a new structure in your camp that you didn’t see before. The sounds of hammering and drilling echo from inside. Upon closer inspection, you realize your daughters have finally built that workshop they were talking about. Inside, your daughters are busy, tinkering with inventions or disassembling scrap metal. " + flags[kFLAGS.ELDEST_GOBLIN_DAUGHTER] + " looks up from a nearby workbench and walks over to greet you.\n\n");
	outputText("\"<i>Good morning, Ma! we finally built that workshop we were all dreaming of! Heh, it didn’t take long once we all pitched in. We’re all working on our own private projects here. You see that scrap pile over there? If there are any parts you want, feel free to take some. It’s the least we can do. </i>\"\n\n");
	outputText("Well, how nice of them! You’re sure you’ll be digging into the spare parts every now and then. Not to mention, this place is a clean and productive area to work on your own projects. You make sure to thank your eldest for setting this up for everyone.\n\n");
...
public function PCGoblinDaughtersBuilingWorkshopSpareParts():void {
	clearOutput();
	outputText("You look over your daily set of spare parts and happily notice that you've acquired the following items: <b>");
